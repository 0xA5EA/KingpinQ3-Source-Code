...
void CopyToBodyQue(gentity_t *ent)
{
  ...
  if (ent->s.eFlags & EF_KAMIKAZE)
  {
    ...
    for (i = 0; i < MAX_GENTITIES; i++)
    {
      ...
      if (qstrcmp(e->classname, "kamikaze timer"))
        continue;
...
team_t PickTeam(int ignoreClientNum)
{
  ...
  counts[TEAM_NIKKIS] = TeamCount(ignoreClientNum, TEAM_NIKKIS);
  counts[TEAM_DRAGONS] = TeamCount(ignoreClientNum, TEAM_DRAGONS);

  if (counts[TEAM_NIKKIS] > counts[TEAM_DRAGONS])
    return TEAM_DRAGONS;

  if (counts[TEAM_DRAGONS] > counts[TEAM_NIKKIS])
    return TEAM_NIKKIS;

  // equal team count, so join the team with the lowest score
  if (level.teamScores[TEAM_NIKKIS] > level.teamScores[TEAM_DRAGONS])
    return TEAM_DRAGONS;
...
void ClientUserinfoChanged(int clientNum)
{
  ...
  char dragonTeam[MAX_INFO_STRING];
  char nikkiTeam[MAX_INFO_STRING];
...
//unlagged - client options
  ...
  {
    if (qstrcmp(oldname, client->pers.netname))
...
char *ClientConnect(int clientNum, qboolean firstTime, qboolean isBot)
{
  ...
  //Info_ValueForKey(temp, "version"))
  if (!(ent->r.svFlags & SVF_BOT) && (qstrcmp(value, "localhost") != 0))
  {
    // check for a password
    value = Info_ValueForKey(userinfo, "password");
    if (g_password.string[0] && Q_stricmp(g_password.string, "none") && qstrcmp(g_password.string, value) != 0)
  ...
  //		client->areabits = G_Alloc( (trap_AAS_PointReachabilityAreaIndex( NULL ) + 7) / 8 );
	G_SendClientPmoveParams(clientNum); //unvan .52
...
//unlagged - backward reconciliation #3
  ...
  Com_Memset(client, 0, sizeof(*client)); // bk FIXME: Com_Memset?
  ...
  if (g_weaponmod.integer < WM_HITMEN)
  {
    ent->health += PLAYER_RESPAWN_HEALTH_ADD;
    client->ps.stats[STAT_HEALTH] += PLAYER_RESPAWN_HEALTH_ADD;
  }
  ...
  ent->antispawncamp = level.time+ (g_antiSpawnCamp.integer *1000);
