...
#include "g_local.h"
//#include "q_shared.h"  // id way
#include "../qcommon/q_shared.h"   // io-way
/*  id-way
...
void QDECL BotAI_Print(int type, char *fmt, ...)
{
  ...
  case PRT_MESSAGE:
  {
    G_Printf("Botlib: %s", str);
    break;
  }
  case PRT_WARNING:
  {
    G_Printf(S_COLOR_YELLOW "Botlib Warning: %s", str);
    break;
  }
  case PRT_ERROR:
  {
    G_Printf(S_COLOR_RED "Botlib Error: %s", str);
    break;
  }
  case PRT_FATAL:
  {
    G_Printf(S_COLOR_RED "Botlib Fatal: %s", str);
    break;
  }
  case PRT_EXIT:
  {
    G_Error(S_COLOR_RED "Botlib Exit: %s", str);
    break;
  }
  default:
  {
...
void BotReportStatus(bot_state_t *bs)
{
  ...
  if (gametype == GT_CTF)
  {
    if (BotCTFCarryingFlag(bs))
    {
      if (BotTeam(bs) == TEAM_DRAGONS)
        ...
        strcpy(flagstatus, S_COLOR_YELLOW"F ");
    }
  }
  ...
  //FIXME(0xA5EA): BAGMAN
   ...
   if (BotTeam(bs) == TEAM_DRAGONS) Com_sprintf(flagstatus, sizeof(flagstatus), S_COLOR_RED"%2d", bs->inventory[INVENTORY_REDCUBE]);
...
void BotTeamplayReport(void)
{
  ...
  for (i = 0; i < maxclients && i < MAX_CLIENTS; i++)
  {
    ...
    if (!qstrlen(buf) || !qstrlen(Info_ValueForKey(buf, "n")))
      continue;
    //skip spectators
    if (atoi(Info_ValueForKey(buf, "t")) == TEAM_DRAGONS)
    {
  ...
  for (i = 0; i < maxclients && i < MAX_CLIENTS; i++)
  {
    ...
    if (!qstrlen(buf) || !qstrlen(Info_ValueForKey(buf, "n")))
      continue;
    //skip spectators
    if (atoi(Info_ValueForKey(buf, "t")) == TEAM_NIKKIS)
    {
      BotReportStatus(botstates[i]);
    }
  }
...
void BotSetInfoConfigString(bot_state_t *bs)
{
  ...
  else if (gametype == GT_1FCTF)
  {
   ...
   if (BotHarvesterCarryingCubes(bs)) {
   if (BotTeam(bs) == TEAM_DRAGONS) Com_sprintf(carrying, sizeof(carrying), "%2d", bs->inventory[INVENTORY_REDCUBE]);
...
void BotInterbreedEndMatch(void)
{
  ...
  if (bot_interbreedmatchcount >= bot_interbreedcycle.integer)
  {
    ...
    if (qstrlen(bot_interbreedwrite.string))
    {
...
void BotInterbreeding(void)
{
  ...
  if (!qstrlen(bot_interbreedchar.string))
    return;
  ...
  while (trap_BotGetServerCommand(client, buf, sizeof(buf)))
  {
    ...
    *args++ = '\0';
    ...
    else if (!Q_stricmp(buf, "print"))
    {
      ...
      Com_Memmove(args, args + 1, qstrlen(args));
      args[qstrlen(args) - 1] = '\0';
...
void BotWriteSessionData(bot_state_t *bs)
{
  ...
  s = va("%i %i %i %i %i %i %i %i"
    " %f %f %f"
    " %f %f %f"
    " %f %f %f", bs->lastgoal_decisionmaker, bs->lastgoal_ltgtype, bs->lastgoal_teammate, bs->lastgoal_teamgoal.areanum,
      bs->lastgoal_teamgoal.entitynum, bs->lastgoal_teamgoal.flags, bs->lastgoal_teamgoal.iteminfo,
      bs->lastgoal_teamgoal.number, bs->lastgoal_teamgoal.origin[0], bs->lastgoal_teamgoal.origin[1],
      bs->lastgoal_teamgoal.origin[2], bs->lastgoal_teamgoal.mins[0], bs->lastgoal_teamgoal.mins[1],
      bs->lastgoal_teamgoal.mins[2], bs->lastgoal_teamgoal.maxs[0], bs->lastgoal_teamgoal.maxs[1],
...
int BotAIStartFrame(int time)
{
  ...
  if (botlib_residual >= thinktime)
  {
    ...
    for (i = 0; i < MAX_GENTITIES; i++)
    {
      ...
      if (ent->s.eType == ET_MISSILE && ent->s.weapon != WP_GRAPPLING_HOOK && ent->s.weapon != WP_FLAMER)
      {
        trap_BotLibUpdateEntity(i, NULL);
        continue;
      }
...
int BotInitLibrary(void)
{
  ...
  trap_BotLibVarSet("log", "1"); //buf);			//0xA5EA, enables botlib loggin ?
