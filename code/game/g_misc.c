...
{
  G_Printf(S_COLOR_CYAN"SP_misc_model\n");
#if 0
...
void Use_Shooter(gentity_t *ent, gentity_t *other, gentity_t *activator)
{
	...
	switch(ent->s.weapon)
	{
		...
		case WP_ROCKET_LAUNCHER:
			...
			break;
		case WP_FLAMER: // Added -KRYPTYK
			fire_flamechunk(ent, ent->s.origin, dir);
			break;
	}
...
//FIXME(0xA5EA): bagman, what about PortalTouch
...
//	}
	...
	else if(other->client->ps.powerups[PW_DRAGONFLAG])      // only happens in standard CTF
	{
		Drop_Item(other, BG_FindItemForPowerup(PW_DRAGONFLAG), 0);
		other->client->ps.powerups[PW_DRAGONFLAG] = 0;
	}
	else if(other->client->ps.powerups[PW_NIKKIFLAG])       // only happens in standard CTF
	{
		Drop_Item(other, BG_FindItemForPowerup(PW_NIKKIFLAG), 0);
		other->client->ps.powerups[PW_NIKKIFLAG] = 0;
	}
