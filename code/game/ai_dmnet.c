...
int BotGetLongTermGoal(bot_state_t *bs, int tfl, int retreat, bot_goal_t *goal)
{
    ...
    if (BotEntityVisible(bs->entitynum, bs->eye, bs->viewangles, 360, bs->teammate))
    {
      ...
      if (VectorLengthSquared(dir) < Square(bs->formation_dist))
      {
        ...
        if (botinfo.origin[2] + botinfo.maxs[2] > entinfo.origin[2] + entinfo.mins[2])
        {
          ...
          if (botinfo.origin[0] + botinfo.maxs[0] > entinfo.origin[0] + entinfo.mins[0] - 4 && botinfo.origin[0]
              + botinfo.mins[0] < entinfo.origin[0] + entinfo.maxs[0] + 4)
          {
            if (botinfo.origin[1] + botinfo.maxs[1] > entinfo.origin[1] + entinfo.mins[1] - 4 && botinfo.origin[1]
                + botinfo.mins[1] < entinfo.origin[1] + entinfo.maxs[1] + 4)
            {
              if (botinfo.origin[2] + botinfo.maxs[2] > entinfo.origin[2] + entinfo.mins[2] - 4 && botinfo.origin[2]
                  + botinfo.mins[2] < entinfo.origin[2] + entinfo.maxs[2] + 4)
              {
  ...
  if (gametype == GT_CTF)
  {
    ...
    if (bs->ltgtype == LTG_GETFLAG)
    {
      ...
      {
      case TEAM_DRAGONS:
        Com_Memcpy(goal, &ctf_blueflag, sizeof(bot_goal_t));
        break;
      case TEAM_NIKKIS:
        Com_Memcpy(goal, &ctf_redflag, sizeof(bot_goal_t));
        break;
      default:
        bs->ltgtype = 0;
        return qfalse;
  ...
  else if (gametype == GT_1FCTF)
  {
    ...
    if (bs->ltgtype == LTG_RUSHBASE)
    {
      switch(BotTeam(bs))
      {
        case TEAM_DRAGONS: Com_Memcpy(goal, &ctf_blueflag, sizeof(bot_goal_t)); break;
        case TEAM_NIKKIS: Com_Memcpy(goal, &ctf_redflag, sizeof(bot_goal_t)); break;
  ...
  else if (gametype == GT_OBELISK)
  {
        ...
        bs->attackaway_time < FloatTime())
    {
      ...
      switch(BotTeam(bs))
      {
        case TEAM_DRAGONS: Com_Memcpy(goal, &blueobelisk, sizeof(bot_goal_t)); break;
        case TEAM_NIKKIS: Com_Memcpy(goal, &redobelisk, sizeof(bot_goal_t)); break;
        default: bs->ltgtype = 0; return qfalse;
      }
...
{
  // 0xA5EA, FIXME:	//hypov8 should they be in a specific order to??
  if	  (bs->inventory[INVENTORY_PISTOL] > 0 &&			bs->inventory[INVENTORY_BULLETS] > 0)		return WEAPONINDEX_PISTOL;
  else if (bs->inventory[INVENTORY_SHOTGUN] > 0 &&			bs->inventory[INVENTORY_SHELLS] > 0)		return WEAPONINDEX_SHOTGUN;
  else if (bs->inventory[INVENTORY_MACHINEGUN] > 0 &&		bs->inventory[INVENTORY_CYLINDER] > 0)		return WEAPONINDEX_MACHINEGUN;
  else if (bs->inventory[INVENTORY_GRENADELAUNCHER] > 0 &&	bs->inventory[INVENTORY_GRENADES] > 0)		return WEAPONINDEX_GRENADE_LAUNCHER;
  else if (bs->inventory[INVENTORY_ROCKETLAUNCHER] > 0 &&	bs->inventory[INVENTORY_ROCKETS] > 0)		return WEAPONINDEX_ROCKET_LAUNCHER;
  else if (bs->inventory[INVENTORY_HMG] > 0 &&				bs->inventory[INVENTORY_308] > 0)			return WEAPONINDEX_HMG;
  else if (bs->inventory[INVERTORY_FLAMER] > 0 &&			bs->inventory[INVENTORY_FLAMETANK] > 0)		return WEAPONINDEX_FLAMER;
  else
  {
    return -1;
  }
...
void BotClearPath(bot_state_t *bs, bot_moveresult_t *moveresult)
{
  ...
  if (bs->blockedbyavoidspot_time > FloatTime() && !(moveresult->flags & (MOVERESULT_MOVEMENTVIEW | MOVERESULT_MOVEMENTWEAPON)))
  {
    ...
    {
      //
      // state->generic1 == TEAM_DRAGONS || state->generic1 == TEAM_NIKKIS
      //
