...
int trap_BotLibStartFrame(float time)
{
  floatint_t fi;
	...
	return syscall(BOTLIB_START_FRAME, fi.i);
}
...
int trap_AAS_PredictClientMovement(void /* struct aas_clientmove_s */ *move, int entnum, vec3_t origin, int presencetype, int onground, vec3_t velocity, vec3_t cmdmove, int cmdframes, int maxframes, float frametime, int stopevent, int stopareanum, int visualize)
{
  floatint_t fi;
	...
	return syscall(BOTLIB_AAS_PREDICT_CLIENT_MOVEMENT, move, entnum, origin, presencetype, onground, velocity, cmdmove, cmdframes, maxframes, fi.i, stopevent, stopareanum, visualize);
}
...
void trap_EA_Move(int client, vec3_t dir, float speed)
{
  floatint_t fi;
	...
	syscall(BOTLIB_EA_MOVE, client, dir, fi.i);
}
...
void trap_EA_EndRegular(int client, float thinktime)
{
  floatint_t fi;
	...
	syscall(BOTLIB_EA_END_REGULAR, client, fi.i);
}
...
void trap_EA_GetInput(int client, float thinktime, void /* struct bot_input_s */ *input)
{
  floatint_t fi;
	...
	syscall(BOTLIB_EA_GET_INPUT, client, fi.i, input);
}
...
int trap_BotLoadCharacter(char *charfile, float skill)
{
  floatint_t fi;
	...
	return syscall(BOTLIB_AI_LOAD_CHARACTER, charfile, fi.i);
}
...
{
	floatint_t fi, minfi, maxfi;
	minfi.f = min;
	maxfi.f = max;

	fi.i = syscall(BOTLIB_AI_CHARACTERISTIC_BFLOAT, character, index, minfi.i, maxfi.i);
	return fi.f;
}
...
int trap_BotChooseNBGItem(int goalstate, vec3_t origin, int *inventory, int travelflags, void /* struct bot_goal_s */ *ltg, float maxtime)
{
  floatint_t fi;
	...
	return syscall(BOTLIB_AI_CHOOSE_NBG_ITEM, goalstate, origin, inventory, travelflags, ltg, fi.i);
}
...
void trap_BotSetAvoidGoalTime(int goalstate, int number, float avoidtime)
{
  floatint_t fi;
	...
	syscall(BOTLIB_AI_SET_AVOID_GOAL_TIME, goalstate, number, fi.i);
}
...
void trap_BotAddAvoidSpot(int movestate, vec3_t origin, float radius, int type)
{
  floatint_t fi;
	...
	syscall(BOTLIB_AI_ADD_AVOID_SPOT, movestate, origin, fi.i, type);
}
...
int trap_BotMoveInDirection(int movestate, vec3_t dir, float speed, int type)
{
  floatint_t fi;
	...
	return syscall(BOTLIB_AI_MOVE_IN_DIRECTION, movestate, dir, fi.i, type);
}
...
int trap_BotMovementViewTarget(int movestate, void /* struct bot_goal_s */ *goal, int travelflags, float lookahead, vec3_t target)
{
  floatint_t fi;
	...
	return syscall(BOTLIB_AI_MOVEMENT_VIEW_TARGET, movestate, goal, travelflags, fi.i, target);
}
