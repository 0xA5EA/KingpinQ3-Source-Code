...
char *BotRandomWeaponName(void)
{
  ...
  case 2:
    return "Machinegun";
  case 3:
    return "Grenade Launcher"; //FIXME: 0xA5EA
  case 4:
    return "Rocket Launcher";
  case 5:
    return "Plasmagun";
...
int BotValidChatPosition(bot_state_t *bs)
{
  ...
  if (bs->inventory[INVENTORY_HASTE] || bs->inventory[INVENTORY_INVISIBILITY] || bs->inventory[INVENTORY_REGEN]
  ...
  else
  {
    ...
    if (bs->botdeathtype == MOD_WATER)
      ...
      BotAI_BotInitialChat(bs, "death_cratered", BotRandomOpponentName(bs), NULL);
        ...
        bs->botdeathtype == MOD_CRUSH || bs->botdeathtype == MOD_SUICIDE || bs->botdeathtype == MOD_TARGET_LASER
    ...
    {
      if ((bs->botdeathtype == MOD_CROWBAR || bs->botdeathtype == MOD_HMG) && random() < 0.5)
      {
        //FIXME(0xA5EA):
        if (bs->botdeathtype == MOD_CROWBAR)
          BotAI_BotInitialChat(bs, "death_gauntlet", name, // 0
                               BotWeaponNameForMeansOfDeath(bs->botdeathtype), // 1
                               NULL);
        else if (bs->botdeathtype == MOD_HMG)
...
int BotChat_Kill(bot_state_t *bs)
{
  ...
  else
  {
    ...
    //
    if (bs->enemydeathtype == MOD_CROWBAR)
    {
      BotAI_BotInitialChat(bs, "kill_gauntlet", name, NULL);
    }
    else if (bs->enemydeathtype == MOD_HMG)
    {
