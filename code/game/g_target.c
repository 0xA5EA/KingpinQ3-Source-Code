    ...
    gentity_t *activator)
{
  ...
  if (activator->client->ps.powerups[PW_DRAGONFLAG])
    Team_ReturnFlag(TEAM_DRAGONS);
  else if (activator->client->ps.powerups[PW_NIKKIFLAG])
    Team_ReturnFlag(TEAM_NIKKIS);
  else if (activator->client->ps.powerups[PW_NEUTRALFLAG])
    Team_ReturnFlag(TEAM_FREE);


  /*hypov8*/ //PW_STOLENSCASH
...
/*QUAKED target_print (1 0 0) (-8 -8 -8) (8 8 8) dragonTeam nikkiTeam private
...
void Use_Target_Print(gentity_t *ent, gentity_t *other, gentity_t *activator)
{
  ...
  //FIXME(0xA5EA): wtf is this spawnflags "cp"
  if (ent->spawnflags & 3)
  {
    if (ent->spawnflags & 1)
      G_TeamCommand(TEAM_DRAGONS, va("cp \"%s\"", ent->message));

    if (ent->spawnflags & 2)
      G_TeamCommand(TEAM_NIKKIS, va("cp \"%s\"", ent->message));
...
{
  if (ent->spawnflags & 3 || ent->soundLooping) //add hypov8 || ent->soundLooping
  {
    if (ent->s.loopSound)
  ...
  {
    if (ent->spawnflags & 8 || ent->soundActivator)    //add hypov8 || ent->soundActivator
      G_AddEvent(activator, EV_GENERAL_SOUND, ent->soundIndex);
    else if (ent->spawnflags & 4 || ent->soundGlobal)  //add hypov8 || ent->soundGlobal
...
void SP_target_speaker(gentity_t *ent)
{
  ...
  if (s[0] == '*')
    ent->spawnflags |= 8;
	//ent->soundActivator = qtrue; //hypov8 merge:
  ...
  if (ent->spawnflags & 1 || ent->soundLooping) //add hypov8 || ent->soundLoopin
    ent->s.loopSound = ent->soundIndex; // start it

  ent->use = Use_Target_Speaker;  //goto-> check for triggered+loop

  if (ent->spawnflags & 4 || ent->soundGlobal) //add hypov8 || ent->soundGlobal
...
{
  if ((self->spawnflags & 1) && activator->client && activator->client->sess.sessionTeam != TEAM_DRAGONS)
    return;

  if ((self->spawnflags & 2) && activator->client && activator->client->sess.sessionTeam != TEAM_NIKKIS)
    return;
