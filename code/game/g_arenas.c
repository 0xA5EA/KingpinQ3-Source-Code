...
void UpdateTournamentInfo(void)
{
  ...
  assist_excellent.s = player->client->ps.persistant[PERS_EXCELLENT_ASSIST_COUNT];
  ...
  else // 0xA5EA, für ACC benutzen
  {
    Com_Printf("player->client->accuracy_hits = %d\n", player->client->accuracy_hits);
    Com_Printf("player->client->accuracy_shots = %d\n", player->client->accuracy_shots);
    ...
    {
      score1 = level.teamScores[TEAM_DRAGONS];
      score2 = level.teamScores[TEAM_NIKKIS];
      if (level.clients[playerClientNum].sess.sessionTeam == TEAM_DRAGONS)
      {
        won = (level.teamScores[TEAM_DRAGONS] > level.teamScores[TEAM_NIKKIS]);
      }
      else
      {
        won = (level.teamScores[TEAM_NIKKIS] > level.teamScores[TEAM_DRAGONS]);
      }
    }
    else
    {
    ...
    Com_sprintf(msg, sizeof(msg), "postgame %i %i %i %i %i %i %i %i %i %i %i %i %i %i", level.numNonSpectatorClients,
                playerClientNum, accuracy, player->client->ps.persistant[PERS_FRAG_COUNT],
                assist_excellent.b[PERS_EXCELLENT_POS], player->client->ps.persistant[PERS_DEFEND_COUNT],
                assist_excellent.b[PERS_ASSIST_POS], player->client->ps.persistant[PERS_KILLED],
                player->client->ps.persistant[PERS_SCORE], perfect, score1, score2, level.time,
    ...
    //	            player->client->ps.persistant[PERS_FRAG_COUNT], player->client->ps.persistant[PERS_EXCELLENT_COUNT],
    //	            player->client->ps.persistant[PERS_KILLED], player->client->ps.persistant[PERS_SCORE],
    //	            perfect);
    //#endif
  }
  ...
  for (i = 0; i < level.numNonSpectatorClients; i++)
  {
    ...
    buflen = qstrlen(buf);
    if ((unsigned int) (msglen + buflen + 1) >= sizeof(msg))
      break;
