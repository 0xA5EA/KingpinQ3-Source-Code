...
#ifdef AI_BM
// TODO: hypov8
bot_goal_t bm_dragonsafe;
bot_goal_t bm_nikkisafe;
bot_goal_t bm_cash; //should be cash and bags?
bot_goal_t bm_cash_bag;
bot_goal_t bm_cash_stolen;
#endif
//FIXME(0xA5EA): bagman
/*  // FIXME: 0xA5EA
...
int BotCTFCarryingFlag(bot_state_t *bs)
{
  ...
  if (bs->inventory[INVENTORY_DRAGONFLAG] > 0)
    return CTF_FLAG_DRAGON;
  else if (bs->inventory[INVENTORY_NIKKIFLAG] > 0)
...
==================
FindItemCash
hypov8 help.. find cash
==================
*/
bot_goal_t FindItemCash( char itemname, bot_state_t  *bs)
{
  //levelitem_t *li;
...
int BotTeam(bot_state_t *bs)
{
  ...
  //
  if (atoi(Info_ValueForKey(info, "t")) == TEAM_DRAGONS)
    return TEAM_DRAGONS;
  else if (atoi(Info_ValueForKey(info, "t")) == TEAM_NIKKIS)
    return TEAM_NIKKIS;
  return TEAM_FREE;
}
...
{
  if (entinfo->powerups & (1 << PW_DRAGONFLAG))
    return qtrue;
  if (entinfo->powerups & (1 << PW_NIKKIFLAG))
    return qtrue;
...
qboolean EntityIsInvisible(aas_entityinfo_t *entinfo)
{
  ...
  if (EntityCarriesFlag(entinfo)) //hypov8 todo: no invisable
    return qfalse;
...
int BotHarvesterCarryingCubes(bot_state_t *bs)
{
  ...
  if (bs->inventory[INVENTORY_DRAGONCUBE] > 0) return qtrue;
...
void BotCTFSeekGoals(bot_state_t *bs)
{
  ...
  if (flagstatus == 1)
  {
    ...
    if (bs->owndecision_time < FloatTime())
    {
      ...
      if (!(bs->ltgtype == LTG_DEFENDKEYAREA && (bs->teamgoal.number == ctf_redflag.number || bs->teamgoal.number
          == ctf_blueflag.number)))
      {
  ...
  else if (flagstatus == 2)
  {
    ...
    if (bs->owndecision_time < FloatTime())
    {
      ...
      if (bs->ltgtype != LTG_GETFLAG && bs->ltgtype != LTG_RETURNFLAG && bs->ltgtype != LTG_TEAMHELP && bs->ltgtype
          != LTG_TEAMACCOMPANY && bs->ltgtype != LTG_CAMPORDER && bs->ltgtype != LTG_PATROL && bs->ltgtype != LTG_GETITEM)
      {
  ...
  if (bs->ltgtype == LTG_TEAMHELP || bs->ltgtype == LTG_TEAMACCOMPANY || bs->ltgtype == LTG_DEFENDKEYAREA || bs->ltgtype
      == LTG_GETFLAG || bs->ltgtype == LTG_RUSHBASE || bs->ltgtype == LTG_RETURNFLAG || bs->ltgtype == LTG_CAMPORDER
      || bs->ltgtype == LTG_PATROL || bs->ltgtype == LTG_GETITEM || bs->ltgtype == LTG_MAKELOVE_UNDER || bs->ltgtype
      == LTG_MAKELOVE_ONTOP)
  {
    return;
  }
  ...
  else if (rnd < l2 && ctf_redflag.areanum && ctf_blueflag.areanum)
  {
    ...
    //
    if (BotTeam(bs) == TEAM_DRAGONS)
      Com_Memcpy(&bs->teamgoal, &ctf_redflag, sizeof(bot_goal_t));
    else
      Com_Memcpy(&bs->teamgoal, &ctf_blueflag, sizeof(bot_goal_t));
    //set the ltg type
...
void Bot1FCTFSeekGoals(bot_state_t *bs)
{
  ...
  if (bs->neutralflagstatus == 1)
  {
    if (bs->owndecision_time < FloatTime())
    {
      ...
      if (bs->ltgtype != LTG_ATTACKENEMYBASE)
      {
        ...
        bs->ordered = qfalse;
        //
        if (BotTeam(bs) == TEAM_DRAGONS) Com_Memcpy(&bs->teamgoal, &ctf_blueflag, sizeof(bot_goal_t));
  ...
  else if (rnd < l2 && ctf_redflag.areanum && ctf_blueflag.areanum)
  {
    ...
    bs->ordered = qfalse;
    //
    if (BotTeam(bs) == TEAM_DRAGONS) Com_Memcpy(&bs->teamgoal, &ctf_redflag, sizeof(bot_goal_t));
...
void BotBMSeekGoals(bot_state_t *bs)
{
  ...
  else if (!BotBMCarryingCash(bs)) /* get some cash if nothing to do */
  {
    ...
    if (!bm_cash.entitynum)
      BotFindWayPoint;
    BotFindMatch("item_cash",0 );


    VectorSubtract(bs->origin, bm_cash.origin, dir);
    bs->teamgoal_time = 0;

    //if not already searching for cash and not standing on top
    if (bs->ltgtype != LTG_GETITEM && VectorLength(dir) >= 128)
    {
      //BotRefuseOrder(bs);
      //bs->ltgtype = LTG_RUSHBASE;
      bs->ltgtype = LTG_GETITEM;
      ...
      Com_Printf("bm cash origin for player #%i %f %f %f\n ", bs->cur_ps.clientNum, bm_cash.origin[0], bm_cash.origin[1], bm_cash.origin[2]);
      ...
      //direction to item
      VectorSubtract(bs->origin, bm_cash.origin, dir); //hypo
      if (VectorLength(dir) < 4)
      {
        //set the ltg type
        bs->ltgtype = LTG_DEFENDKEYAREA;
        //set the time the bot stops defending the base
        bs->teamgoal_time = FloatTime() + CTF_ROAM_TIME;
        bs->defendaway_time = 0;
        BotSetTeamStatus(bs);
        bs->owndecision_time = FloatTime() + 5;
    ...
    //else if (bs->teamgoal.origin == bm_cash.origin)
    //{
...
void BotObeliskSeekGoals(bot_state_t *bs)
{
  ...
  if (rnd < l1 && redobelisk.areanum && blueobelisk.areanum)
  {
    ...
    bs->ordered = qfalse;
    //
    if (BotTeam(bs) == TEAM_DRAGONS) Com_Memcpy(&bs->teamgoal, &blueobelisk, sizeof(bot_goal_t));
  ...
  else if (rnd < l2 && redobelisk.areanum && blueobelisk.areanum)
  {
    ...
    bs->ordered = qfalse;
    //
    if (BotTeam(bs) == TEAM_DRAGONS) Com_Memcpy(&bs->teamgoal, &redobelisk, sizeof(bot_goal_t));
...
char *EasyClientName(int client, char *buf, int size)
{
  ...
  for (i = 0; name[i]; i++)
    name[i] &= 127;
  //remove all spaces
  for (ptr = Q_strstr(name, " "); ptr; ptr = Q_strstr(name, " "))
  {
    Com_Memmove(ptr, ptr + 1, qstrlen(ptr + 1) + 1);
  }
  ...
  {
    if (str2 > str1)
      Com_Memmove(str1, str2 + 1, qstrlen(str2 + 1) + 1);
    else
      Com_Memmove(str2, str1 + 1, qstrlen(str1 + 1) + 1);
  }
  //remove Mr prefix
  if ((name[0] == 'm' || name[0] == 'M') && (name[1] == 'r' || name[1] == 'R'))
  {
    Com_Memmove(name, name + 2, qstrlen(name + 2) + 1);
  }
...
int BotSynonymContext(bot_state_t *bs)
{
  ...
  //
  if (gametype == GT_CTF || gametype == GT_1FCTF || gametype == GT_BAGMAN) //hypov8 add bm
  {
    if (BotTeam(bs) == TEAM_DRAGONS)
      context |= CONTEXT_CTFDRAGONTEAM;
    else
      context |= CONTEXT_CTFNIKKITEAM;
  }
#ifdef GT_USE_TA_TYPES
   else if (gametype == GT_OBELISK) {
   if (BotTeam(bs) == TEAM_DRAGONS) context |= CONTEXT_OBELISKDRAGONTEAM;
   else context |= CONTEXT_OBELISKNIKKITEAM;
   }
   else if (gametype == GT_HARVESTER) {
   if (BotTeam(bs) == TEAM_DRAGONS) context |= CONTEXT_HARVESTERDRAGONTEAM;
   else context |= CONTEXT_HARVESTERNIKKITEAM;
   }
#endif
  return context;
}
...
void BotUpdateInventory(bot_state_t *bs)
{
  ...
  bs->inventory[INVENTORY_ARMOR_LEGS] = bs->cur_ps.stats[STAT_ARMOR_LEGS];
  bs->inventory[INVENTORY_ARMOR_BODY] = bs->cur_ps.stats[STAT_ARMOR_BODY];
  bs->inventory[INVENTORY_ARMOR_HEAD] = bs->cur_ps.stats[STAT_ARMOR_HEAD];
  //FIXME (0xA5EA): was wird sonst noch mit INVENTORY_ARMOR_HEAD gemacht ?
  //weapons
  bs->inventory[INVENTORY_CROWBAR]			= (bs->cur_ps.stats[STAT_WEAPONS] & (1 << WP_CROWBAR)) != 0;
  ...
  bs->inventory[INVERTORY_FLAMER]			= (bs->cur_ps.stats[STAT_WEAPONS] & (1 << WP_FLAMER)) != 0;
  bs->inventory[INVENTORY_GRAPPLINGHOOK]	= (bs->cur_ps.stats[STAT_WEAPONS] & (1 << WP_GRAPPLING_HOOK)) != 0;
  //FIXME: 0xA5EA, other Weapons to add
   ...
   */
  //ammo
  bs->inventory[INVENTORY_BULLETS]	= bs->cur_ps.ammo_all[WP_MACHINEGUN] + bs->cur_ps.ammo_mag[WP_MACHINEGUN]; //combined WP_PISTOL
  bs->inventory[INVENTORY_SHELLS]		= bs->cur_ps.ammo_all[WP_SHOTGUN] + bs->cur_ps.ammo_mag[WP_SHOTGUN];
  bs->inventory[INVENTORY_CYLINDER]	= bs->cur_ps.ammo_all[WP_MACHINEGUN] + bs->cur_ps.ammo_mag[WP_MACHINEGUN];
  bs->inventory[INVENTORY_GRENADES]	= bs->cur_ps.ammo_all[WP_GRENADE_LAUNCHER] + bs->cur_ps.ammo_mag[WP_GRENADE_LAUNCHER];
  bs->inventory[INVENTORY_ROCKETS]	= bs->cur_ps.ammo_all[WP_ROCKET_LAUNCHER] + bs->cur_ps.ammo_mag[WP_ROCKET_LAUNCHER];
  bs->inventory[INVENTORY_308]		= bs->cur_ps.ammo_all[WP_HMG] + bs->cur_ps.ammo_mag[WP_HMG];
  bs->inventory[INVENTORY_FLAMETANK]	= bs->cur_ps.ammo_all[WP_FLAMER] + bs->cur_ps.ammo_mag[WP_FLAMER];
  ...
  /*  // 0xA5EA, FIXME:
   ...
   #endif
   */
  bs->inventory[INVENTORY_DRAGONFLAG] = bs->cur_ps.powerups[PW_DRAGONFLAG] != 0;
  bs->inventory[INVENTORY_NIKKIFLAG] = bs->cur_ps.powerups[PW_NIKKIFLAG] != 0;
  bs->inventory[INVENTORY_NEUTRALFLAG] = bs->cur_ps.powerups[PW_NEUTRALFLAG] != 0;
  /*added hypov8*/
  //bs->inventory[INVENTORY_BAG_CASH] = bs->cur_ps.powerups[PW_STOLENSCASH];

  bs->inventory[INVENTORY_PLAYER_CASH] = bs->cur_ps.stats[STAT_CASH];
  bs->inventory[INVENTORY_BAG_CASH] = bs->cur_ps.stats[STAT_STOLEN_CASH];
  ...
  if (BotTeam(bs) == TEAM_DRAGONS)
  {
    bs->inventory[INVENTORY_DRAGONCUBE] = bs->cur_ps.generic1;
    bs->inventory[INVENTORY_BLUECUBE] = 0;
  }
  else
  {
    bs->inventory[INVENTORY_DRAGONCUBE] = 0;
...
void BotUseKamikaze(bot_state_t *bs)
{
  ...
  else if (gametype == GT_OBELISK)
  {
    switch(BotTeam(bs))
    {
      case TEAM_DRAGONS: goal = &blueobelisk; break;
...
void BotUseInvulnerability(bot_state_t *bs)
{
  ...
  if (gametype == GT_CTF)
  {
    ...
    switch(BotTeam(bs))
    {
      case TEAM_DRAGONS: goal = &ctf_blueflag; break;
...
void BotBattleUseItems(bot_state_t *bs)
{
  if (bs->inventory[INVENTORY_HEALTH] < 40)
  {
    ...
    if (!BotCTFCarryingFlag(bs) &&
      ...
      !BotBMCarryingStolen(bs) /* add hypov8 */
...
{
#if 0 //hypov8 test new bot agresion
  //if the bot has quad
  if (bs->inventory[INVENTORY_HMG_MOD_COOL])
  { //mit hmgmod cooling austauschen
    //if the bot is not holding the gauntlet or the enemy is really nearby
    if (bs->weaponnum != WP_CROWBAR || bs->inventory[ENEMY_HORIZONTAL_DIST] < 80)
    {
      return 70;
    }
  }
  ...
  if (bs->inventory[INVENTORY_HEALTH] < 60)
    return 0; //FIXME: 0xA5EA, Kingpinq3 max. health 100
  //if the bot is low on health
  if (bs->inventory[INVENTORY_HEALTH] < 80)
  {
    int armor;
    armor = bs->inventory[INVENTORY_ARMOR_LEGS] + bs->inventory[INVENTORY_ARMOR_BODY] + bs->inventory[INVENTORY_ARMOR_HEAD];
    //if the bot has insufficient armor
    if (armor < 20)
      return 0;
    //FIXME (0xA5EA): botArgression ok with that armor calculation ?;
  }
  //if the bot can use the bfg
  if (bs->inventory[INVENTORY_PISTOL] > 0 && bs->inventory[INVENTORY_BULLETS] > 7)
    return 100;
  //if the bot can use the railgun
  if (bs->inventory[INVENTORY_HMG] > 0 && //fixme: 0xA5EA, ok ?
      bs->inventory[INVENTORY_308] > 5)
    return 95;
  //if the bot can use the lightning gun
  if (bs->inventory[INVENTORY_MACHINEGUN] > 0 && bs->inventory[INVENTORY_CYLINDER] > 30)
    return 90;
  ...
  if (bs->inventory[INVERTORY_FLAMER] > 0 && bs->inventory[INVENTORY_FLAMETANK] > 40)
    return 85;
...
//	if (bs->inventory[INVENTORY_HMG_MOD_COOL])
  ...
  //if the enemy is located way higher than the bot
  if (bs->inventory[ENEMY_HORIZONTAL_DIST] < 64) agressionTotal += 50;

  if		(bs->inventory[INVENTORY_HEALTH] < 40)		agressionTotal -= 30;
  else if(bs->inventory[INVENTORY_HEALTH] < 60)		agressionTotal -= 10;
  else 												agressionTotal += 20;

  if ((bs->inventory[INVENTORY_ARMOR_LEGS] +
    bs->inventory[INVENTORY_ARMOR_BODY] +
    bs->inventory[INVENTORY_ARMOR_HEAD]) < 100)
    agressionTotal += 0;
  else
    agressionTotal += 10;


  //if (bs->inventory[INVENTORY_PISTOL] > 0 && bs->inventory[INVENTORY_BULLETS] > 7)			agressionTotal += 10;
  if (bs->inventory[INVENTORY_HMG] > 0 && bs->inventory[INVENTORY_308] > 3)					agressionTotal += 10;
  if (bs->inventory[INVENTORY_MACHINEGUN] > 0 && bs->inventory[INVENTORY_CYLINDER] > 20)		agressionTotal += 10;
  if (bs->inventory[INVENTORY_ROCKETLAUNCHER] > 0 && bs->inventory[INVENTORY_ROCKETS] > 3)	agressionTotal += 10;
  if (bs->inventory[INVERTORY_FLAMER] > 0 && bs->inventory[INVENTORY_FLAMETANK] > 30)			agressionTotal += 30;
  if (bs->inventory[INVENTORY_GRENADELAUNCHER] > 0 && bs->inventory[INVENTORY_GRENADES] > 2)	agressionTotal += 10;
  if (bs->inventory[INVENTORY_SHOTGUN] > 0 && bs->inventory[INVENTORY_SHELLS] > 5)			agressionTotal += 10;
  if (bs->inventory[INVENTORY_PISTOL] > 0 && bs->inventory[INVENTORY_BULLETS] <= 20)			agressionTotal -= 10; //just spawned
  //otherwise the bot is not feeling too good
  return agressionTotal;
#endif
}
...
{
  if (bs->weaponnum == WP_CROWBAR)
    return 100;

  if (bs->inventory[INVENTORY_HEALTH] < 40)
    return 100;
  if (bs->inventory[INVENTORY_BULLETS] < 30)
    return 80;

 // if (bs->weaponnum == WP_MACHINEGUN) //hypov8 disable
   // return 90;

 // if (bs->inventory[INVENTORY_HEALTH] < 60)  //hypov8 disable
   // return 80;
...
int BotWantsToRetreat(bot_state_t *bs)
{
  ...
  {
    //if carrying the bag then always retreat
    if (BotBMCarryingStolen(bs))
        return qtrue; //retreat to base
  }
#endif
...
#endif //hypov8 bot feels bad, use
...
int BotWantsToChase(bot_state_t *bs)
{
  ...
  else if (gametype == GT_1FCTF)
  {
    ...
    //always chase if the enemy is carrying a flag
    BotEntityInfo(bs->enemy, &entinfo);
    if (EntityCarriesFlag(&entinfo)) ///+bm added
      return qtrue;
  }
  else if (gametype == GT_BAGMAN) //add hypov8
  {
    //never chase if carrying the flag
  if (BotBMCarryingStolen(bs))
    return qfalse;
...
int BotCanAndWantsToRocketJump(bot_state_t *bs)
{
  ...
  if (bs->inventory[INVENTORY_HEALTH] < 90)
  {
    ...
    int armor;
    armor = bs->inventory[INVENTORY_ARMOR_LEGS] +
      bs->inventory[INVENTORY_ARMOR_BODY] +
      bs->inventory[INVENTORY_ARMOR_HEAD];
    if (armor < 20)
      return qfalse;
    //FIXME (0xA5EA): botArgression ok with that armor calculation ?;
  }
...
int BotHasPersistantPowerupAndWeapon(bot_state_t *bs)
{
 ...
 //if the bot can use the guns //FIXME: 0xA5EA, ok ?
//	if (bs->inventory[INVENTORY_HMG] > 0 &&		bs->inventory[INVENTORY_308] > 60 &&		bs->inventory[INVENTORY_HMG_MOD_COOL] > 0)	return qtrue;
if (bs->inventory[INVENTORY_ROCKETLAUNCHER] > 0 && bs->inventory[INVENTORY_ROCKETS] > 22)  return qtrue;
if (bs->inventory[INVENTORY_HMG] > 0 && bs->inventory[INVENTORY_308] > 80)					return qtrue;
if (bs->inventory[INVENTORY_MACHINEGUN] > 0 && bs->inventory[INVENTORY_CYLINDER] > 120)    return qtrue;
if (bs->inventory[INVENTORY_GRENADELAUNCHER] > 0 && bs->inventory[INVENTORY_GRENADES] > 10)    return qtrue;
//if (bs->inventory[INVENTORY_SHOTGUN] > 0 && bs->inventory[INVENTORY_SHELLS] > 40)			return qtrue;
//if (bs->inventory[INVENTORY_PISTOL] > 0 && bs->inventory[INVENTORY_BULLETS] > 59)			return qtrue;



  if (bs->inventory[INVERTORY_FLAMER] > 0 && bs->inventory[INVENTORY_FLAMETANK] > 100)  return qtrue;
...
int BotWantsToCamp(bot_state_t *bs)
{
  ...
  if (bs->ltgtype == LTG_TEAMHELP || bs->ltgtype == LTG_TEAMACCOMPANY || bs->ltgtype == LTG_DEFENDKEYAREA || bs->ltgtype
      == LTG_GETFLAG || bs->ltgtype == LTG_RUSHBASE || bs->ltgtype == LTG_CAMP || bs->ltgtype == LTG_CAMPORDER || bs->ltgtype
      == LTG_PATROL)
  {
    return qfalse;
  }
  ...
  if ((bs->inventory[INVENTORY_ROCKETLAUNCHER] <= 0 || bs->inventory[INVENTORY_ROCKETS] < 10) && (bs->inventory[INVENTORY_HMG]
      <= 0 || bs->inventory[INVENTORY_308] < 10)) //&& FIXME: 0xA5EA, ok ?
...
bot_moveresult_t BotAttackMove(bot_state_t *bs, int tfl)
{
  ...
  vec3_t forward, backward, sideward, hordir, up =
  { 0, 0, 1 };
  aas_entityinfo_t entinfo;
  ...
  if (bs->cur_ps.weapon == WP_CROWBAR)
  {
    attack_dist = 0;
    attack_range = 0;
  }
...
#if 0 // #ifdef MISSIONPACK  //FIXME: 0xA5EA
/*
 ==================
...
void BotAimAtEnemy(bot_state_t *bs)
{
  ...
  else if (wi.number == WP_HMG)
  { //FIXME: 0xA5EA
    aim_accuracy = trap_Characteristic_BFloat(bs->character, CHARACTERISTIC_AIM_ACCURACY_RAILGUN, 0, 1);
  }

  else if (wi.number == WP_FLAMER) //add hypov8 flamer CHARACTERISTIC ACCURACY/SKILL
  {
    ...
    if (wi.speed)
    {
      ...
      if (!(dist > 100 && VectorLengthSquared(dir) < Square(32)))
      {
            ...
            bs->cur_ps.weaponstate == WEAPON_READY)
        {
          ...
          trap_AAS_PredictClientMovement(&move, bs->enemy, origin, PRESENCE_CROUCH, qfalse, dir, cmdmove, 0, dist * 10
              / wi.speed, 0.1f, 0, 0, qfalse);
  ...
  if (wi.number == WP_MACHINEGUN || wi.number == WP_SHOTGUN || wi.number == WP_HMG)
  {
    //FIXME: ok for hmg ?
    //distance towards the enemy
    dist = VectorLength(dir);
...
void BotCheckAttack(bot_state_t *bs)
{
  ...
  else if ( bs->weaponnum == WP_FLAMER )
  {
     if (VectorLengthSquared(dir) > Square(450))
      return;
  }
  else if ( bs->weaponnum == WP_GRENADE_LAUNCHER )
  {
     if (VectorLengthSquared(dir) > Square(700))
      return;
  }
...
void BotMapScripts(bot_state_t *bs)
{
  ...
  {
    vec3_t mins = { 700, 204, 672 };
    vec3_t maxs = { 964, 468, 680 };
...
int BotGetActivateGoal(bot_state_t *bs, int entitynum, bot_activategoal_t *activategoal)
{
  ...
  //if it is a door
  if (!qstrcmp(classname, "func_door")
 #if 1 //add hypov8 func_door_rotating
 || !qstrcmp(classname, "func_door_rotating")
 #endif
 )
  {
  ...
  if (!qstrcmp(classname, "func_button"))
    return 0;
  ...
  for (i = 0; i >= 0 && i < 10;)
  {
    for (ent = cur_entities[i]; ent; ent = trap_AAS_NextBSPEntity(ent))
    {
      ...
      if (!qstrcmp(targetname[i], target))
      {
    ...
    if (!qstrcmp(classname, "func_button"))
    {
      ...
      if (bs->activatestack && bs->activatestack->inuse && bs->activatestack->goal.entitynum == activategoal->goal.entitynum
    ...
    else if (!qstrcmp(classname, "trigger_multiple"))
    {
      //
    ...
    else if (!qstrcmp(classname, "func_timer"))
    {
  ...
  //else if ("func_plate") //hypov8 hypov8
  }
#ifdef OBSTACLEDEBUG
...
void BotPrintActivateGoalInfo(bot_state_t *bs, bot_activategoal_t *activategoal, int bspent)
{
  ...
  if (activategoal->shoot)
  {
    Com_sprintf(buf, sizeof(buf), "%s: I have to shoot at a %s from %1.1f %1.1f %1.1f in area %d\n", netname, classname,
                activategoal->goal.origin[0], activategoal->goal.origin[1], activategoal->goal.origin[2],
...
int BotAIPredictObstacles(bot_state_t *bs, bot_goal_t *goal)
{
  ...
  trap_AAS_PredictRoute(&route, bs->areanum, bs->origin, goal->areanum, bs->tfl, 100, 1000, RSE_USETRAVELTYPE
      | RSE_ENTERCONTENTS, AREACONTENTS_MOVER, TFL_BRIDGE, 0);
...
void BotCheckForGrenades(bot_state_t *bs, entityState_t *state)
{
  ...
  if (state->eType != ET_MISSILE && state->weapon != WP_GRENADE_LAUNCHER && state->weapon != WP_FLAMER)
    return;
  // try to avoid the grenade/rocket/flamer
...
void BotCheckEvents(bot_state_t *bs, entityState_t *state)
{
  ...
  case EV_GLOBAL_SOUND:
  {
    ...
    /*
     if (!qstrcmp(buf, "sound/teamplay/flagret_red.ogg")) {
     //Dragon Flag is returned
     bs->redflagstatus = 0;
     bs->flagstatuschanged = qtrue;
     }
     else if (!qstrcmp(buf, "sound/teamplay/flagret_blu.ogg")) {
     //Nikki Flag is returned
     bs->blueflagstatus = 0;
     bs->flagstatuschanged = qtrue;
     }
     else*/
    if (!qstrcmp(buf, "sound/items/kamikazerespawn.ogg" ))
    {
      //the kamikaze respawned so dont avoid it
      BotDontAvoid(bs, "Kamikaze");
    }
    else if (!qstrcmp(buf, "sound/items/poweruprespawn.ogg"))
    {
  ...
  case EV_GLOBAL_TEAM_SOUND:
  {
    if (gametype == GT_CTF)
    {
      ...
      {
      case GTS_DRAGON_CAPTURE:
        bs->blueflagstatus = 0;
        bs->redflagstatus = 0;
        bs->flagstatuschanged = qtrue;
        break; //see BotMatch_CTF
      case GTS_NIKKI_CAPTURE:
        bs->blueflagstatus = 0;
        bs->redflagstatus = 0;
        bs->flagstatuschanged = qtrue;
        break; //see BotMatch_CTF
      case GTS_DRAGON_RETURN:
        //Nikki Flag is returned
        bs->blueflagstatus = 0;
        bs->flagstatuschanged = qtrue;
        break;
      case GTS_NIKKI_RETURN:
        //Dragon Flag is returned
        bs->redflagstatus = 0;
        bs->flagstatuschanged = qtrue;
        break;
      case GTS_DRAGON_TAKEN:
        //Nikki Flag is taken
        bs->blueflagstatus = 1;
        bs->flagstatuschanged = qtrue;
        break; //see BotMatch_CTF
      case GTS_NIKKI_TAKEN:
        //Dragon Flag is taken
        bs->redflagstatus = 1;
        bs->flagstatuschanged = qtrue;
        break; //see BotMatch_CTF
      }
    ...
    else if (gametype == GT_1FCTF)
    {
      ...
      break;
        case GTS_DRAGON_TAKEN:
      bs->neutralflagstatus = BotTeam(bs) == TEAM_DRAGONS ? 2 : 1; //FIXME: check Team_TakeFlagSound in g_team.c
      bs->flagstatuschanged = qtrue;
      break;
        case GTS_NIKKI_TAKEN:
      bs->neutralflagstatus = BotTeam(bs) == TEAM_NIKKIS ? 2 : 1; //FIXME: check Team_TakeFlagSound in g_team.c
      bs->flagstatuschanged = qtrue;
      break;
      }
    }
...
#ifdef AI_BM
  else if (gametype == GT_BAGMAN) //hypov8 todo: bm goal?
  {
    /*
    if (trap_BotGetLevelItemGoal(-1, "Cashroll", &bm_cash) < 0)
      BotAI_Print(PRT_WARNING, "no alt routes without cash\n");
    if (bm_cash.areanum)
    {
      //
      red_numaltroutegoals = trap_AAS_AlternativeRouteGoals(
        bm_cash.origin, bm_cash.areanum,
        bm_dragonsafe.origin, bm_dragonsafe.areanum, TFL_DEFAULT,
        red_altroutegoals, MAX_ALTROUTEGOALS,
        ALTROUTEGOAL_CLUSTERPORTALS|
        ALTROUTEGOAL_VIEWPORTALS);
      blue_numaltroutegoals = trap_AAS_AlternativeRouteGoals(
        bm_cash.origin, bm_cash.areanum,
        bm_nikkisafe.origin, bm_nikkisafe.areanum, TFL_DEFAULT,
        blue_altroutegoals, MAX_ALTROUTEGOALS,
        ALTROUTEGOAL_CLUSTERPORTALS|
        ALTROUTEGOAL_VIEWPORTALS);
    }
    if (bm_cash_bag.areanum)
    {
      //
      red_numaltroutegoals = trap_AAS_AlternativeRouteGoals(
        bm_cash_bag.origin, bm_cash_bag.areanum,
        bm_dragonsafe.origin, bm_dragonsafe.areanum, TFL_DEFAULT,
        red_altroutegoals, MAX_ALTROUTEGOALS,
        ALTROUTEGOAL_CLUSTERPORTALS|
        ALTROUTEGOAL_VIEWPORTALS);
...
void BotSetupDeathmatchAI(void)
{
  ...
  if (gametype == GT_CTF)
  {
    if (trap_BotGetLevelItemGoal(-1, ITEMNAME_DRAGON_FLAG, &ctf_redflag) < 0)
      BotAI_Print(PRT_WARNING, "CTF without Dragons Flag\n");
    if (trap_BotGetLevelItemGoal(-1, ITEMNAME_NIKKIS_FLAG, &ctf_blueflag) < 0)
      BotAI_Print(PRT_WARNING, "CTF without Nikkis Flag\n");
  }
  else if (gametype == GT_1FCTF)
  {
    if (trap_BotGetLevelItemGoal(-1, ITEMNAME_NEUTRAL_FLAG, &ctf_neutralflag) < 0)
  ...
  { //hypov8 merge:
    if (trap_BotGetLevelItemGoal(-1, ITEMNAME_DRAGON_SAFE, &bm_dragonsafe) < 0)
      BotAI_Print(PRT_WARNING, "Bagman without Dragons Safe\n");
  if (trap_BotGetLevelItemGoal(-1, ITEMNAME_NIKKIS_SAFE, &bm_nikkisafe) < 0)
      BotAI_Print(PRT_WARNING, "Bagman without Nikkis Safe\n");
  //if (trap_BotGetLevelItemGoal(-1, ITEMNAME_CASHROLL, &bm_cash) < 0)
  //	BotAI_Print(PRT_WARNING, "Bagman without cash\n");

//	if (trap_BotGetLevelItemGoal(-1, ITEMNAME_CASHBAG, &bm_cash_bag) < 0)
  //	BotAI_Print(PRT_WARNING, "Bagman without cash\n");

  //if (trap_BotGetLevelItemGoal(-1, ITEMNAME_CASHSPAWN, &bm_cash_spawn) < 0)
    //BotAI_Print(PRT_WARNING, "Bagman without cash\n");


  //if (trap_BotGetLevelItemGoal(-1, ITEMNAME_CASHBAG_STOLEN, &bm_cash_stolen) < 0)
    //BotAI_Print(PRT_WARNING, "Bagman without cash\n");


    //FIXME(0xA5EA): bagman botlib
  }
#endif
#ifdef GT_USE_TA_TYPES
  else if (gametype == GT_OBELISK)
