...
void G_RankFireWeapon(int self, int weapon)
{
  if (level.warmupTime != 0)
    return; // no reports during warmup period

  if (weapon == WP_CROWBAR)
    return;    // the gauntlet only "fires" when it actually hits something
  ...
  case WP_ROCKET_LAUNCHER:
    ...
    break;
    //FIXME(0xA5EA): this file is unused but clean up this crap anyways
  case WP_FLAMER:
    trap_RankReportInt(self, -1, QGR_KEY_SHOT_FIRED_FLAMEGUN, 1, 1);
    break;
  case WP_HMG:
...
void G_RankDamage(int self, int attacker, int damage, int means_of_death)
{
  ...
  if ((attacker != ENTITYNUM_WORLD) && (attacker != self) &&
      (means_of_death == MOD_CROWBAR)  &&
      (g_entities[attacker].client))
  {
    trap_RankReportInt(attacker, -1, QGR_KEY_SHOT_FIRED_CROWBAR, 1, 1);
  }
  ...
  {
  case MOD_CROWBAR:
    key_hit    = QGR_KEY_HIT_TAKEN_CROWBAR;
    key_damage = QGR_KEY_DAMAGE_TAKEN_CROWBAR;
    break;
  case MOD_MACHINEGUN:
  ...
  case MOD_HMG:
    ...
    break;
  case MOD_FLAMEGUN:
    key_hit    = QGR_KEY_HIT_TAKEN_FLAMEGUN;
    key_damage = QGR_KEY_DAMAGE_TAKEN_FLAMEGUN;
    break;
  case MOD_GRAPPLE:
  ...
  if ((attacker != ENTITYNUM_WORLD) && (attacker != self))
  {
    ...
    {
    case MOD_CROWBAR:
      key_hit    = QGR_KEY_HIT_GIVEN_CROWBAR;
      key_damage = QGR_KEY_DAMAGE_GIVEN_CROWBAR;
      break;
    case MOD_MACHINEGUN:
    ...
    case MOD_ROCKET_SPLASH:
      ...
      break;
      //FIXME(0xA5EA): cleanup this crap some time
#if 0
    case MOD_PLASMA:
    case MOD_PLASMA_SPLASH:
    ...
    case MOD_HMG:
      ...
      break;
    case MOD_FLAMEGUN:
      key_hit    = QGR_KEY_HIT_GIVEN_FLAMEGUN;
      key_damage = QGR_KEY_DAMAGE_GIVEN_FLAMEGUN;
      break;
    case MOD_PISTOL:
      //FIXME(0xA5EA): this is still BGF
...
void G_RankPlayerDie(int self, int attacker, int means_of_death)
{
  ...
  else if (attacker == self)
  {
    ...
    {
    case MOD_CROWBAR:
      trap_RankReportInt(p1, p2, QGR_KEY_SUICIDE_CROWBAR, 1, 1);
      break;
    case MOD_MACHINEGUN:
    ...
    case MOD_HMG:
      ...
      break;
    case MOD_FLAMEGUN:
      trap_RankReportInt(p1, p2, QGR_KEY_SUICIDE_FLAMEGUN, 1, 1);
      break;
    case MOD_BFG:
    case MOD_BFG_SPLASH:
  ...
  else
  {
    ...
    {
    case MOD_CROWBAR:
      trap_RankReportInt(p1, p2, QGR_KEY_FRAG_CROWBAR, 1, 1);
      break;
    case MOD_MACHINEGUN:
    ...
    case MOD_HMG:
      ...
      break;
    case MOD_FLAMEGUN:
      trap_RankReportInt(p1, p2, QGR_KEY_FRAG_FLAMEGUN, 1, 1);
      break;
    case MOD_BFG:
    case MOD_BFG_SPLASH:
...
void G_RankWeaponTime(int self, int weapon)
{
  ...
  {
  case WP_CROWBAR:
    trap_RankReportInt(self, -1, QGR_KEY_TIME_CROWBAR, time, 1);
    break;
  case WP_MACHINEGUN:
  ...
  case WP_ROCKET_LAUNCHER:
    ...
    break;
  case WP_FLAMEGUN:
    trap_RankReportInt(self, -1, QGR_KEY_TIME_FLAMEGUN, time, 1);
    break;
  case WP_HMG:
...
void G_RankPickupWeapon(int self, int weapon)
{
  ...
  {
  case WP_CROWBAR:
    trap_RankReportInt(self, -1, QGR_KEY_PICKUP_CROWBAR, 1, 1);
    break;
  case WP_MACHINEGUN:
  ...
  case WP_ROCKET_LAUNCHER:
    ...
    break;
  case WP_FLAMEGUN:
    trap_RankReportInt(self, -1, QGR_KEY_PICKUP_FLAMEGUN, 1, 1);
    break;
  case WP_HMG:
