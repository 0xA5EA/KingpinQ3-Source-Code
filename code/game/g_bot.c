...
int G_ParseInfos(char *buf, int max, char *infos[])
{
  ...
  while (1)
  {
    ...
    infos[count] = (char*)G_Alloc(qstrlen(info) + qstrlen("\\num\\") + qstrlen(va("%d", MAX_ARENAS)) + 1);
    if (infos[count])
    {
      qstrcpy(infos[count], info);
      count++;
    }
  }
  return count;
...
static void G_LoadArenas(void)
{
  ...
  {
    G_LoadArenasFromFile("scripts/_arenas.txt"); //hypov8 add: ( _ )
  }
  ...
  for (i = 0; i < numdirs; i++, dirptr += dirlen + 1)
  {
    ...
    qstrcpy(filename, "scripts/");
    qstrcat(filename, dirptr);
...
int G_RemoveRandomBot(int team)
{
  ...
  for (i = 0; i < g_maxclients.integer; i++)
  {
    ...
    qstrcpy(netname, cl->pers.netname);
    Q_CleanStr(netname);
...
void G_CheckMinimumPlayers(void)
{
  ...
  if (g_gametype.integer >= GT_TEAM)
  {
    ...
    humanplayers = G_CountHumanPlayers(TEAM_DRAGONS);
    botplayers = G_CountBotPlayers(TEAM_DRAGONS);
    //
    if (humanplayers + botplayers < minplayers)
    {
      G_AddRandomBot(TEAM_DRAGONS);
    }
    else if (humanplayers + botplayers > minplayers && botplayers)
    {
      G_RemoveRandomBot(TEAM_DRAGONS);
    }
    //
    humanplayers = G_CountHumanPlayers(TEAM_NIKKIS);
    botplayers = G_CountBotPlayers(TEAM_NIKKIS);
    //
    if (humanplayers + botplayers < minplayers)
    {
      G_AddRandomBot(TEAM_NIKKIS);
    }
    else if (humanplayers + botplayers > minplayers && botplayers)
    {
      G_RemoveRandomBot(TEAM_NIKKIS);
    }
  }
...
static void G_AddBot(const char *name, float skill, const char *team, int delay, char *altname)
{
  ...
  Info_SetValueForKey(userinfo, "sex", s); //hypov8 todo: "gender" typo?
  ...
  s = Info_ValueForKey(botinfo, "aifile");
	if (!*s ) {
	  G_Printf( S_COLOR_RED "Error: bot has no aifile specified\n" );
		trap_BotFreeClient( clientNum );
    return;
  }
...
void Svcmd_BotList_f(void)
{
  ...
  {
    qstrcpy(name, Info_ValueForKey(g_botInfos[i], "name"));
    if (!*name)
    {
      qstrcpy(name, "UnnamedPlayer");
    }
    qstrcpy(funname, Info_ValueForKey(g_botInfos[i], "funname"));
    if (!*funname)
    {
      qstrcpy(funname, "");
    }
    qstrcpy(model, Info_ValueForKey(g_botInfos[i], "model"));
    if (!*model)
    {
      qstrcpy(model, "thug/default");
    }
    qstrcpy(aifile, Info_ValueForKey(g_botInfos[i], "aifile"));
    if (!*aifile)
    {
      qstrcpy(aifile, "bots/thug_c.c");
    }
...
static void G_LoadBots(void)
{
  ...
  {
    G_LoadBotsFromFile("botfiles/bots/_bots.txt"); //hypov8 was "scripts/bots.txt"
  }

  // get all bots from .bot files
  numdirs = trap_FS_GetFileList("botfiles/bots", ".bot", dirlist, 1024); //hypov8 was "scripts
  dirptr = dirlist;
  for (i = 0; i < numdirs; i++, dirptr += dirlen + 1)
  {
    dirlen = qstrlen(dirptr);
    qstrcpy(filename, "botfiles/bots/"); //hypov8 was "scripts/
    qstrcat(filename, dirptr);
