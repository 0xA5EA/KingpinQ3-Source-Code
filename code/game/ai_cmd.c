...
void BotMatch_AttackEnemyBase(bot_state_t *bs, bot_match_t *match)
{
  ...
  if (gametype == GT_CTF)
  {
    BotMatch_GetFlag(bs, match);
      ...
      || gametype == GT_OBELISK || gametype == GT_HARVESTER
#endif
  )
  {
#ifdef GT_USE_TA_TYPES
...
void BotMatch_RushBase(bot_state_t *bs, bot_match_t *match)
{
  ...
  {
    //FIXME (0xA5EA): das sollte eigentlich auch fuer 1ctf da sein
#if 0
    if (!redobelisk.areanum || !blueobelisk.areanum)
      return;
#endif
...
void BotMatch_StartTeamLeaderShip(bot_state_t *bs, bot_match_t *match)
{
  ...
  if (match->subtype & ST_I)
  {
    ...
    bs->teamleader[sizeof(bs->teamleader) - 1] = '\0'; //0xA5EA: wegen compiler warnung minus 1 dazu
  }
...
void BotMatch_WhereAreYou(bot_state_t *bs, bot_match_t *match)
{
  ...
  char *nearbyitems[] =
  {
    ...
    "Flight",
    "Mega Health",
    ITEMNAME_DRAGON_FLAG,
    ITEMNAME_NIKKIS_FLAG,
    ITEMNAME_NEUTRAL_FLAG,
    ITEMNAME_DRAGON_SAFE,
    ITEMNAME_NIKKIS_SAFE,
  ...
  if (bestitem != -1)
  {
    if (gametype == GT_CTF || gametype == GT_1FCTF)
    {
      ...
      {
		  BotAI_BotInitialChat(bs, "teamlocation", nearbyitems[bestitem], TEAM_NAME_DRAGONS, NULL); //"red"
      }
      else if (bluett < (redtt + bluett) * 0.4)
      {
		  BotAI_BotInitialChat(bs, "teamlocation", nearbyitems[bestitem], TEAM_NAME_NIKKIS, NULL); //"blue"
      }
      else
      {
    ...
    //FIXME(0xA5EA): BAGMAN
	//add hypov8 copy from ctf
#ifdef AI_BM
	if (gametype == GT_BAGMAN)
	{
		redtt = trap_AAS_AreaTravelTimeToGoalArea(bs->areanum, bs->origin, bm_dragonsafe.areanum, TFL_DEFAULT);
		bluett = trap_AAS_AreaTravelTimeToGoalArea(bs->areanum, bs->origin, bm_nikkisafe.areanum, TFL_DEFAULT);
		if (redtt < (redtt + bluett) * 0.4)
		{
			BotAI_BotInitialChat(bs, "teamlocation", nearbyitems[bestitem], TEAM_NAME_DRAGONS, NULL); //"red"
		}
...
void BotMatch_CTF(bot_state_t *bs, bot_match_t *match)
{
  ...
  if (gametype == GT_CTF)
  {
    ...
    {
		if (!Q_stricmp(flag, TEAM_NAME_DRAGONS)) //"red"
      {
        bs->redflagstatus = 1;
        if (BotTeam(bs) == TEAM_NIKKIS)
        {
          trap_BotMatchVariable(match, NETNAME, netname, sizeof(netname));
    ...
    {
		if (!Q_stricmp(flag, TEAM_NAME_DRAGONS))
        bs->redflagstatus = 0; //"red"
      else
        bs->blueflagstatus = 0;
      bs->flagstatuschanged = 1;
    }
