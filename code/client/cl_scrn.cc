...
void SCR_DrawDemoRecording(void)
{
  ...
  SCR_DrawStringExt(320 - qstrlen(string) * 4, 20, 8, string, (float*)g_color_table[7], qtrue, qfalse);
}
...
void SCR_DrawVoipMeter(void)
{
  ...
  SCR_DrawStringExt(320 - qstrlen(string) * 4, 10, 8, string, (float*)g_color_table[7], qtrue, qfalse);
}
#endif
...
//----------------------------------------
int SCR_Text_Height(const char *text, float scale, int limit, fontInfo_t *font)
{
  int len, count;
  float max;
...
void SCR_Text_Paint(float x, float y, float scale, vec4_t color, const char *text, float adjust, int limit, int style, fontInfo_t *font)
{
  int len, count;
  vec4_t newColor ;
  glyphInfo_t *glyph;
...
//      const unsigned char *s = text;
    ...
    while (s && *s && count < len)
    {
      ...
      if (Q_IsColorString(s))
      {
        Com_Memcpy(newColor, (float *)g_color_table[ColorIndex(*(s + 1))], sizeof(newColor));
        newColor[3] = color[3];
...
int SCR_Text_Width(const char *text, float scale, int limit, fontInfo_t *font)
{
  int count, len;
  float out;
  const glyphInfo_t *glyph;
