...
void CL_Voip_f(void)
{
  ...
  if (qstrcmp(cmd, "ignore") == 0)
  {
    CL_UpdateVoipIgnore(Cmd_Argv(2), qtrue);
	} else if (qstrcmp(cmd, "unignore") == 0) {
    CL_UpdateVoipIgnore(Cmd_Argv(2), qfalse);
	} else if (qstrcmp(cmd, "gain") == 0) {
		if (Cmd_Argc() > 3) {
...
void CL_ChangeReliableCommand( void )
{
  ...
  l     = qstrlen(clc.reliableCommands[index]);
...
void CL_Record_f( void ) {
  ...
  char *s;
	...
	if (clc.demoplaying) // add hypov8
	{
		Com_Printf("Already playing a demo.\n");
		return;
	}
  if (clc.demorecording)
  {
...
void CL_DemoCompleted(void)
{
  ...
  if (cl_timedemo && cl_timedemo->integer)
  {
    ...
    if (time > 0)
    {
      ...
      if (cl_timedemoLog && qstrlen(cl_timedemoLog->string) > 0)
      {
        int i;
        int numFrames;
        fileHandle_t f;
...
static void CL_UpdateGUID(const char *prefix, int prefix_len)
{
  ...
  if (len != QKEY_SIZE)
    ...
    Cvar_Set("cl_guid", Com_MD5File((const char*)QKEY_FILE, (int)QKEY_SIZE, //add hypov8 (const char*) + (int)
                                    prefix, prefix_len));
}
...
{
  if (!qstrlen(clc.servername) || !qstrcmp(clc.servername, "localhost"))
  {
...
void CL_Connect_f(void)
{
  ...
  {
    if (!qstrcmp(Cmd_Argv(1), "-4"))
      family = NA_IP;
    else if (!qstrcmp(Cmd_Argv(1), "-6"))
      family = NA_IP6;
    else
  ...
  if (com_sv_running->integer && !qstrcmp(server, "localhost"))
  {
  ...
  if (cl_guidServerUniq->integer)
    CL_UpdateGUID(serverString, qstrlen(serverString));
  else
    CL_UpdateGUID(NULL, 0);
...
void CL_Rcon_f(void)
{
  ...
  NET_SendPacket(NS_CLIENT, qstrlen(message) + 1, message, to);
}
    ...
    *s++      = 0;
    ...
    if ((s = strchr(s, '@')) != NULL)
      *s++ = 0;
      ...
      s = localName + qstrlen(localName);                                                                                                                                                                                             // point at the nul byte
#ifdef USE_CURL
    ...
    // move over the rest
    Com_Memmove(clc.downloadList, s, qstrlen(s) + 1);
...
void CL_CheckForResend(void)
{
  ...
  case CA_CHALLENGING:
    ...
    qstrcpy(data, "connect ");
    // TTimo adding " " around the userinfo string to avoid truncated userinfo on the server
    //   (Com_TokenizeString tokenizes around spaces)
    data[8] = '"';

    for (i = 0; i < qstrlen(info); i++)
    {
...
void CL_MotdPacket(netadr_t from)
{
  ...
  if (qstrcmp(challenge, cls.updateChallenge))
    return;
...
void CL_ServersResponsePacket( const netadr_t* from, msg_t *msg, qboolean extended ) {
  ...
  //Com_DPrintf("1: count %d; numservers = %d\n", count, numservers);
...
void CL_InitRenderer(void)
{
  ...
  cls.whiteShader = re.RegisterShader("white", RSF_DEFAULT);
// cls.consoleShader = re.RegisterShader( "console" );

  //fonts
  //LucidaTypewriterRegular.ttf
  //re.RegisterFont("fonts/Lucidatwregular.ttf", 24, &cls.consoleFont);
  re.RegisterFont("fonts/LibMonoReg.ttf", 14, &cls.consoleFont); //FixedDisplay.ttf Fixedsys500c.ttf VeraMono.ttf
  re.RegisterFont("fonts/royalp.ttf", 24, &cls.consoleBoldFont);
...
void CL_InitRef(void)
{
  ...
  refexport_t *ret;
	...
	ri.RealTime				= Com_RealTime; //daemon
	ri.Z_Malloc				= CL_RefMalloc;
	ri.Free					= Z_Free;
	ri.Tag_Free				= CL_RefTagFree; //daemon
  ...
  ri.Hunk_FreeTempMemory     = Hunk_FreeTempMemory;
	//ri.CM_ClusterPVS           = CM_ClusterPVS; //hypove merge: disabled?
  ...
  //ri.Cvar_CheckRange         = Cvar_CheckRange;
	...
	ri.FS_Seek = FS_Seek; //daemon
	ri.FS_FTell = FS_FTell; //daemon
...
===============
*/
//FIXME: 0xA5EA, weg bei standanole ??
...
void CL_Init(void)
{
  ...
  Com_Memset(&cls.cpuConfig, 0, sizeof(cpuConfig_t));
  Sys_GetCpuConfig(&cls.cpuConfig);
  ...
  Cvar_Get("cg_autoswitch", "2", CVAR_ARCHIVE); // hypov8 add 2. only switch if holding crow/pistol
...
static void CL_SetServerInfo(serverInfo_t *server, const char *info, int ping)
{
  if (server)
  {
    if (info)
    {
      ...
      server->weaponMode =  atoi(Info_ValueForKey(info, "weaponmode"));
...
void CL_ServerInfoPacket(netadr_t from, msg_t *msg)
{
  ...
  char *infoString;
  int prot;
    cvar_t *debug; //hypov8 merge:
	char	*gamename;

  infoString = MSG_ReadString(msg);
	// if this isn't the correct gamename, ignore it
  gamename = Info_ValueForKey(infoString, "gamename");  //hypov8 "com_gamename" //hypov8 "gamename"
  if (!*gamename || qstrcmp(gamename, com_gamename->string) !=0)
	{
  ...
  debug = Cvar_Get("debug_protocol", va("%i",PROTOCOL_VERSION), 0);
  if (qstrlen(debug->string))
  {
	  prot = debug->integer;
  }
  else
...
// cls.localServers[i].punkbuster = 0;
  cls.localServers[i].weaponMode = 0;
...
void CL_ServerStatusResponse(netadr_t from, msg_t *msg)
{
  ...
  len = qstrlen(serverStatus->string);
...
void CL_LocalServers_f(void)
{
  ...
  for (i = 0; i < 2; i++)
  {
    ...
    for (j = 0; j < NUM_SERVER_PORTS; j++)
    {
      ...
      NET_SendPacket(NS_CLIENT, qstrlen(message), message, to);
  ...
  for (i = 0; i < MAX_PINGREQUESTS; i++, pingptr++)
  {
    ...
    if (pingptr->adr.port)
    {
      if (!pingptr->time)
      {
        ...
        continue; // results have not been queried
    }
    ...
    default:
      return qfalse;
        ...
        else if (server[i].ping == 0)
        {
          ...
          if (source == AS_GLOBAL) //hypov8 todo: >global <fav?
          {
...
qboolean CL_CDKeyValidate(const char *key, const char *checksum)
{
  ...
  if (checksum && qstrlen(checksum) != CDCHKSUM_LEN)
    return qfalse;
