...
qboolean CL_GetServerCommand(int serverCommandNumber)
{
  ...
  //FIXME(0xA5EA): removed this temporarily
...
rescan:
  ...
  {
    s = Cmd_Argv(2);
    if (qstrlen(bigConfigString) + qstrlen(s) >= BIG_INFO_STRING)
    {
      Com_Error(ERR_DROP, "bcs exceeded BIG_INFO_STRING");
    }
    qstrcat(bigConfigString, s);
    return qfalse;
  }
  ...
  {
    s = Cmd_Argv(2);
    if (qstrlen(bigConfigString) + qstrlen(s) + 1 >= BIG_INFO_STRING)
    {
      Com_Error(ERR_DROP, "bcs exceeded BIG_INFO_STRING");
    }
    qstrcat(bigConfigString, s);
    qstrcat(bigConfigString, "\"");
    s = bigConfigString;
    goto rescan;
  }
...
// ZOID
  ...
  case CG_CM_BOXTRACE:
    CM_BoxTrace((trace_t*)VMA(1), (const vec_t*)VMA(2),(const vec_t*) VMA(3), (vec_t*)VMA(4), (vec_t*)VMA(5),
                args[6], args[7], TT_AABB);
    return 0;

  case CG_CM_CAPSULETRACE:
    CM_BoxTrace((trace_t*)VMA(1), (const vec_t*)VMA(2), (const vec_t*)VMA(3), (vec_t*)VMA(4), (vec_t*)VMA(5),
                 args[6], args[7], TT_CAPSULE);
    return 0;

  case CG_CM_TRANSFORMEDBOXTRACE:
    CM_TransformedBoxTrace((trace_t *)VMA(1), (const vec_t*)VMA(2), (const vec_t*)VMA(3),
                           ( vec_t*)VMA(4), ( vec_t*)VMA(5), args[6], args[7], (const vec_t*)VMA(8),
                           (const vec_t*)VMA(9), TT_AABB);
    return 0;

  case CG_CM_TRANSFORMEDCAPSULETRACE:
    CM_TransformedBoxTrace((trace_t *)VMA(1), (const vec_t*)VMA(2), (const vec_t*)VMA(3), ( vec_t*)VMA(4), ( vec_t*)VMA(5),
                           args[6], args[7], (const vec_t*)VMA(8), (const vec_t*)VMA(9), TT_CAPSULE);
    return 0;

  case CG_CM_BISPHERETRACE:
    CM_BiSphereTrace((trace_t*)VMA(1), (const vec_t*)VMA(2), (const vec_t*)VMA(3), VMF(4), VMF(5), args[6], args[7]);
    return 0;

  case CG_CM_TRANSFORMEDBISPHERETRACE:
    CM_TransformedBiSphereTrace((trace_t*)VMA(1), (const vec_t*)VMA(2), (const vec_t*)VMA(3), VMF(4), VMF(5), args[6], args[7], (const vec_t*)VMA(8));
    return 0;
  ...
  case CG_R_REGISTERANIMATION:
    return re.RegisterAnimation((const char*)VMA(1));

  case CG_R_REGISTERSKIN:
    return re.RegisterSkin((const char*)VMA(1));

  case CG_R_REGISTERSHADER:
    return re.RegisterShader((const char*)VMA(1), RSF_DEFAULT);

  case CG_R_REGISTERSHADERNOMIP:
    return re.RegisterShader((const char*)VMA(1), RSF_NOMIP);

  case CG_R_REGISTERSHADERLIGHTATTENUATION:
    return re.RegisterShader((const char*)VMA(1), RSF_LIGHT_ATTENUATION);
  ...
  case CG_R_ADDREFLIGHTSTOSCENE:
    re.AddRefLightToScene((const refLight_t*)VMA(1));
    return 0;
  ...
  case CG_R_ADDLIGHTTOSCENE:
    re.AddLightToScene((const vec_t*)VMA(1), VMF(2), VMF(3), VMF(4), VMF(5),  VMF(6), args[7], args[8]);
    return 0;
  ...
  case CG_R_BLENDSKELETON:
    return re.BlendSkeleton((refSkeleton_t*)VMA(1), (const refSkeleton_t*)VMA(2), VMF(3));

  case CG_R_BONEINDEX:
    return re.BoneIndex(args[1], (const char*)VMA(2));
...
void CL_InitCGame(void)
{
  ...
  // load the dll or bytecode
	interpret = static_cast<vmInterpret_t>(static_cast<int>(Cvar_VariableValue("vm_cgame")));
  ...
  VM_Call(cgvm, CG_INIT, clc.serverMessageSequence, clc.lastExecutedServerCommand, clc.clientNum, Com_Milliseconds());
