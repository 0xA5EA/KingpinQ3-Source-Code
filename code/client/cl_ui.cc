...
{
//	Com_Printf("LAN_GetServerAddressString call \n" );
  switch (source)
  {
  case AS_LOCAL:
...
static void LAN_GetServerInfo(int source, int n, char *buf, int buflen)
{
  ...
  if (server && buf)
  {
    ...
    //	Info_SetValueForKey( info, "punkbuster", va("%i", server->punkbuster));  //FIXME: 0xA5EA
    Info_SetValueForKey(info, "weaponmode", va("%i", server->weaponMode));
...
static void CL_GetCpuConfig(cpuConfig_t * cpuConfig)
{
  *cpuConfig = cls.cpuConfig;
}
/*
====================
CL_GetClipboardData
...
intptr_t CL_UISystemCalls(intptr_t *args)
{
  ...
  case UI_CMD_EXECUTETEXT:
		if(args[1] == EXEC_NOW
		&& (!strncmp((char const*)VMA(2), "snd_restart", 11)
		|| !strncmp((char const*)VMA(2), "vid_restart", 11)
		|| !strncmp((char const*)VMA(2), "quit", 5)))
    {
  ...
  case UI_R_BLENDSKELETON:
    return re.BlendSkeleton((refSkeleton_t*)VMA(1), (refSkeleton_t const*)VMA(2), VMF(3));
  ...
  case UI_R_LERPTAG:
	...
	return re.LerpTag((orientation_t*)VMA(1),(refEntity_t*) args[2], (const char*)VMA(3), args[4]);
#else
...
#endif
    //re.LerpTag((orientation_t*)VMA(1), (refEntity_t*)VMA(2), (char*)VMA(3), args[4]);
    return 0;
...
#endif
  case UI_GETCPUCONFIG:
    CL_GetCpuConfig((cpuConfig_t*)VMA(1));
    return 0;
...
void CL_InitUI(void)
{
  ...
  {
//		Com_Printf(S_COLOR_YELLOW "WARNING: loading old KingpinQ3 User Interface version %d\n", v );
    // init for this gamestate
    VM_Call(uivm, UI_INIT, (clc.state >= CA_AUTHORIZING && clc.state < CA_ACTIVE),  Com_Milliseconds());
  }
