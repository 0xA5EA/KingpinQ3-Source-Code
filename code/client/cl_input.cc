...
void CL_MouseMove(usercmd_t *cmd)
{
  ...
  //if ((cl.viewangles[PITCH] > 1) && (cl.viewangles[PITCH] < 2))
 // Com_Printf("cl.viewangles[PITCH] between 1 and 2.... viewangles %f\n ", cl.viewangles[0]);;

  //if ((cl.viewangles[YAW] > 1) && (cl.viewangles[YAW] < 2))
//	  Com_Printf("cl.viewangles[YAW] central... viewangles %f\n", cl.viewangles[1]);

  //hypov8 error in cl.viewangles, getting     ***    -1.#IND0000     ***
  if (!(cl.viewangles[PITCH] >= -999999))
   Com_Printf("ERROR cl.viewangles[PITCH].... viewangles %f\n ", cl.viewangles[0]);;

  if (!(cl.viewangles[YAW] >= -999999))
  	  Com_Printf("ERROR cl.viewangles[YAW]... viewangles %f\n", cl.viewangles[1]);
...
//hypov8 merge: check this
  //hypov8 dont allow frames less than 8ms
  if (frame_msec < 8)//add hypov8 trying to fix mouse issue. could be to much frame rate info. 4=250 8=125 fps limit
	  frame_msec = 8; //hypov8 cl.viewangles bug?? 

  //hypov8 'com_maxfps 125' added to match frame rates above. or syc is wrong
  if (com_maxfps->value > 125)// hypov8 can this go elsewhere
	  Cvar_Set("com_maxfps", "125"); // hypov8 limit max fps
 //end
...
qboolean CL_ReadyToSendPacket(void)
{
  ...
  if (cl_maxpackets->integer < 15)
    ...
    Cvar_Set("cl_maxpackets", "125"); //hypov8 60?? do we need so many.. lan??
