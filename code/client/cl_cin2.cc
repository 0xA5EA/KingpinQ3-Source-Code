...
e_status CIN_RunCinematic(int handle)
{
  ...
  currentHandle = handle;
	...
	Com_DPrintf("handle = %d, cin.currentHandle %d\n", handle, cin.currentHandle);
...
int CIN_PlayCinematic(const char *arg, int x, int y, int w, int h, int systemBits)
{
  ...
  qstrcpy(cinTable[currentHandle].fileName, name);
	...
	if(!Q_stricmp(fileextPtr, ".ogm") || !Q_stricmp(fileextPtr, ".ogv") /*|| !Q_stricmp(fileextPtr, ".ogg")*/)
	{
		if(int err = Cin_OGM_Init(name))
		{
			Com_DPrintf(S_COLOR_RED"starting ogm-playback failed(%s) with error %i\n", arg, err);
			cinTable[currentHandle].fileName[0] = 0;
			Cin_OGM_Shutdown();
			return -1;
		}
