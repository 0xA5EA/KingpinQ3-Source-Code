...
void CL_ParseSnapshot(msg_t *msg)
{
  ...
  // read areamask
  len = static_cast<unsigned int>(MSG_ReadByte(msg));
...
void CL_SystemInfoChanged(void)
{
  ...
  while (s)
  {
    ...
    if ((cvar_flags = Cvar_Flags(key)) == (int)CVAR_NONEXISTENT)
...
void CL_ParseVoip(msg_t *msg)
{
  ...
  // This is a new "generation" ... a new recording started, reset the bits.
    ...
    for (i = 0; i < seqdiff; i++)
    {
      assert((written + clc.speexFrameSize) * 2 < (int)(sizeof(decoded)));
  ...
  for (i = 0; i < frames; i++)
  {
    ...
    if ((written + clc.speexFrameSize) * 2 > (int)sizeof(decoded))
    {
