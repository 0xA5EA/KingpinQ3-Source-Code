...
typedef struct
{
	...
	ogg_uint32_t             outputWidht;
	ogg_uint32_t             outputHeight;
...
static int init_xvid()
{
	...
	Com_Memset(&xvid_gbl_init, 0, sizeof(xvid_gbl_init_t));
	Com_Memset(&xvid_dec_create, 0, sizeof(xvid_dec_create_t));
...
static int dec_xvid(unsigned char *input, int input_size)
{
	...
	Com_Memset(&xvid_dec_frame, 0, sizeof(xvid_dec_frame_t));
	Com_Memset(&g_ogm.xvid_dec_stats, 0, sizeof(xvid_dec_stats_t));
...
//          int i,j;
			...
			if(g_ogm.outputBufferSize < (int)(g_ogm.th_info.width * g_ogm.th_info.height))
			{
...
int Cin_OGM_Init(const char *filename)
{
	...
	Com_Memset(&g_ogm, 0, sizeof(cin_ogm_t));

	FS_FOpenFileRead(filename, &g_ogm.ogmFile, qtrue);
	if(!g_ogm.ogmFile)
	{
		Com_DPrintf(S_COLOR_YELLOW"WARNING: Can't open ogm-file for reading (%s)\n", filename);
		return -1;
	}
	...
	while(!g_ogm.os_audio.serialno || !g_ogm.os_video.serialno)
	{
		if(ogg_sync_pageout(&g_ogm.oy, &og) == 1)
		{
      //Com_Printf("og.body %s\n", (char*)og.body);
      //Com_Printf("og.body + 1 %s\n", (char*)og.body +1);
			if(Q_strstr((char *)(og.body + 1), "vorbis"))
			{
...
#ifdef USE_CIN_THEORA
			//Com_Printf("%i \n", g_ogm.os_video.serialno);
			if(Q_strstr((char *)(og.body + 1), "theora"))
			{
				if(g_ogm.os_video.serialno)
				{
...
#endif
#ifdef USE_CIN_XVID
			if(Q_strstr((char *)(og.body + 1), "video"))
	...
	{
		Com_Printf(S_COLOR_RED"ERROR: Found \"video\"- and \"theora\"-stream ,ogm-file (%s)\n", filename);
		return -2;
	}
	...
	if(!g_ogm.os_audio.serialno)
	{
		Com_Printf(S_COLOR_RED"ERROR: Haven't found a audio(vorbis) stream in ogm-file (%s)\n", filename);
		return -2;
	}
#endif
	if(!g_ogm.os_video.serialno)
	{
		Com_Printf(S_COLOR_RED"ERROR: Haven't found a video stream in ogm-file (%s)\n", filename);
		return -3;
	}
	...
	while(i < 3)
	{
		...
		if(status < 0)
		{
			Com_Printf(S_COLOR_RED"ERROR: Corrupt ogg packet while loading vorbis-headers, ogm-file(%s)\n", filename);
			return -8;
		}
		if(status > 0)
		{
			...
			if(i == 0 && status < 0)
			{
				Com_Printf(S_COLOR_RED"ERROR: This Ogg bitstream does not contain Vorbis audio data, ogm-file(%s)\n", filename);
				return -9;
			}
			++i;
		}
		else if(loadPagesToStreams())
		{
			if(loadBlockToSync())
			{
				Com_Printf(S_COLOR_RED"ERROR: Couldn't find all vorbis headers before end of ogm-file (%s)\n", filename);
				return -10;
			}
		}
	}
	...
	if(g_ogm.videoStreamIsTheora)
	{
		...
		while(i < 3)
		{
			...
			if(status < 0)
			{
				Com_Printf(S_COLOR_RED"ERROR: Corrupt ogg packet while loading theora-headers, ogm-file(%s)\n", filename);
				return -8;
			}
			if(status > 0)
			{
				...
				if(i == 0 && status != 0)
				{
					Com_Printf(S_COLOR_RED"ERROR: This Ogg bitstream does not contain theora data, ogm-file(%s)\n", filename);
					return -9;
				}
				++i;
			}
			else if(loadPagesToStreams())
			{
				if(loadBlockToSync())
				{
					Com_Printf(S_COLOR_RED"ERROR: Couldn't find all theora headers before end of ogm-file (%s)\n", filename);
					return -10;
				}
			}
		}
...
int Cin_OGM_Run(int time)
{
	...
	//FIXME(0xA5EA): somtimes this loops because g_ogm.Vtime_unit is zero
	// it happens mainly in server-browser menu when selecting server
	if (g_ogm.Vtime_unit == 0)
	{
    Com_Printf(S_COLOR_RED"ERROR: catched engine freeze because g_ogm.Vtime_unit = %i\n", static_cast<int>(g_ogm.Vtime_unit));
    return 2;
	}
