...
void S_AL_StartLocalSound(sfxHandle_t sfx, int channel)
{
  ...
  if(src == -1)
  { //hypov8 add debug print when sound cant be allocated
    Com_DPrintf(S_COLOR_YELLOW "WARNING: Failed to allocate source "
      "for start local sfx %d on channel %d\n", sfx, channel);
    return;
  }
...
void S_AL_StartBackgroundTrack( const char *intro, const char *loop )
{
  ...
  // Copy the loop over
  strncpy( s_backgroundLoop, loop, sizeof( s_backgroundLoop ) -1 );
...
#ifdef _WIN32
#define ALDRIVER_DEFAULT "openal-32.dll"
#elif defined(_WIN64)
#define ALDRIVER_DEFAULT "openal-64.dll"
...
qboolean S_AL_Init( soundInterface_t *si )
{
  ...
  {
    Com_Printf(S_COLOR_YELLOW "Failed to load library: \"%s\".\n", s_alDriver->string );
    return qfalse;
  }
