		...
		float fCameraPosition[3], float fCameraFront[3], float fCameraTop[3])
{
	...
	Com_Memcpy(lm->fAvatarPosition, fAvatarPosition, sizeof(lm->fAvatarPosition));
	Com_Memcpy(lm->fAvatarFront, fAvatarFront, sizeof(lm->fAvatarFront));
	Com_Memcpy(lm->fAvatarTop, fAvatarTop, sizeof(lm->fAvatarTop));
	Com_Memcpy(lm->fCameraPosition, fCameraPosition, sizeof(lm->fCameraPosition));
	Com_Memcpy(lm->fCameraFront, fCameraFront, sizeof(lm->fCameraFront));
	Com_Memcpy(lm->fCameraTop, fCameraTop, sizeof(lm->fCameraTop));
	lm->uiVersion = 2;
...
void mumble_set_context(const unsigned char* context, size_t len)
{
	...
	lm->context_len = len;
	Com_Memcpy(lm->context, context, len);
}
