...
void S_Base_SoundList(void)
{
	...
	qstrcpy(type[0], "16bit");
	qstrcpy(type[1], "adpcm");
	qstrcpy(type[2], "daub4");
	qstrcpy(type[3], "mulaw");
	qstrcpy(mem[0], "paged out");
	qstrcpy(mem[1], "resident ");
	total = 0;
...
sfxHandle_t S_Base_RegisterSound(const char *name, qboolean compressed)
{
	...
	if(qstrlen(name) >= MAX_QPATH)
	{
		Com_Printf(S_COLOR_YELLOW"Sound name %s exceeds MAX_QPATH\n", name);
		return 0;
	}
...
void S_Base_BeginRegistration(void)
{
	...
	if(s_numSfx == 0)
	{
		...
		S_Base_RegisterSound("sound/feedback/hit", qfalse);		// changed to a sound in baseq3
	}
}
