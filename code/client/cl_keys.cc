...
===================
*/
void Field_VariableSizeDraw2(field_t *edit, int x, int y, int width, float fontScale, qboolean showCursor, qboolean noColorEscape)
{
  ...
  SCR_Text_Paint(x, y, fontScale, color, edit->buffer, 0, 0, style, &cls.consoleFont);
  dx = SCR_Text_Width(edit->buffer, fontScale, 512, &cls.consoleFont);
...
//hypov8 merge:
  // drawLen = edit->widthInChars - 1;	// - 1 so there is always a space for the cursor
   ...
   if (len)
	dx -= ((dx * len) - (dx*edit->cursor)) / len; //hypov8 Cursor BUG Fix. Get a percent to remove! So cursor location is correct
  ...
  if (showCursor)
  {
    ...
    {
      //cursorChar = 11;
      //SCR_Text_Paint(x + (edit->cursor - prestep) * 4.5, y, fscale, color, "*", 0, 0, style, &clc.consoleFont);
      SCR_Text_Paint(x + dx, y, fontScale, color, "*", 0, 512, style, &cls.consoleFont);

    }
    else
    {
      SCR_Text_Paint(x + dx, y, fontScale, color, "_", 0, 512, style, &cls.consoleFont);
      //cursorChar = 10;
    }
  }
}
...
//	int		i;
  ...
  SCR_Text_Paint(x, y, fscale, color, str, 0, 0, style, &clc.consoleFont);
  xofs = SCR_Text_Width(str, fscale,0, &clc.consoleFont);
  // draw the cursor
  if (showCursor)
  {
    // let it blink
    if ((int)(clc.realtime >> 8) & 1)
      return;
    ...
    if (key_overstrikeMode)
    {
      cursorChar = 11;
      //SCR_Text_Paint(x + (edit->cursor - prestep) * 4.5, y, fscale, color, "*", 0, 0, style, &clc.consoleFont);
      SCR_Text_Paint(x + xofs, y, fscale, color, "*", 0, 0, style, &clc.consoleFont);

    }
    else
    {
      SCR_Text_Paint(x + xofs, y, fscale, color, "_", 0, 0, style, &clc.consoleFont);
      cursorChar = 10;
    }
  }
}
...
void Field_KeyDownEvent(field_t *edit, int key)
{
  ...
  case K_DEL:
    if (edit->cursor < len)
    {
      Com_Memmove(edit->buffer + edit->cursor,
...
void Field_CharEvent(field_t *edit, int ch)
{
  ...
  if (ch == 'h' - 'a' + 1)      // ctrl-h is backspace
  {
    if (edit->cursor > 0)
    {
      Com_Memmove(edit->buffer + edit->cursor - 1,
...
void CL_KeyDownEvent( int key, unsigned time )
{
	...
	if (key == K_F12) //add hypov8 f12 will allways quit
		Com_Quit_f();
	...
	if ( key == K_ESCAPE ) {
		...
		if ( !( Key_GetCatcher( ) & KEYCATCH_UI ) ) {
			...
			//hypov8 add CA_PRIMED. esc would dissconnect a CA_PRIMED client, about to enter the server
			else if (clc.state != CA_DISCONNECTED && clc.state != CA_PRIMED /*&& !clc.demoplaying*/) //hypov8 demo always quits??
			{
				CL_Disconnect_f();
				S_StopAllSounds();
...
#define         CONSOLE_HISTORY_FILE "kpq3history"
...
void CL_LoadConsoleHistory(void)
{
      ...
      FS_Read(consoleSaveBuffer, consoleSaveBufferSize, f) == consoleSaveBufferSize)
  {
    ...
    Com_Memmove(&historyEditLines[0], &historyEditLines[i + 1], numLines * sizeof(field_t));
...
void CL_SaveConsoleHistory(void)
{
  ...
  do
  {
    ...
    {
      lineLength       = qstrlen(historyEditLines[i].buffer);
      saveBufferLength = qstrlen(consoleSaveBuffer);

      //ICK
      additionalLength = lineLength + qstrlen("999 999 999  ");
  ...
  consoleSaveBufferSize = qstrlen(consoleSaveBuffer);
