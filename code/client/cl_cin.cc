...
typedef struct
{
  ...
  byte  half, smootheddouble, inMemory; //qboolean
  long normalBuffer0;
  long roq_flags;
  long roqF0;
  long roqF1;
...
{
  Com_Printf("CIN_StopCinematic(%i)\n", handle);
...
e_status CIN_RunCinematic(int handle)
{
  ...
  if (cin.currentHandle != handle)
  {
    ...
    cinTable[currentHandle].status = FMV_EOF;
    RoQReset();
    //FIXME(0xA5EA): whats up with ogv ?
  }
  ...
  if (cinTable[currentHandle].status == FMV_IDLE)
  {
	...
	if(cinTable[currentHandle].fileType == FT_OGM)
	{
		if(int err = Cin_OGM_Run(cinTable[currentHandle].startTime == 0 ? 0 : CL_ScaledMilliseconds() - cinTable[currentHandle].startTime))
		{
		  if (err == 1)
		    cinTable[currentHandle].status = FMV_EOF;
		  else if (err == 2)
...
int CIN_PlayCinematic(const char *arg, int x, int y, int w, int h, int systemBits)
{
  ...
  char name[MAX_OSPATH];
	...
	if (Q_strstr(arg, "/") == NULL && Q_strstr(arg, "\\") == NULL) {
	...
	if (!(systemBits & CIN_system)) {
		for ( i = 0 ; i < MAX_VIDEO_HANDLES ; i++ ) {
			if (!qstrcmp(cinTable[i].fileName, name) ) {
        return i;
    }
  }
	}
  ...
  qstrcpy(cinTable[currentHandle].fileName, name);
  char const *fileextPtr = COM_GetExtension(name);	// using the function from soundfile/audiocodec-detection
	if(!Q_stricmp(fileextPtr, "ogm") || !Q_stricmp(fileextPtr, "ogv"))
	{
		if(int err = Cin_OGM_Init(name))
		{
			Com_DPrintf(S_COLOR_YELLOW"starting ogm-playback failed(%s) with error %i\n", arg, err);
			cinTable[currentHandle].fileName[0] = 0;
			Cin_OGM_Shutdown();
			return -1;
		}
