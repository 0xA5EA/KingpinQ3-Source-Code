...
//FIXME: 0xA5EA, reicht auch für win ?
...
void AASOuputFile(quakefile_t *qf, char *outputpath, char *filename)
{
	...
	if (!Q_stricmp(ext, "pk3") /*|| !stricmp(ext, "pak") || !stricmp(ext, "sin")*/)
	{
		...
		switch(comp)
		{
			...
			case COMP_BSP2AAS:
			{
				...
				for (qf = qfiles; qf; qf = qf->next)
				{
					...
					create_aas = 1;
					Log_Print("LoadMapFromBSP:\n");
					LoadMapFromBSP(qf);
					//create the AAS file
					Log_Print("AAS_Create:\n");
					AAS_Create(filename);
...
#if defined(WIN32) || defined(_WIN32)
			"Example 1: bspc -bsp2aas d:\\KingpinQ3\\basekpq3\\maps\\mymap?.bsp\n"
			"Example 2: bspc -bsp2aas d:\\KingpinQ3\\basekpq3\\pak0.pk3\\maps/kpq3dm*.bsp\n"
#else
