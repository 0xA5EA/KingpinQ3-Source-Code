...
quakefile_t *FindQuakeFilesInPak(char *pakfile, char *filter)
{
	...
	if ((fread(&packheader, 1, sizeof(dpackheader_t), fp) != sizeof(dpackheader_t))
		...
		||	(fseek(fp, LittleLongl(packheader.dirofs), SEEK_SET))
		)
	{
		fclose(fp);
	...
	if (packheader.ident == IDPAKHEADER)
	{
		...
		numpackdirs = LittleLongl(packheader.dirlen) / sizeof(dpackfile_t);
		...
		for (i = 0; i < numpackdirs; i++)
		{
			...
			packfiles[i].filepos = LittleLongl(idpackfiles[i].filepos);
			packfiles[i].filelen = LittleLongl(idpackfiles[i].filelen);
		} //end for
		free(idpackfiles);
	} //end if
	...
	{
		//number of dir entries in the pak file
		numpackdirs = LittleLongl(packheader.dirlen) / sizeof(dsinpackfile_t);
		packfiles = (dsinpackfile_t *) malloc(numpackdirs * sizeof(dsinpackfile_t));
		if (!packfiles) Error("out of memory");
		//read the dir entry
		...
		for (i = 0; i < numpackdirs; i++)
		{
			packfiles[i].filepos = LittleLongl(packfiles[i].filepos);
			packfiles[i].filelen = LittleLongl(packfiles[i].filelen);
		} //end for
	} //end else
	//
	for (i = 0; i < numpackdirs; i++)
...
quakefile_t *FindQuakeFilesWithPakFilter(char *pakfilter, char *filter)
{
	...
	int j;
#endif
	quakefile_t *qfiles, *lastqf, *qf = NULL;
