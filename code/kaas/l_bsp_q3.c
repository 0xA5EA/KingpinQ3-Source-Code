...
void Q3_SwapBSPFile( void ) {
  ...
  byte* byteptr;
	...
	for ( i = 0 ; i < q3_numShaders ; i++ ) {
		q3_dshaders[i].contentFlags = LittleLongl( q3_dshaders[i].contentFlags );
		q3_dshaders[i].surfaceFlags = LittleLongl( q3_dshaders[i].surfaceFlags );
	}
  ...
  ((int *)&q3_visBytes)[1] = LittleLongl( ((int *)&q3_visBytes)[1] );
#endif

	// drawverts (don't swap colors )
	for ( i = 0 ; i < q3_numDrawVerts ; i++ ) {
		q3_drawVerts[i].lightmap[0] = LittleFloatl( q3_drawVerts[i].lightmap[0] );
		q3_drawVerts[i].lightmap[1] = LittleFloatl( q3_drawVerts[i].lightmap[1] );
		q3_drawVerts[i].st[0] = LittleFloatl( q3_drawVerts[i].st[0] );
		q3_drawVerts[i].st[1] = LittleFloatl( q3_drawVerts[i].st[1] );
		q3_drawVerts[i].xyz[0] = LittleFloatl( q3_drawVerts[i].xyz[0] );
		q3_drawVerts[i].xyz[1] = LittleFloatl( q3_drawVerts[i].xyz[1] );
		q3_drawVerts[i].xyz[2] = LittleFloatl( q3_drawVerts[i].xyz[2] );
		q3_drawVerts[i].normal[0] = LittleFloatl( q3_drawVerts[i].normal[0] );
		q3_drawVerts[i].normal[1] = LittleFloatl( q3_drawVerts[i].normal[1] );
		q3_drawVerts[i].normal[2] = LittleFloatl( q3_drawVerts[i].normal[2] );
	}
	...
	for ( i = 0 ; i < q3_numFogs ; i++ ) {
		q3_dfogs[i].brushNum = LittleLongl( q3_dfogs[i].brushNum );
	}
}
...
void Q3_AddLump( FILE *bspfile, q3_dheader_t *header, int lumpnum, void *data, int len )
{
	...
	lump->fileofs = LittleLongl( ftell(bspfile) );
	lump->filelen = LittleLongl( len );
...
void Q3_WriteBSPFile( char *filename )
{
	...
	header->ident = LittleLongl( Q3_BSP_IDENT );
	header->version = LittleLongl( Q3_BSP_VERSION );
