...
void SV_SetConfigstring (int index, const char *val) {
	...
	if ( !qstrcmp( val, sv.configstrings[ index ] ) ) {
		return;
	}
...
#ifdef USE_LLVM //hypov8 check this
	fileHandle_t f;
...
//	Cvar_Set( "nextmap", va("map %s", server) );
	...
	// FIXME: 0xA5EA, was ist mit weaponmode, hier beachten ????
...
void SV_Init (void)
{
	...
	// 3 fixed masters
	sv_master[0] = Cvar_Get("sv_master0", MASTER_SERVER_NAME0, CVAR_ROM);
	sv_master[1] = Cvar_Get("sv_master1", MASTER_SERVER_NAME1, CVAR_ROM);
	sv_master[2] = Cvar_Get("sv_master2", MASTER_SERVER_NAME2, CVAR_ROM);
	// 3 custom masters
	for (index = 3; index < MAX_MASTER_SERVERS; index++)
		sv_master[index] = Cvar_Get(va("sv_master%d", index), "", CVAR_ARCHIVE);
