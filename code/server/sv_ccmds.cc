...
static void SV_MapRestart_f(void)
{
	...
	//FIXME: 0xA5EA, weaponmode auch beruecksichtigen ??
...
static void SV_WriteBans(void)
{
	...
	if ((writeto = FS_SV_FOpenFileWrite(filepath)))
	{
		...
		for(index = 0; index < serverBansCount; index++)
		{
			...
			FS_Write(writebuf, qstrlen(writebuf), writeto);
		}
...
static qboolean SV_DelBanEntryFromList(int index)
{
	...
	{
		Com_Memmove(serverBans + index, serverBans + index + 1, (serverBansCount - index - 1) * sizeof(*serverBans));
		serverBansCount--;
	}
	else
		return qtrue;
...
static void SV_Status_f(void)
{
  ...
  Com_Printf("======================== Server Status =================================\n");
  Com_Printf("%s\n", sv_hostname->string);
	Com_Printf("gametype  : %s\n", sv_gametype->string);
  Com_Printf("weaponmode: %d\n", sv_weaponMode->integer);
  Com_Printf("dmflags:    %u\n", sv_dmflags->integer);
	Com_Printf("map       : %s\n", sv_mapname->string);
  Com_Printf("\n");
  Com_Printf("--- ----- ---- --------------- ------- --------------------- ----- -----\n");
...
void SV_AddOperatorCommands(void)
{
	...
	{
	  //FIXME(0xA5EA): wtf, why only STANDALONE ?
