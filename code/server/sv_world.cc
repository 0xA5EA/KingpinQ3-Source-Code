...
void SV_ClipToEntity(trace_t *trace, const vec3_t start, const vec3_t mins, const vec3_t maxs, const vec3_t end, int entityNum, int contentmask, int capsule)
{
	...
	CM_TransformedBoxTrace(trace, (float *)start, (float *)end,
	                       ...
	                       origin, angles, (traceType_t)capsule);
...
static void SV_ClipMoveToEntities( moveclip_t *clip ) {
	...
	for(i = 0; i < num; i++)
	{
		...
		CM_TransformedBoxTrace(&trace, (float *)clip->start, (float *)clip->end,
		                       ...
		                       origin, angles, (traceType_t)clip->capsule);
...
void SV_Trace(trace_t *results, const vec3_t start, vec3_t mins, vec3_t maxs, const vec3_t end, int passEntityNum, int contentmask, int capsule)
{
	...
	// clip to world
	CM_BoxTrace(&clip.trace, start, end, mins, maxs, 0, contentmask, (traceType_t)capsule);
...
int SV_PointContents(const vec3_t p, int passEntityNum)
{
	...
	for(i = 0; i < num; i++)
	{
		...
		clipHandle = SV_ClipHandleForEntity(hit);
                // 0xA5EA: no rotating entities in kpq3
#define ROTATING_ENTS 0
#if (ROTATING_ENTS > 0)
		angles     = hit->s.angles;
		if(!hit->r.bmodel)
		{
