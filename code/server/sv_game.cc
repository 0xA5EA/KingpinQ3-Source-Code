...
void SV_SetBrushModel(sharedEntity_t *ent, const char *name)
{
  ...
  //Com_Printf(S_COLOR_CYAN"SV_SetBrushModel\n");
	if(!name)
	{
	...
	//Com_Printf(S_COLOR_CYAN"ent->r.mins[0] %f, ent->r.mins[1] %f, ent->r.mins[2] %f\n", ent->r.mins[0], ent->r.mins[1],ent->r.mins[2]);
  //Com_Printf(S_COLOR_CYAN"ent->r.maxs[0] %f, ent->r.maxs[1] %f, ent->r.maxs[2] %f\n", ent->r.maxs[0], ent->r.maxs[1],ent->r.maxs[2]);
	
	ent->r.contents = -1;       // we don't know exactly what is in the brushes
	Com_Printf(S_COLOR_CYAN"ent->s.solid %d\n", ent->s.solid);
...
qboolean SV_EntityContact(vec3_t mins, vec3_t maxs, const sharedEntity_t *gEnt, int capsule)
{
	...
	CM_TransformedBoxTrace(&trace, vec3_origin, vec3_origin, mins, maxs, ch, -1, origin, angles, (traceType_t)capsule);
    ...
    case G_ACOS:
      return FloatAsInt(Q_acos(VMF(1)));
		...
		case BOTLIB_AAS_FLOAT_FOR_BSP_EPAIR_KEY:
			return botlib_export->aas.AAS_FloatForBSPEpairKey(args[1], (char*)VMA(2), (vec_t*)VMA(3));
		...
		case BOTLIB_AI_GENETIC_PARENTS_AND_CHILD_SELECTION:
			return botlib_export->ai.GeneticParentsAndChildSelection(args[1], (vec_t*)VMA(2), (int*)VMA(3), (int*)VMA(4), (int*)VMA(5));
		...
		case TRAP_MATRIXMULTIPLY:
			MatrixMultiply((const vec_t*)VMA(1), (const vec_t*)VMA(2), (vec_t*)VMA(3));
			return 0;
