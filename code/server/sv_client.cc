...
void SV_GetChallenge(netadr_t from)
{
	...
	if (gameName && *gameName)
	{
		...
		if (qstrcmp(gameName, com_gamename->string) !=0 )
		{
	...
	if (challenge->adr.type == NA_IP && !com_standalone->integer && !Sys_IsLANAddress(from))
	{
		...
		else
		{
			...
			qstrcpy(game, BASEGAME);
			fs = Cvar_Get ("fs_game", "", CVAR_INIT|CVAR_SYSTEMINFO );
			if (fs && fs->string[0] != 0) {
				qstrcpy(game, fs->string);
			}
...
void SV_DirectConnect(netadr_t from)
{
	...
	version = atoi(Info_ValueForKey(userinfo, "protocol")); //hypov8 "protocol"
	...
	if ((qstrlen(ip) + qstrlen(userinfo) + 4) >= MAX_INFO_STRING)
	{
		NET_OutOfBandPrint(NS_SERVER, from,
...
//			VM_Call( gvm, GAME_CLIENT_DISCONNECT, newcl - svs.clients );
	...
	if (!qstrcmp(password, sv_privatePassword->string))
	{
		startIndex = 0;
	}
	else
...
void SV_DropClient(client_t *drop, const char *reason)
{
	...
	if ( isBot )
		drop->state = CS_FREE; // bots shouldn't go zombie, as there's no real net connection.
	else
	{
...
int SV_WriteDownloadToClient(client_t *cl, msg_t *msg)
{
	      ...
	      cl->downloadSize != cl->downloadCount)
	{
		...
		if (!cl->downloadBlocks[curindex])
			cl->downloadBlocks[curindex] = static_cast<unsigned char*>(Z_Malloc(MAX_DOWNLOAD_BLKSIZE));
...
void SV_UserinfoChanged(client_t *cl)
{
	...
	if (val[0])
		len = qstrlen(ip) - qstrlen(val) + qstrlen(cl->userinfo);
	else
		len = qstrlen(ip) + 4 + qstrlen(cl->userinfo);
...
static qboolean SV_ClientCommand(client_t *cl, msg_t *msg)
{
	...
	// don't allow another command for 250 milliseconds
...
void SV_UserVoip(client_t *cl, msg_t *msg)
{
	...
	for (i = 0, client = svs.clients; i < sv_maxclients->integer ; i++, client++) {
		...
		if (client->queuedVoipPackets >= (int)ARRAY_LEN(client->voipPacket)) {
			Com_Printf("Too many VoIP packets queued for client #%d\n", i);
			continue;                                                                                                                                                             // no room for another packet right now.
		}

		packet = static_cast<voipServerPacket_t*>(Z_Malloc(sizeof(*packet)));
...
void SV_ExecuteClientMessage(client_t *cl, msg_t *msg)
{
	...
	if ( serverId != sv.serverId && !*cl->downloadName && !Q_strstr(cl->lastClientCommandString, "nextdl") ) 
	{
