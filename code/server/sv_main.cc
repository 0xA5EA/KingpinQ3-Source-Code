...
cvar_t *sv_minRate;
cvar_t *sv_maxRate;
cvar_t *sv_maxvelocity; //add hypov8 cap bunnyhop max speed?

cvar_t	*sv_dlRate;
cvar_t *sv_minPing;
cvar_t *sv_maxPing;
cvar_t *sv_gametype;
cvar_t *sv_weaponMode;
cvar_t *sv_dmflags;
cvar_t *sv_pure;
cvar_t *sv_floodProtect;
cvar_t *sv_lanForceRate;                  // dedicated 1 (LAN) server forces local client rates to 99999 (bug #491)
#ifndef STANDALONE                        // 0xA5EA, added
...
static int SV_ReplacePendingServerCommands( client_t *client, const char *cmd ) {
	...
	for ( i = client->reliableSent+1; i <= client->reliableSequence; i++ ) {
		...
		//
		if ( !Q_strncmp(cmd, client->reliableCommands[ index ], qstrlen("cs")) ) {
...
================
*/
#define HEARTBEAT_MSEC 300 * 1000 //300 seconds. hypov8 speed up to debug master server setup
void SV_MasterHeartbeat(const char *message)
{
	...
	for(i = 0; i < MAX_MASTER_SERVERS; i++)
	{
		...
		//hypo todo: add cvar to keep getting dns resolves
		...
		if(sv_master[i]->modified || (adr[i][0].type == NA_BAD && adr[i][1].type == NA_BAD))
		{
			...
			{
				Com_Printf("Resolving Master%i (IPv4) %s \n", i+1, sv_master[i]->string);
				...
				if(res)
					Com_Printf("Resolved  Master%i (IPv4) %s  to  %s\n", i+1, sv_master[i]->string, NET_AdrToStringwPort(adr[i][0]));
				else
			...
			if(netenabled & NET_ENABLEV6)
			{
				...
				if(res)
					Com_Printf("%s resolved IPv6 to %s\n", sv_master[i]->string, NET_AdrToStringwPort(adr[i][1]));
				else
		...
		if(adr[i][0].type != NA_BAD)
			NET_OutOfBandPrint(NS_SERVER, adr[i][0], "heartbeat %s\n", message); // HEARTBEAT_FOR_MASTER //flatlineformaster
			//NET_OutOfBandPrint(NS_SERVER, adr[i][0], "heartbeat %s kpq3_server\n", message); //HEARTBEAT_FOR_MASTER
...
static void SVC_Status( netadr_t from )
{
  ...
  qstrcpy(infostring, Cvar_InfoString(CVAR_SERVERINFO));
  ...
  for (i = 0; i < sv_maxclients->integer; i++)
  {
    ...
    if (cl->state >= CS_CONNECTED)
    {
      ...
      playerLength = qstrlen(player);
      if (statusLength + playerLength >= (int) sizeof(status))
      {
        break; // can't hold any more
      }
      qstrcpy(status + statusLength, player);
...
void SVC_Info( netadr_t from ) {
  ...
  //hypov8 reset whole sting. incase
  ...
  //infostring[0] = 0;
	...
	Info_SetValueForKey(infostring, "gamename", com_gamename->string); //hypov8  "gamename"  //hypov8 "com_gamename"
	Info_SetValueForKey(infostring, "protocol", va("%i", PROTOCOL_VERSION));
	Info_SetValueForKey(infostring, "hostname", sv_hostname->string);
	Info_SetValueForKey(infostring, "mapname", sv_mapname->string);
	Info_SetValueForKey(infostring, "clients", va("%i", count));
	Info_SetValueForKey(infostring, "g_humanplayers", va("%i", humans)); //hypov8 do we need all this info
	Info_SetValueForKey(infostring, "sv_maxclients", //hypov8 todo: remove sv_ and g_ values. reduce string length sent
	                    va("%i", sv_maxclients->integer - sv_privateClients->integer));
	Info_SetValueForKey(infostring, "gametype", va("%i", sv_gametype->integer));
	Info_SetValueForKey(infostring, "weaponmode", va("%i", sv_weaponMode->integer));      // 0xA5EA, added
	Info_SetValueForKey(infostring, "pure", va("%i", sv_pure->integer));
	Info_SetValueForKey(infostring, "g_needpass", va("%d", Cvar_VariableIntegerValue("g_needpass"))); //hypov8 do we need all this info
...
#define SV_OUTPUTBUF_LENGTH (1024 - 16)
	...
	if (!qstrlen( sv_rconPassword->string ) || qstrcmp (Cmd_Argv(1), sv_rconPassword->string))
	{
		static leakyBucket_t bucket;
	...
	if (!qstrlen( sv_rconPassword->string ))
	{
...
static void SV_CalcPings(void)
{
  ...
  if (sv_maxclients->integer <= 0) //add hypov8
	  return;
