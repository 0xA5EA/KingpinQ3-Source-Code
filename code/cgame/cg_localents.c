...
void CG_FragmentBounceMark(localEntity_t *le, trace_t *trace)
{
  ...
  {
    radius = 16 + (rand() & 31);
    CG_ImpactMark(cgs.media.bloodMarkShader[rand() % 3], trace->endpos, trace->plane.normal, random() * 360,
...
static void CG_AddExplosion(localEntity_t *ex)
{
  ...
  if (ex->light)
  {
    ...
    trap_R_AddAdditiveLightToScene(ent->origin, light, ex->lightColor[0], ex->lightColor[1], ex->lightColor[2]);
  }
}
