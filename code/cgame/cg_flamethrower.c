...
hypov8 todo: match distance to server
===============
*/
...
void CG_MoveFlameChunk(flameChunk_t *f)
{
  ...
  while (f->velSpeed > 1 && f->baseOrgTime != cg.time)
  {
    ...
    if (trace.fraction == 1.0)
    {
      ...
      if ((f->ownerCent != cg.snap->ps.clientNum) && !(cg.snap->ps.eFlags & EF_DEAD) && Distance(newOrigin, cg.snap->ps.origin) < 32)
...
===============
*/
static vec3_t v_right, v_up;
static vec3_t r_right, r_up;
...
static void CG_AddFlameSpriteToScene(flameChunk_t *f, float lifeFrac, float alpha)
{
  ...
  pPolyBuffer = CG_PB_FindFreePolyBuffer(cg.flameFxRotate & 1 ? getTestShader() : flameShaders[frameNum], 4, 6); //cg_fxflags
  ...
  if ((rotatingFlames) && (!(cg.flameFxRotate & 1))) //cg_fxflags
  { // JPW NERVE no rotate for alt flame shaders
    VectorToAngles(cg.refdef.viewaxis[0], rotate_ang);
    rotate_ang[ROLL] += f->rollAngle;
    AngleVectors(rotate_ang, NULL, r_right, r_up);
  }
  else
  {
    VectorCopy(v_right, r_right);
    VectorCopy(v_up,    r_up);
  }

  VectorMA(f->org, -rST[1], r_up, point);
  VectorMA(point, -rST[0], r_right, point);
  ...
  VectorMA(point, rST[1] * 2, r_up, point);
  ...
  VectorMA(point, rST[0] * 2, r_right, point);
...
static void CG_AddFlameToScene(flameChunk_t *fHead)
{
  ...
  while (f)
  {
    ...
    {
      CG_AddFlameSpriteToScene(f, f->lifeFrac, 1.0);   // is it in the blue ignition section of the flame?
    }
    else if (isClientFlame && f->blueLife > (lived / 2.0))
    {
      ...
      if (!skip)
      {
        ...
        if (f->blueLife > lived * (f->ignitionOnly ? 3.0 : 3.0))
        {
          ...
          blueTrailHead = CG_AddTrailJunc(blueTrailHead, NULL, shader, cg.time, STYPE_STRETCH, f->org, 1,
                                          alpha, alpha, f->size*(f->ignitionOnly  /*&& (cg.snap->ps.clientNum != f->ownerCent || cg_thirdPerson.integer)*/? 2.0 : 1.0), FLAME_MAX_SIZE, TJFL_NOCULL | TJFL_FIXDISTORT, c, c, 1.0, 5.0);
        }
        ...
        if (!f->ignitionOnly)
        {
          ...
          if (!fskip)
          {
            ...
            fuelTrailHead = CG_AddTrailJunc(fuelTrailHead, NULL, // rain - zinx's trail fix
                                            cgs.media.flamethrowerFireStream, cg.time, (f->ignitionOnly ? STYPE_STRETCH : STYPE_REPEAT),
                                            f->org, 1, alpha, alpha, (f->size / 2 < f->sizeMax / 4 ? f->size / 2 : f->sizeMax/4),
                                            FLAME_MAX_SIZE, TJFL_NOCULL | TJFL_FIXDISTORT | TJFL_CROSSOVER, c, c, 0.5, 1.5);
          }
        }
      }
    }
...
#define FLAME_SPRITE_START_BLUE_SCALE   0.2
    ...
    if (!f->ignitionOnly && ((float) (FLAME_SPRITE_START_BLUE_SCALE * f->blueLife) < (float) lived))
    {
      ...
      while (fNext && !droppedTrail)
      {
        if ((Distance(f->org, fNext->org) < ((0.1 + 0.9 * f->lifeFrac) * f->size * 0.35)) &&
            (fabs(f->size - fNext->size) < (40.0)) && (fabs((float)(f->timeStart - fNext->timeStart)) < 100) &&
      ...
      if (!skip)
        CG_AddFlameSpriteToScene(f, lifeFrac, alpha);   // draw the sprite
  ...
  if (fHead->ignitionOnly)
  {
    ...
    trap_R_AddAdditiveLightToScene(lightOrg, 80, 0.2f, 0.21f, 0.5f);
  }
  else if (isClientFlame || (fHead->ownerCent == cg.snap->ps.clientNum))
  {
    trap_R_AddAdditiveLightToScene(lightOrg, 320, 1.000000f, 0.603922f, 0.207843f);
  }
}
...
/*
 ...
 =============
 */
void CG_GenerateShaders(char *filename, char *shaderName, char *dir, int numFrames, char *srcBlend, char *dstBlend,
                        char *extras, qboolean compressedVersionAvailable, qboolean nomipmap)
{
  fileHandle_t f;
...
void CG_AddFlameChunks(void)
{
  ...
  //AngleVectors(cg.refdef.viewangles, NULL, v_right, v_up);
  VectorCopy(cg.refdef.viewaxis[1], v_right);
  VectorCopy(cg.refdef.viewaxis[2], v_up);
  ...
  while (f)
  {
    ...
    if (f->dead)
    {
      ...
      CG_AddFlameToScene(f); // don't draw the ignition flame after we start firing
...
===============
*/
void CG_UpdateFlamethrowerSounds(void) //hypov8 todo: do we need this
{
#if 0
...
#define MIN_BLOW_VOLUME     30
  ...
  // draw each of the headFlameChunk's
  f = headFlameChunks;
  //FIXME (0xA5EA): deaktivated code to get rid of compile errors
  ...
  while (f)
  {
    ...
    if (centFlameInfo[f->ownerCent].lastSoundUpdate != cg.time)
    {
      ...
      if (centFlameStatus[f->ownerCent].blowVolume * 255.0 > MIN_BLOW_VOLUME)
      {
        ...
        trap_S_AddLoopingSound(const vec3_t origin, const vec3_t velocity, sfxHandle_t sfx, int volume, int soundTime)
#endif
      ...
      {
        trap_S_AddLoopingSound(f->org, vec3_origin, cgs.media.flameStreamSound, (int) (255.0
                * centFlameStatus[f->ownerCent].streamVolume), 0); // JPW NERVE
      }
    ...
    for (trav = f; trav; trav = trav->nextFlameChunk)
    {
      ...
      {
        trap_S_AddLoopingSound(trav->org, vec3_origin, cgs.media.flameSound, (int) (255.0 * (0.2
                    * (trav->size / FLAME_MAX_SIZE))), 0);
      }
    }
