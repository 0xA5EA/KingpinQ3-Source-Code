...
qboolean CG_XPPM_RegisterClientModel(clientInfo_t *ci, const char *modelName, const char *skinName, const char *headModelName, const char *headSkinName, const char *teamName)
{
  ...
  ci->bodyModel = trap_R_RegisterModel(filename); //hypov8 was = RegisterModel(filename, qfalse);
...
static void CG_XPPM_PlayerAngles(centity_t *cent, vec3_t legsAngles, vec3_t torsoAngles, vec3_t headAngles)
{
  ...
  */
  ...
  //FIXME: 0xA5EA, auskommentiert wegen compilerfehler
...
#ifdef GT_USE_TA_TYPES     //hypov8 remove TA stuff
...
void CG_XPPM_Player(centity_t *cent)
{
  ...
  Com_Memcpy(&body.skeleton, &cent->pe.legs.skeleton, sizeof(refSkeleton_t));
  ...
  if (firstTorsoBone >= 0 && firstTorsoBone < cent->pe.torso.skeleton.numBones)
  {
    ...
    if (lastTorsoBone >= 0 && lastTorsoBone < cent->pe.torso.skeleton.numBones)
    {
      ...
      {
        Com_Memcpy(&body.skeleton.bones[i], &cent->pe.torso.skeleton.bones[i], sizeof(refBone_t));
      }
    }
