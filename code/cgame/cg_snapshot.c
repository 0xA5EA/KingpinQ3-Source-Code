...
void CG_SetInitialSnapshot(snapshot_t *snap)
{
  ...
  cg.flameFxRotate = 0; //cg_fxflags = 0; //WP_FLAMER
...
static void CG_TransitionSnapshot(void)
{
	...
	oldWeapon = oldFrame->ps.weapon; //daemon .5
	...
	playerState_t *ops = &oldFrame->ps, *ps = &cg.snap->ps; //daemon .5
	...
	if (oldWeapon != ps->weapon) { //unvan 0.50
...
void CG_ProcessSnapshots(void)
{
  ...
  while (!cg.snap)
  {
    ...
    if (!snap)
      return; // we can't continue until we get a snapshot
  ...
  do
  {
    ...
    if (!cg.nextSnap)
    {
	  ...
	  if (cg.nextSnap && cg.nextSnap->serverTime < cg.snap->serverTime)// hypov8 dereferencing null pointer
	  {
  ...
  if (cg.time < cg.snap->serverTime)
    cg.time = cg.snap->serverTime;  // this can happen right after a vid_restart
