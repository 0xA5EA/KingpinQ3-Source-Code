...
void CG_BuildSolidList(void)
{
  ...
  for (i = 0; i < snap->numEntities; i++)
  {
    ...
    if (cent->nextState.solid && ent->eType != ET_MISSILE && ent->eType != ET_FLAMETHROWER_CHUNK )
    {
...
===================
CG_TouchItem
hypov8 client and server Touch_Item
===================
*/
static void CG_TouchItem(centity_t *cent)
{
  ...
  if (!BG_CanItemBeGrabbed(cgs.gametype, &cent->currentState, &cg.predictedPlayerState, cgs.playermaxcash, cgs.playermaxcashstolen))
    return;     // can't hold it

  item = &bg_itemlist[cent->currentState.modelindex];

  //FIXME(0xA5EA): bagman !!
  //FIXME(0xA5EA): Bagman cash
#ifdef WITH_BAGMAN_MOD
	if (cgs.gametype == GT_BAGMAN)
	{
		if (item->giType == IT_CASH)
		{
		  if (!BG_CanPickupCash(cgs.playermaxcash, &cg.predictedPlayerState))
			return;
		}
		else if (item->giType == IT_CASH_STOLEN)
		{
			if (!BG_CanPickupCashStolen(cgs.playermaxcashstolen, &cg.predictedPlayerState))
				return;
		}
		else if (item->giType == IT_TEAM_SAFE)
			return; //no client prediction needed
	}
	...
	{
		//its our own flag!!
		if (cg.predictedPlayerState.persistant[PERS_TEAM] == TEAM_DRAGONS && item->giTag == PW_DRAGONFLAG)
			return;
		//its our own flag!!
		if (cg.predictedPlayerState.persistant[PERS_TEAM] == TEAM_NIKKIS && item->giTag == PW_NIKKIFLAG)
			return;
	}
  ...
  //if (item->giType != IT_WEAPON) //prediction dont remove weps 
	//cent->currentState.eFlags |= EF_NODRAW; //hypov8 todo: buggy. disapears, aperars then disapears.. 
  ...
  {
	if (!(cg.predictedPlayerState.stats[STAT_WEAPONS] & (1 << item->giTag))) //hypov8
    cg.predictedPlayerState.stats[STAT_WEAPONS] |= (1 << item->giTag);
    //FIXME(0xA5EA): rounds ?

	//hypov8 add ammo to prediction
	if (!cg.predictedPlayerState.ammo_mag[item->giTag])
		cg.predictedPlayerState.ammo_mag[item->giTag] = 1; //add 1. so we can load wep early..

	//hypov8 add ammo to prediction
	//if (!cg.predictedPlayerState.ammo[item->giTag])
	//	cg.predictedPlayerState.ammo[item->giTag] = /*1*/ item->quantity; //hypov8 change to ammo count??
...
static int CG_IsUnacceptableError( playerState_t *ps, playerState_t *pps ) {
	...
	for ( i = 0; i < MAX_WEAPONS; i++ ) {
		if (pps->ammo_all[BG_AmmoCombineCheck(i)] != ps->ammo_all[BG_AmmoCombineCheck(i)]) {
			return 17;
		}
	}

	for ( i = 0; i < MAX_WEAPONS; i++ ) { //hypov8 todo: arrange
		if (pps->ammo_mag[i] != ps->ammo_mag[i]) {
			return 18;
		}
	}
