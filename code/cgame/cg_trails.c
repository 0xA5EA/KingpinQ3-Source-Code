...
==============
*/
static vec3_t svforward, svright, svup;
...
#define TRAIL_FADE_FAR_SCALE    4.0
  ...
  if (trail->flags & TJFL_SPARKHEADFLARE)
  {
    ...
    if (pPolyBuffer)
    {
      ...
      VectorMA( pPolyBuffer->xyz[pos], -j->width * 2, svup, pPolyBuffer->xyz[pos] );
      VectorMA( pPolyBuffer->xyz[pos], -j->width * 2, svright, pPolyBuffer->xyz[pos] );
  ...
  if (!numJuncs)
  {
    ...
    while (j)
    {
      ...
      if (!j->inuse && j->nextJunc && !j->nextJunc->inuse)
        ...
        j->nextJunc = NULL;  // not sure how this can happen, but it does, and causes infinite loops

      if (j->nextJunc)
        sInc += Distance(j->nextJunc->pos, j->pos);
  ...
  while (jNext)
  {
    ...
    {
      //s += sInc;
      s += Distance(j->pos, jNext->pos) / sInc;
      if (s > 1.0)
        s = 1.0;
    }
...
void CG_AddTrails(void)
{
  ...
  //AngleVectors( cg.snap->ps.viewangles, svforward, svright, svup );
  VectorCopy( cg.refdef.viewaxis[0], svforward );
  VectorCopy( cg.refdef.viewaxis[1], svright );
  VectorCopy( cg.refdef.viewaxis[2], svup );
