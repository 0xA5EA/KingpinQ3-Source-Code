...
void CG_InitParticles(void)
{
  ...
  Com_Memset(cg_allParticles, 0, sizeof(cg_allParticles));
...
void CG_AddParticleToScene(cparticle_t *p, vec3_t org, vec4_t color)
{
  ...
  else if (p->type == P_FLAT_SCALEUP)
  {
    ...
    //Q_SinCos(p->roll*DEG2RAD, &sinR, &cosR);

    //sinR *= height*M_SQRT2;
    //cosR *= width*M_SQRT2;
    sinR = height * sin(p->roll*M_DEG2RAD)*M_SQRT2; //* sqrt(2);
    cosR = width * cos(p->roll*M_DEG2RAD)*M_SQRT2; // * sqrt(2);
  ...
  if (!p->pshader)
  {
    Com_DPrintf(S_COLOR_RED"CG_AddParticleToScene type %d p->pshader == ZERO\n", p->type);
    return;
  }
      ...
      if (p->bounceFactor)
      {
        ...
        if (trace.fraction > 0 && trace.fraction < 1)
        {
          ...
          if(p->type == P_BLOOD)
          {
            ...
            radius = 3 + random() * 5;
            r = rand() & 2;
            shader = cgs.media.bloodMarkShader[r];
...
void CG_ParticleBlood(vec3_t org, vec3_t dir, int count)
{
  ...
  for (i = 0; i < count; i++)
  {
    ...
    p->pshader =  cgs.media.bloodSpurtShader[rand() % 3];
...
// 		p->pshader = cgs.media.smokePuffShader;  // 0xA5EA, war orginal unkommentiert
...
{
    vec3_t start, end;
  Com_Printf("executing CG_ParticleBloodCloud\n");
...
{
  vec3_t start, end;
  Com_Printf("executing CG_ParticleBlood\n");
  VectorMA(cg.refdef.vieworg, 100, cg.refdef.viewaxis[0], start);
  VectorMA(start, 20, cg.refdef.viewaxis[1], end);
  CG_ParticleBlood(start, cg.refdef.viewaxis[1], 100);
}

void CG_TestParticleRocketFire_f(void)
{
    vec3_t start, end;
  Com_Printf("executing CG_ParticleRocketFire\n");
...
{
  vec3_t start, end;
  Com_Printf("executing CG_BloodPool\n");
  VectorMA(cg.refdef.vieworg, 100, cg.refdef.viewaxis[0], start);
  VectorMA(start, 20, cg.refdef.viewaxis[1], end);
  CG_BloodPool(cgs.media.bloodMarkShader[0], cg.testModelEntity.origin);
}

void CG_TestParticleBleed_f(void)
{
    vec3_t start, end;
  Com_Printf("executing CG_Bleed\n");
...
{
 vec3_t start;
 Com_Printf("executing CG_GibPlayer\n");
 VectorMA(cg.refdef.vieworg, 100, cg.refdef.viewaxis[0], start);
 CG_GibPlayer(start);
}
...
{
 vec3_t start;
 Com_Printf("executing CG_ParticleTeleportEffect\n");
 VectorMA(cg.refdef.vieworg, 100, cg.refdef.viewaxis[0], start);
...
void CG_TestParticles_f(void)
{
  ...
  case 0:
    CG_ParticleBloodCloud(cg.testModelEntity.origin, cg.refdef.viewaxis[0]);
  Com_Printf("CG_ParticleBloodCloud \n");
    break;
  case 1:
    CG_ParticleRocketFire(start, end);
    Com_Printf("CG_ParticleRocketFire \n");
    break;
  case 2:
    CG_ParticleSparks2(start, cg.refdef.viewaxis[1], 50);
    Com_Printf("CG_ParticleSparks2\n");
    break;
  case 3:
    CG_ParticleSparks(start, cg.refdef.viewaxis[1], 1000, 20, 30, 600);
    Com_Printf("CG_ParticleSparks\n");
    break;
  case 4:
    CG_ParticleRick(start, cg.refdef.viewaxis[1]);
    Com_Printf("CG_ParticleRick\n");
    break;
  case 5:
    CG_ParticleBlood(start, cg.refdef.viewaxis[1], 100);
    Com_Printf("CG_ParticleBlood\n");
    break;
  case 6:
    CG_BloodPool(cgs.media.bloodMarkShader[0], cg.testModelEntity.origin);
    Com_Printf("CG_BloodPool\n");
    break;
  case 7:
    CG_ParticleDust(NULL, start, cg.refdef.viewaxis[1]);
    Com_Printf("CG_ParticleDust\n");
    break;
  case 8:
    CG_Bleed(start, 1);
    Com_Printf("CG_Bleed\n");
