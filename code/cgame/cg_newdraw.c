...
void CG_SetPrintString(int type, const char *p)
{
  ...
  {
    qstrcpy(systemChat, p);
  }
  else
  {
    qstrcpy(teamChat2, teamChat1);
    qstrcpy(teamChat1, p);
  }
}
  ...
  if (cgs.orderPending)
  {
    ...
    else
    {
      ...
      if (sortedTeamPlayers[cg_currentSelectedPlayer.integer] == cg.snap->ps.clientNum && p1)
      {
        ...
        //trap_SendConsoleCommand(va("cmd say_team %s\n", p2));  // nicht von mir auskommentiert, unten auch
        trap_SendConsoleCommand(va("cmd vsay_team %s\n", p1));
      }
      else if (p2)
      {
...
static void CG_DrawPlayerArmorIcon(rectDef_t *rect, qboolean draw2D, armor_t armorRegion)
{
  vec3_t angles;
  vec3_t origin;
  int modelIdx = armorRegion - AR_HEAD; //  AR_HEAD = 5
  int value;
  playerState_t *ps;
  ...
  if (armorRegion == AR_BODY || armorRegion == AR_LEGS || armorRegion == AR_HEAD)
  {
	  ps = &cg.snap->ps;
	  value = ps->stats[armorRegion];
	  if (value <= 0)
		  return;

	  if (value < 50)
		  modelIdx += 3; //hypov8 orange armour
  }
  else
	  modelIdx = 6; //hypov8 add: player background
  ...
  case AR_BODY:
	if (value < 50)
		modelIdx+=3; //hypov8 todo: orange armour?
    break;
  case AR_LEGS:
	if (value < 50)
		modelIdx+=3;
  ...
  else if (cg_draw3dIcons.integer)
  {
    ...
    case AR_LEGS:
      origin[PITCH] = 90;
      origin[YAW]   = 0;
      origin[ROLL]  =  0;       // war -10, hat es etwas nach oben gesetzt
      angles[PITCH] = 90;
      angles[YAW]   = 0;
      angles[ROLL]  =   0;
      break;
    case AR_HEAD:
      //origin[PITCH] = 90;	origin[YAW] = 0; origin[ROLL] = -10;
      origin[PITCH] = 90;
      origin[YAW]   = 0;
      origin[ROLL]  = 0;
      angles[PITCH] =  0;
    ...
    case AR_BODY:
      ...
      angles[YAW]   = 90;
      angles[ROLL]  =   0;
      //angles[YAW] = ( cg.time & 2047 ) * 360 / 2048.0;		// 0xA5EA, no rotation
      break;
    }
    CG_Draw3DModel(rect->x, rect->y, rect->w, rect->h, cgs.media.armorModel[modelIdx], 0, origin, angles);
  }
}

static void CG_DrawPlayerArmorValue(rectDef_t *rect, float scale, vec4_t color, qhandle_t shader, int textStyle, armor_t armorRegion)
{
  char num[16];
  int value;
  playerState_t *ps;
  ...
  else
  {
    ...
    value = CG_Text_Width(num, scale, 0, NULL_FONT);
    CG_Text_Paint(rect->x + (rect->w - value) / 2, rect->y + rect->h, scale, color, num, 0, 0, textStyle, NULL_FONT);
#else
    int w;
	//hypov8 add: right align
    Com_sprintf(num, sizeof(num), "%i",value);
	w = CG_Text_Width(num, scale, 0, NULL_FONT);
    CG_Text_Paint(rect->x - w, rect->y + rect->h, scale, color, num, 0, 0, textStyle, NULL_FONT);
#endif
  }
}


//add hypov8 cash display
static void CG_DrawPlayerCashStolenValue(rectDef_t *rect, float scale, vec4_t color, qhandle_t shader, int textStyle)
{
  char num[16];
  int value;
  playerState_t *ps;
  ...
  ps   = &cg.snap->ps;
  value = ps->stats[STAT_STOLEN_CASH];
...
static void CG_DrawPlayerCashValue(rectDef_t *rect, float scale, vec4_t color, qhandle_t shader, int textStyle)
{
  char num[16];
  int value;
  playerState_t *ps;
...
static void CG_DrawPlayerAmmoIcon(rectDef_t *rect, qboolean draw2D)
{
  ...
  if (draw2D || (!cg_draw3dIcons.integer && cg_drawIcons.integer)) // bk001206 - parentheses
  {
    ...
    if (icon)
    {
      ...
      //CG_DrawPic(  rect->x,  rect->y, 42, 18, icon );
    }
  }
  else if (cg_draw3dIcons.integer)
  {
...
static void CG_DrawPlayerAmmoValue(rectDef_t *rect, float scale, vec4_t color, qhandle_t shader, int textStyle)
{
  ...
  {
	  value = ps->ammo_all[BG_AmmoCombineCheck(cent->currentState.weapon)];
    if (value > -1)
    {
      if (shader)
      {
...
static void CG_DrawPlayerAmmoRoundsValue(rectDef_t *rect, float scale, vec4_t color, qhandle_t shader, int textStyle)
{
  char num[16];
  int value;
  centity_t *cent;
  ...
  if (cent->currentState.weapon)
  {
	  value = ps->ammo_mag[cent->currentState.weapon];
    if (value > -1)
    {
      if (shader)
      {
      ...
      else
      {
        ...
        value = CG_Text_Width(num, scale, 0, NULL_FONT);
        CG_Text_Paint(rect->x + (rect->w - value) / 2, rect->y + rect->h, scale, color, num, 0, 0, textStyle, NULL_FONT);
#if 1 //hypov8 add slash. now removed with crowbar
		Com_sprintf(num, sizeof(num), "%6s /", " ");
        value = CG_Text_Width(num, scale, 0, NULL_FONT);
        CG_Text_Paint(rect->x + (rect->w - value) / 2, rect->y + rect->h, scale, color, num, 0, 0, textStyle, NULL_FONT);
#endif
      }
...
static void CG_DrawSelectedPlayerName(rectDef_t *rect, float scale, vec4_t color, qboolean voice, int textStyle)
{
  ...
  if (ci)
  {
    CG_Text_Paint(rect->x, rect->y + rect->h, scale, color, ci->name, 0, 0, textStyle, NULL_FONT);
  }
}
...
static void CG_DrawSelectedPlayerLocation(rectDef_t *rect, float scale, vec4_t color, int textStyle)
{
  ...
  if (ci)
  {
    ...
    CG_Text_Paint(rect->x, rect->y + rect->h, scale, color, p, 0, 0, textStyle, NULL_FONT);
  }
}
...
static void CG_DrawCapFragLimit(rectDef_t *rect, float scale, vec4_t color, qhandle_t shader, int textStyle)
{
  ...
  Com_sprintf(num, sizeof(num), "%2i", limit);

  value = CG_Text_Width(num, scale, 0, NULL_FONT);
  CG_Text_Paint(rect->x + rect->w - value, rect->y + rect->h, scale, color, num, 0, 0, textStyle, NULL_FONT);
}
...
{
  CG_Text_Paint(rect->x, rect->y + rect->h, scale, color, cg_DragonTeamName.string, 0, 0, textStyle, NULL_FONT);
}

static void CG_DrawBlueName(rectDef_t *rect, float scale, vec4_t color, int textStyle)
{
  CG_Text_Paint(rect->x, rect->y + rect->h, scale, color, cg_NikkiTeamName.string, 0, 0, textStyle, NULL_FONT);
}
...
static void CG_DrawBlueFlagName(rectDef_t *rect, float scale, vec4_t color, int textStyle)
{
  ...
  {
    if (cgs.clientinfo[i].infoValid && cgs.clientinfo[i].team == TEAM_DRAGONS  && cgs.clientinfo[i].powerups & (1 << PW_NIKKIFLAG))
    {
      CG_Text_Paint(rect->x, rect->y + rect->h, scale, color, cgs.clientinfo[i].name, 0, 0, textStyle, NULL_FONT);
      return;
    }
  }
}
...
static void CG_DrawBlueFlagStatus(rectDef_t *rect, qhandle_t shader)
{
  ...
  {
    gitem_t *item = BG_FindItemForPowerup(PW_NIKKIFLAG);
    if (item)
    {
      vec4_t color = {0, 0, 1, 1};
      trap_R_SetColor(color);
...
static void CG_DrawRedFlagName(rectDef_t *rect, float scale, vec4_t color, int textStyle)
{
  ...
  for (i = 0; i < cgs.maxclients; i++)
  {
    if (cgs.clientinfo[i].infoValid && cgs.clientinfo[i].team == TEAM_NIKKIS  && cgs.clientinfo[i].powerups & (1 << PW_DRAGONFLAG))
    {
      CG_Text_Paint(rect->x, rect->y + rect->h, scale, color, cgs.clientinfo[i].name, 0, 0, textStyle, NULL_FONT);
      return;
    }
...
static void CG_DrawRedFlagStatus(rectDef_t *rect, qhandle_t shader)
{
  ...
  {
    gitem_t *item = BG_FindItemForPowerup(PW_DRAGONFLAG);
    if (item)
    {
      vec4_t color = {1, 0, 0, 1};
      trap_R_SetColor(color);
...
static void CG_HarvesterSkulls(rectDef_t *rect, float scale, vec4_t color, qboolean force2D, int textStyle)
{
  ...
  if (cg_drawIcons.integer)
  {
    if (!force2D && cg_draw3dIcons.integer)
    {
      ...
      origin[2]   = -10;
      angles[YAW] = (cg.time & 2047) * 360 / 2048.0;
      if (cg.snap->ps.persistant[PERS_TEAM] == TEAM_NIKKIS)
      {
  ...
  else
  {
    ...
    if (item)
    {
      if (cgs.flagStatus >= 0 && cgs.flagStatus <= 4)
      {
        ...
        int index    = 0;
        if (cgs.flagStatus == FLAG_TAKEN_DRAGON)
        {
          color[1] = color[2] = 0;
          index    = 1;
        }
        else if (cgs.flagStatus == FLAG_TAKEN_NIKKI)
        {
          color[0] = color[1] = 0;
          index    = 1;
        }
...
static void CG_DrawAreaPowerUp(rectDef_t *rect, int align, float special, float scale, vec4_t color)
{
  ...
  for (i = 0; i < active; i++)
  {
    ...
    if (item)
    {
	  ...
	  if (sorted[i] >= PW_DEGRADE_START) //hypov8 add: degrading powerups show timmer
	  {
		  Com_sprintf(num, sizeof(num), "%i", sortedTime[i] / 1000);
		  CG_Text_Paint(r2.x + (r2.w * .75) + 3, r2.y + r2.h, scale, color, num, 0, 0, 0, NULL_FONT);
	  }
      *inc += r2.w + special;
    }
  }
...
float CG_GetValue(int ownerDraw)
{
  ...
  case CG_PLAYER_ARMOR_VALUE:
    ...
    break;
#endif
  case CG_PLAYER_ARMOR_BODY_VALUE:
    return ps->stats[STAT_ARMOR_BODY];
    break;
  case CG_PLAYER_ARMOR_LEGS_VALUE:
    return ps->stats[STAT_ARMOR_LEGS];
    break;
  case CG_PLAYER_ARMOR_HEAD_VALUE:
    return ps->stats[STAT_ARMOR_HEAD];
    break;
	
	// add hypov8 BM scoreboard values
	
  case CG_PLAYER_CASH_STOLEN_VALUE:
    return ps->stats[STAT_STOLEN_CASH];
    break;
  case CG_PLAYER_CASH_VALUE:
    return ps->stats[STAT_CASH];
    break;
  ...
  case CG_PLAYER_AMMO_VALUE:
    if (cent->currentState.weapon)
    {
		return ps->ammo_all[BG_AmmoCombineCheck(cent->currentState.weapon)];
    }
    break;
...
qboolean CG_OtherTeamHasFlag(void)
{
  if (cgs.gametype == GT_CTF || cgs.gametype == GT_1FCTF)
  {
    ...
    if (cgs.gametype == GT_1FCTF)
    {
      if (team == TEAM_DRAGONS && cgs.flagStatus == FLAG_TAKEN_NIKKI)
        return qtrue;
      else if (team == TEAM_NIKKIS && cgs.flagStatus == FLAG_TAKEN_DRAGON)
        return qtrue;
      else
        return qfalse;
    }
    ...
    {
      if (team == TEAM_DRAGONS && cgs.redflag == FLAG_TAKEN)
        return qtrue;
      else if (team == TEAM_NIKKIS && cgs.blueflag == FLAG_TAKEN)
        return qtrue;
      else
        return qfalse;
    }
...
qboolean CG_YourTeamHasFlag(void)
{
  if (cgs.gametype == GT_CTF || cgs.gametype == GT_1FCTF)
  {
    ...
    if (cgs.gametype == GT_1FCTF)
    {
      if (team == TEAM_DRAGONS && cgs.flagStatus == FLAG_TAKEN_DRAGON)
        return qtrue;
      else if (team == TEAM_NIKKIS && cgs.flagStatus == FLAG_TAKEN_NIKKI)
        return qtrue;
      else
        return qfalse;
    }
    ...
    {
      if (team == TEAM_DRAGONS && cgs.blueflag == FLAG_TAKEN)
        return qtrue;
      else if (team == TEAM_NIKKIS && cgs.redflag == FLAG_TAKEN)
        return qtrue;
      else
        return qfalse;
    }
...
qboolean CG_OwnerDrawVisible(int flags)
{
  ...
  if (flags & (CG_SHOW_NIKKI_TEAM_HAS_DRAGONFLAG | CG_SHOW_DRAGON_TEAM_HAS_NIKKIFLAG))
  {
    if (flags & CG_SHOW_NIKKI_TEAM_HAS_DRAGONFLAG && (cgs.redflag == FLAG_TAKEN || cgs.flagStatus == FLAG_TAKEN_DRAGON))
      return qtrue;
    else if (flags & CG_SHOW_DRAGON_TEAM_HAS_NIKKIFLAG && (cgs.blueflag == FLAG_TAKEN || cgs.flagStatus == FLAG_TAKEN_NIKKI))
      return qtrue;
  ...
  if (flags & CG_SHOW_ANYTEAMGAME)
  {
    ...
    if (cgs.gametype >= GT_TEAM && cgs.gametype != GT_BAGMAN)
#else
...
#ifdef WITH_BAGMAN_MOD
  if (flags & CG_SHOW_BAGMAN) //FIXME: 0xA5EA
  {
  ...
  {
    if (cg.snap->ps.powerups[PW_DRAGONFLAG] || cg.snap->ps.powerups[PW_NIKKIFLAG] || cg.snap->ps.powerups[PW_NEUTRALFLAG])
      return qtrue;
  }
  return qfalse;
}
...
static void CG_DrawPlayerHasFlag(rectDef_t *rect, qboolean force2D)
{
  ...
  if (cg.predictedPlayerState.powerups[PW_DRAGONFLAG])
  {
    CG_DrawFlagModel(rect->x + adj, rect->y + adj, rect->w - adj, rect->h - adj, TEAM_DRAGONS, force2D);
  }
  else if (cg.predictedPlayerState.powerups[PW_NIKKIFLAG])
  {
    CG_DrawFlagModel(rect->x + adj, rect->y + adj, rect->w - adj, rect->h - adj, TEAM_NIKKIS, force2D);
  }
...
{
  CG_Text_Paint(rect->x, rect->y + rect->h, scale, color, systemChat, 0, 0, 0, NULL_FONT);
}

static void CG_DrawAreaTeamChat(rectDef_t *rect, float scale, vec4_t color, qhandle_t shader)
{
  CG_Text_Paint(rect->x, rect->y + rect->h, scale, color, teamChat1, 0, 0, 0, NULL_FONT);
}

static void CG_DrawAreaChat(rectDef_t *rect, float scale, vec4_t color, qhandle_t shader)
{
  CG_Text_Paint(rect->x, rect->y + rect->h, scale, color, teamChat2, 0, 0, 0, NULL_FONT);
}
...
static void CG_DrawKiller(rectDef_t *rect, float scale, vec4_t color, qhandle_t shader, int textStyle)
{
  ...
  if (cg.killerName[0])
  {
    ...
    CG_Text_Paint(x - CG_Text_Width(CG_GetKillerText(), scale, 0, NULL_FONT) / 2, rect->y + rect->h, scale, color, CG_GetKillerText(), 0, 0, textStyle, NULL_FONT);
  }
}
...
static void CG_Draw1stPlace(rectDef_t *rect, float scale, vec4_t color, qhandle_t shader, int textStyle)
{
  if (cgs.scores1 != SCORE_NOT_PRESENT)
  {
    CG_Text_Paint(rect->x, rect->y, scale, color, va("%2i", cgs.scores1), 0, 0, textStyle, NULL_FONT);
  }
}
...
static void CG_Draw2ndPlace(rectDef_t *rect, float scale, vec4_t color, qhandle_t shader, int textStyle)
{
  if (cgs.scores2 != SCORE_NOT_PRESENT)
  {
    CG_Text_Paint(rect->x, rect->y, scale, color, va("%2i", cgs.scores2), 0, 0, textStyle, NULL_FONT);
  }
}
...
const char *CG_GetGameStatusText(void)
{
  ...
  else
  {
    ...
    {
      s = va("Dragon leads Nikki, %i to %i", cg.teamScores[0], cg.teamScores[1]);
    }
    else
    {
      s = va("Nikki leads Dragon, %i to %i", cg.teamScores[1], cg.teamScores[0]);
    }
  }
  return s;
}

static void CG_DrawGameModeInfo(rectDef_t *rect, float scale, vec4_t color, qhandle_t shader, int textStyle)
{
  char const* s = va("Server is running %s", CG_GetGamemodeText(cgs.gametype, cgs.weaponmod));
  CG_Text_Paint(rect->x, rect->y + rect->h, scale, color, s, 0, 0, textStyle, NULL_FONT);
}

static void CG_DrawGameStatus(rectDef_t *rect, float scale, vec4_t color, qhandle_t shader, int textStyle)
{
  CG_Text_Paint(rect->x, rect->y + rect->h, scale, color, CG_GetGameStatusText(), 0, 0, textStyle, NULL_FONT);
}
...
{
  CG_Text_Paint(rect->x, rect->y + rect->h, scale, color, CG_GameTypeString(), 0, 0, textStyle, NULL_FONT);
}
...
//    const unsigned char *s = text; // bk001206 - unsigned
    ...
    while (s && *s && count < len)
    {
      ...
      else
      {
        ...
        if (CG_Text_Width(s, useScale, 1, NULL_FONT) + x > max)
        {
          *maxX = 0;
          break;
        }
...
void CG_DrawNewTeamInfo(rectDef_t *rect, float text_x, float text_y, float scale, vec4_t color, qhandle_t shader)
{
  ...
  for (i = 0; i < count; i++)
  {
    ...
    {
      len = CG_Text_Width(ci->name, scale, 0, NULL_FONT);
      if (len > pwidth)
        pwidth = len;
    }
  }
  ...
  for (i = 1; i < MAX_LOCATIONS; i++)
  {
    ...
    if (p && *p)
    {
      len = CG_Text_Width(p, scale, 0, NULL_FONT);
      if (len > lwidth)
        lwidth = len;
    }
  }
  ...
  for (i = 0; i < count; i++)
  {
    ...
    if (ci->infoValid && ci->team == cg.snap->ps.persistant[PERS_TEAM])
    {
     ...
     // CG_GetColorForHealth(ci->health, ci->armor, hcolor);
      //FIXME (0xA5EA): CG_GetColorForHealth
  ...
  if (cg.spectatorLen)
  {
    ...
    if (cg.time > cg.spectatorTime)
    {
      ...
      if (cg.spectatorPaintX <= rect->x + 2)
      {
        if (cg.spectatorOffset < cg.spectatorLen)
        {
          cg.spectatorPaintX += CG_Text_Width(&cg.spectatorList[cg.spectatorOffset], scale, 1, NULL_FONT) - 1;
          cg.spectatorOffset++;
        }
        else
        {
...
void CG_DrawMedal(int ownerDraw, rectDef_t *rect, float scale, vec4_t color, qhandle_t shader)
{
  ...
  {
    color[3] = 1.0;
    value    = CG_Text_Width(text, scale, 0, NULL_FONT);
    CG_Text_Paint(rect->x + (rect->w - value) / 2, rect->y + rect->h + 10, scale, color, text, 0, 0, 0, NULL_FONT);
  }
  trap_R_SetColor(NULL);
}
...
=================
*/
void CG_DrawLevelTimer(rectDef_t *rect, float scale, vec4_t color, qhandle_t shader, int textStyle)
{
  char *s;
...
=================
*/
void CG_DrawWeaponTimer(rectDef_t *rect, float scale, vec4_t color, qhandle_t shader, int textStyle)
{
  char *s;

  s = va("%i", cg.weapontimer_hm );
...
=================
*/
void CG_DrawFPS(rectDef_t *rect, float scale, vec4_t color, qhandle_t shader, int textStyle)
{
  char *s;
...
void CG_OwnerDraw(float x, float y, float w, float h, float text_x, float text_y, int ownerDraw, int ownerDrawFlags, int align, float special, float scale, vec4_t color, qhandle_t shader, int textStyle)
{
  ...
  case CG_PLAYER_ARMOR_ICON:
    CG_DrawPlayerArmorIcon(&rect, qtrue, AR_NONE);        // FIXME: 0xA5EA
    break;
  case CG_PLAYER_ARMOR_BODY_ICON2D:
    CG_DrawPlayerArmorIcon(&rect, qtrue, AR_BODY);                                 // qtrue
    break;
  case CG_PLAYER_ARMOR_HEAD_ICON2D:
    CG_DrawPlayerArmorIcon(&rect, qtrue, AR_HEAD);                                 // qtrue
    break;
  case CG_PLAYER_ARMOR_LEGS_ICON2D:
    CG_DrawPlayerArmorIcon(&rect, qtrue, AR_LEGS);                                 // qtrue
    break;
  case CG_PLAYER_ARMOR_BODY_VALUE:
    CG_DrawPlayerArmorValue(&rect, scale, color, shader, textStyle, AR_BODY);       // 0xA5EA
    break;
  case CG_PLAYER_ARMOR_LEGS_VALUE:
    CG_DrawPlayerArmorValue(&rect, scale, color, shader, textStyle, AR_LEGS);       // 0xA5EA
    break;
  case CG_PLAYER_ARMOR_HEAD_VALUE:
    CG_DrawPlayerArmorValue(&rect, scale, color, shader, textStyle, AR_HEAD);       // 0xA5EA
    break;
	
	
	 case CG_PLAYER_CASH_STOLEN_VALUE:
    CG_DrawPlayerCashStolenValue(&rect, scale, color, shader, textStyle);       // hypov8
    break;
	 case CG_PLAYER_CASH_VALUE:
    CG_DrawPlayerCashValue(&rect, scale, color, shader, textStyle);       // hypov8
    break;
  ...
  case CG_VOICE_NAME:
    ...
    break;
#if 0   // 0xA5EA, removed not needed
  ...
  case CG_LEVEL_TIMER:
    ...
    {
      CG_DrawLevelTimer(&rect, scale, color, shader, textStyle);
    }
    break;
  case CG_WEAPON_TIMER:
      CG_DrawWeaponTimer(&rect, scale, color, shader, textStyle);
      //FIXME 0xA5EA: draw only in hitmen
    break;
  case CG_FRAMES_PER_SECOND:
    if (cg_drawFPS.integer)
      CG_DrawFPS(&rect, scale, color, shader, textStyle);
    break;
  case CG_PLAYER_AMMOROUNDS_VALUE:
    CG_DrawPlayerAmmoRoundsValue(&rect, scale, color, shader, textStyle);
    break;
  case CG_GAMEMODE_INFO:
    CG_DrawGameModeInfo(&rect, scale, color, shader, textStyle);
    break;
  default:
    break;
  }
