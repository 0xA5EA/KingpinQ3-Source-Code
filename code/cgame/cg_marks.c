...
markPoly_t *CG_AllocMark(void)
{
  ...
  if (cg_freeMarkPolys) //add hypov8 'dereferenceing null pointer
  {
  le               = cg_freeMarkPolys;
...
void CG_ImpactMark(qhandle_t markShader, const vec3_t origin, const vec3_t dir, float orientation, float red, float green, float blue, float alpha, qboolean alphaFade, float radius, qboolean temporary)
{
  ...
  {
    Com_DPrintf(S_COLOR_YELLOW"markTotal limit reached\n");
    return;
  }
*/
  ...
  for (i = 0, mf = markFragments; i < numFragments; i++, mf++)
  {
    ...
    markPoly_t *mark;
	//trace_t tr; //add hypov8
	//bspSurface_t * surf;

	//CG_Trace(&tr, start, NULL, NULL, end, skipNum, MASK_SHOT);
	//if ( tr.surfaceFlags & SURF_NOIMPACT)
	//	continue; //hypov8 splatter marks mirrored onto close objects
...
#if 0 // Q3_VM // hypov8 was ifndef (seems to cause errors in weapon decals)
      colors[0] = v->modulate[0];
      colors[1] = v->modulate[1];
      colors[2] = v->modulate[2];
      colors[3] = v->modulate[3];
#else
      *(int *)v->modulate = *(int *)colors;
#endif
    }
    (void)colors; //shutup compiler
    ...
    if (temporary)
    {
      if (markShader)
        ...
        Com_DPrintf(S_COLOR_RED"CG_ImpactMark: markShader = null\n");
...
void CG_AddMarks(void)
{
  ...
  for (; mp != &cg_activeMarkPolys; mp = next)
  {
    ...
    if (mp->markShader)
      ...
      Com_DPrintf(S_COLOR_RED"CG_AddMarks: markShader = null\n");
  }
}
