...
int pcount[256];
//#define PARANOID
//FIXME (0xA5EA):  if enabled game crashes
/*
==============================================================================
...
netField_t playerStateFields[] = { //hypov8 this needs to match playerState_s. 0=float
	...
	{ PSF(eventParms[2]), 8 },
	{ PSF(eventParms[3]), 8 },
	{ PSF(externalEvent), 10 }, //hypov8 todo: ok?
...
//		pcount[i]++;
	...
	{
		if (to->ammo_all[i] != from->ammo_all[i])
		{
			ammobits |= 1 << i;
		}
	}
  ...
  for(i = 0; i < MAX_WEAPONS; i++)
  {
	  if (to->ammo_mag[i] != from->ammo_mag[i])
    {
      roundbits |= 1 << i;
    }
  }
	...
	if (statsbits || persistantbits || ammobits || roundbits || powerupbits)
	{
		...
		if (ammobits)
		{
			...
			for (i = 0; i < MAX_WEAPONS; i++)
				if (ammobits & (1 << i))
					MSG_WriteShort(msg, to->ammo_all[i]);
		}
		else
		{
			MSG_WriteBits(msg, 0, 1); // no change
		...
		{
			MSG_WriteBits(msg, 1, 1); // changed
			MSG_WriteBits(msg, roundbits, MAX_WEAPONS);
			for (i = 0; i < MAX_WEAPONS; i++)
				if (roundbits & (1 << i))
					MSG_WriteShort(msg, to->ammo_mag[i]);
		}
		else
		{
			MSG_WriteBits(msg, 0, 1); // no change
...
void MSG_ReadDeltaPlayerstate(msg_t *msg, playerState_t *from, playerState_t *to)
{
	...
	*to = *from; //memcpy( to, from, playerStateSize );
