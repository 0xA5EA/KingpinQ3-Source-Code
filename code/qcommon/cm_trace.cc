...
void CM_TestInLeaf(traceWork_t * tw, cLeaf_t * leaf)
{
	...
	for(k = 0; k < leaf->numLeafSurfaces; k++)
	{
		...
		//FIXME: 0xA5EA, orginal q3 doesnt have cm.perPolyCollision
		{
...
void CM_TraceThroughLeaf(traceWork_t * tw, cLeaf_t * leaf)
{
	...
	for(k = 0; k < leaf->numLeafSurfaces; k++)
	{
		if (!(cm.surfaces[cm.leafsurfaces[leaf->firstLeafSurface + k]])) //hypov8 dereferencing null pointer
			continue;
		surface = cm.surfaces[cm.leafsurfaces[leaf->firstLeafSurface + k]];
							...
							clipHandle_t model, int brushmask, const vec3_t origin, const vec3_t angles, traceType_t type)
{
	...
	*results = trace;
	...
	vec3_t          offset;
	vec3_t          symetricSize[2];
	matrix_t        ALIGN16(rotation);
	matrix_t ALIGN16(transform);
	matrix_t ALIGN16(inverse);
	int             i;
	float           halfwidth;
	float           halfheight;
	float           t;
