...
cvar_t *Cvar_Get( const char *var_name, const char *var_value, int flags ) {
	...
	if(var)
	{
		...
		if ( !var->resetString[0] ) {
			...
			Z_Free( var->resetString );
			var->resetString = CopyString( var_value );
		} else if ( var_value[0] && qstrcmp( var->resetString, var_value ) ) {
...
void Cvar_Print( cvar_t *v ) 
{
	if (com_dedicated->integer)
		Com_Printf ("\"%s\" is:\"%s\"", v->name, v->string ); //hypov8 stop dedicated console printing color characters
	else
	...
	if ( !( v->flags & CVAR_ROM ) ) 
	{
		...
		{
			if (com_dedicated->integer)
				Com_Printf (" default:\"%s\"",	v->resetString ); //hypov8 stop dedicated console printing color characters
			else
...
//	Com_DPrintf( "Cvar_Set2: %s %s\n", var_name, value );
	...
	if((var->flags & CVAR_LATCH) && var->latchedString)
	{
		...
		if(!qstrcmp(value, var->latchedString))
			return var;
	}
	else if(!qstrcmp(value, var->string))
		return var;
	...
	if (!force)
	{
		...
		if (var->flags & CVAR_LATCH)
		{
			if (var->latchedString)
			{
				if (qstrcmp(value, var->latchedString) == 0)
					return var;
				Z_Free (var->latchedString);
			}
			else
			{
				if (qstrcmp(value, var->string) == 0)
					return var;
			}
...
void Cvar_SetSafe( const char *var_name, const char *value )
{
	...
	if((flags != (int)CVAR_NONEXISTENT) && (flags & CVAR_PROTECTED))
	{
		if( value )
...
void Cvar_SetCheatState(void)
{
	...
	for(var = cvar_vars; var ; var = var->next)
	{
		if(var->flags & CVAR_CHEAT)
		{
			...
			if (qstrcmp(var->resetString,var->string))
...
void Cvar_Toggle_f( void ) {
	...
	for(i = 2; i + 1 < c; i++) {
		if(qstrcmp(curval, Cmd_Argv(i)) == 0) {
...
void Cvar_WriteVariables(fileHandle_t f)
{
	...
	for (var = cvar_vars; var; var = var->next)
	{
		...
		if( var->flags & CVAR_ARCHIVE ) {
			...
			if ( var->latchedString ) {
				if( qstrlen( var->name ) + qstrlen( var->latchedString ) + 10 > sizeof( buffer ) ) {
				...
				if( qstrlen( var->name ) + qstrlen( var->string ) + 10 > sizeof( buffer ) ) {
					Com_Printf( S_COLOR_YELLOW "WARNING: value of variable "
							"\"%s\" too long to write to file\n", var->name );
					continue;
				}
...
void	Cvar_Update( vmCvar_t *vmCvar ) {
	...
	if ( qstrlen(cv->string)+1 > MAX_CVAR_VALUE_STRING )
	  Com_Error( ERR_DROP, "Cvar_Update: src %s length %u exceeds MAX_CVAR_VALUE_STRING", cv->string, (unsigned int)qstrlen(cv->string));
