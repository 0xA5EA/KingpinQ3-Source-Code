...
*/
void VM_Init( void ) {
	Cvar_Get( "vm_cgame", "0", CVAR_ARCHIVE );	// !@# SHIP WITH SET TO 2 //hypov8 todo: reset default calls this
...
vmHeader_t *VM_LoadQVM( vm_t *vm, qboolean alloc, qboolean unpure)
{
	...
	if(header.h->vmMagic == VM_MAGIC_VER2)
	{
		...
		if(alloc)
		{
			vm->jumpTableTargets = static_cast<byte*>(Hunk_Alloc(header.h->jtrgLength, h_high));
		}
		else
		{
...
vm_t *VM_Create( const char *module, intptr_t (*systemCalls)(intptr_t *),
	...
	vm->instructionPointers =  static_cast<intptr_t*>(Hunk_Alloc(vm->instructionCount * sizeof(*vm->instructionPointers), h_high));
...
intptr_t	QDECL VM_Call( vm_t *vm, int callnum, ... ) {
	...
	if ( vm->entryPoint ) {
		...
		va_list ap;
		va_start(ap, callnum);
		for (i = 0; i < (int)ARRAY_LEN(args); i++) {
		...
		a.callnum = callnum;
		va_start(ap, callnum);
		for (i = 0; i < (int)ARRAY_LEN(a.args); i++) {
