...
static void dst_insn_append(struct func_info * const fp)
{
	...
	if (insns_size)
		Com_Memcpy(&dp->code[0], fp->insn_buf, insns_size);
...
static void blockcopy(unsigned int dest, unsigned int src, unsigned int count)
{
	...
	Com_Memcpy(currentVM->dataBase+dest, currentVM->dataBase+src, count);
}
...
static void sparc_compute_code(vm_t *vm, struct func_info * const fp)
{
	...
	while (dp) {
		...
		if (!dp->jump) {
			Com_Memcpy(code_now, &dp->code[0], dp->length * sizeof(unsigned int));
			code_now += dp->length;
		} else {
			int i;
...
void VM_Compile(vm_t *vm, vmHeader_t *header)
{
	...
	Com_Memset(fi.dst_by_i_count, 0, header->instructionCount * sizeof(void *));
...
int VM_CallCompiled(vm_t *vm, int *args)
{
	...
	Com_Memcpy( argPointer, args, 4 * MAX_VMMAIN_ARGS );
	argPointer[-1] = 0;
	argPointer[-2] = -1;
