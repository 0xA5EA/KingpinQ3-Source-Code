...
//	Netchan_UnScramblePacket( msg );
	...
	if(fragmented)
	{
		...
		if(fragmentLength < 0 || (unsigned int)msg->readcount + fragmentLength > (unsigned int)msg->cursize ||
...
static void NET_QueuePacket( int length, const void *data, netadr_t to,	int offset )
{
	...
	neww = static_cast<packetQueue_t*>(S_Malloc(sizeof(packetQueue_t)));
	neww->data = (byte*)S_Malloc(length);
	Com_Memcpy(neww->data, data, length);
	neww->length  = length;
	neww->to      = to;
	neww->release = Sys_Milliseconds() + offset;
	neww->next    = NULL;
...
void QDECL NET_OutOfBandPrint(netsrc_t sock, netadr_t adr, const char *format, ...)
{
	...
	// send the datagram
	NET_SendPacket(sock, qstrlen(string), string, adr);
}
