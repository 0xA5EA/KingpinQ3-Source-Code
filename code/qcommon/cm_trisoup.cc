...
==================
*/
static int CM_FindPlane(vec_t *p1, vec_t *p2, vec_t *p3)
{
	float plane[4];
	int i;
//	float d;
...
static int CM_GenerateBoundaryForPoints(const vec4_t triPlane, const vec3_t p1, const vec3_t p2)
{
	...
	return CM_FindPlane((float*)p1, (float*)p2, (float*)up);
}
			...
			for(dir = -1; dir <= 1; dir += 2)
			{
				...
				if(i == facet->numBorders)
				{
					...
					if(!w2)
					{
						...
						Com_DPrintf("WARNING: Invalid tri %f %f %f %f\n", w->p[0][0], w->p[0][1], w->p[0][2], w->p[0][3]);
						continue;
					}
					else
					{
