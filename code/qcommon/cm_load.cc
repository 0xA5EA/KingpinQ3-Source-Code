...
void CMod_LoadSurfaces(lump_t * surfs, lump_t * verts, lump_t * indexesLump)
{
	...
	if(surfs->filelen % sizeof(*in))
		Com_Error(ERR_DROP, "CMod_LoadSurfaces: dsurface_t funny lump size");
	...
	if(verts->filelen % sizeof(*dv))
		Com_Error(ERR_DROP, "CMod_LoadSurfaces: drawVert_t funny lump size");

	index = (int *)(cmod_base + indexesLump->fileofs);
	if(indexesLump->filelen % sizeof(*index))
		Com_Error(ERR_DROP, "CMod_LoadSurfaces: int funny lump size");
...
void CM_LoadMap(const char *name, qboolean clientload, int *checksum)
{
	...
	//#define MAP_LOAD_TIME //hypov8 display loaded times (enabled in my debug project )
#ifdef MAP_LOAD_TIME
	int time1, time2;
#endif
	...
	if(!qstrcmp(cm.name, name) && clientload)
	{
	...
	*checksum = last_checksum;
	...
	{
		Com_Error(ERR_DROP, "CM_LoadMap: %s has wrong version number (%i should be %i)", name, header.version, BSP_VERSION_KPQ3);
	}
    ...
    Com_Printf("CMod_LoadShaders time = %d\n", time2 - time1 );
    time1 = Com_Milliseconds();
#endif
	CMod_LoadLeafs(&header.lumps[LUMP_LEAFS]);
	#ifdef MAP_LOAD_TIME
    time2 = Com_Milliseconds();
    Com_Printf("CMod_LoadLeafs time = %d\n", time2 - time1 );
    time1 = Com_Milliseconds();
#endif
	CMod_LoadLeafBrushes(&header.lumps[LUMP_LEAFBRUSHES]);
#ifdef MAP_LOAD_TIME
    time2 = Com_Milliseconds();
    Com_Printf("CMod_LoadLeafBrushes time = %d\n", time2 - time1 );
    time1 = Com_Milliseconds();
#endif
	CMod_LoadLeafSurfaces(&header.lumps[LUMP_LEAFSURFACES]);
	#ifdef MAP_LOAD_TIME
    time2 = Com_Milliseconds();
    Com_Printf("CMod_LoadLeafSurfaces time = %d\n", time2 - time1 );
    time1 = Com_Milliseconds();
#endif
	CMod_LoadPlanes(&header.lumps[LUMP_PLANES]);
#ifdef MAP_LOAD_TIME
    time2 = Com_Milliseconds();
    Com_Printf("CMod_LoadPlanes time = %d\n", time2 - time1 );
    time1 = Com_Milliseconds();
#endif
	CMod_LoadBrushSides(&header.lumps[LUMP_BRUSHSIDES]);
#ifdef MAP_LOAD_TIME
    time2 = Com_Milliseconds();
    Com_Printf("CMod_LoadBrushSides time = %d\n", time2 - time1 );
    time1 = Com_Milliseconds();
#endif
	CMod_LoadBrushes(&header.lumps[LUMP_BRUSHES]);
	#ifdef MAP_LOAD_TIME
    time2 = Com_Milliseconds();
    Com_Printf("CMod_LoadBrushes time = %d\n", time2 - time1 );
    time1 = Com_Milliseconds();
#endif
	CMod_LoadSubmodels(&header.lumps[LUMP_MODELS]);
#ifdef MAP_LOAD_TIME
    time2 = Com_Milliseconds();
    Com_Printf("CMod_LoadSubmodels time = %d\n", time2 - time1 );
    time1 = Com_Milliseconds();
#endif
	CMod_LoadNodes(&header.lumps[LUMP_NODES]);
#ifdef MAP_LOAD_TIME
    time2 = Com_Milliseconds();
    Com_Printf("CMod_LoadNodes time = %d\n", time2 - time1 );
    time1 = Com_Milliseconds();
#endif
	CMod_LoadEntityString(&header.lumps[LUMP_ENTITIES]);
#ifdef MAP_LOAD_TIME
    time2 = Com_Milliseconds();
    Com_Printf("CMod_LoadEntityString time = %d\n", time2 - time1 );
    time1 = Com_Milliseconds();
#endif
	CMod_LoadVisibility(&header.lumps[LUMP_VISIBILITY]);
#ifdef MAP_LOAD_TIME
    time2 = Com_Milliseconds();
    Com_Printf("CMod_LoadVisibility time = %d\n", time2 - time1 );
    time1 = Com_Milliseconds();
#endif
	CMod_LoadSurfaces(&header.lumps[LUMP_SURFACES], &header.lumps[LUMP_DRAWVERTS], &header.lumps[LUMP_DRAWINDEXES]);
#ifdef MAP_LOAD_TIME
    time2 = Com_Milliseconds();
    Com_Printf("CMod_LoadSurfaces time = %d\n", time2 - time1 );
    time1 = Com_Milliseconds();
#endif
	CMod_CreateBrushSideWindings();
#ifdef MAP_LOAD_TIME
    time2 = Com_Milliseconds();
    Com_Printf("CMod_CreateBrushSideWindings time = %d\n", time2 - time1 );
#endif
