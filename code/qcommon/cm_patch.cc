...
{
  //FIXME(0xA5EA): tons of errors when loading map here
  int i, j, k, l;
  ...
  for (j = 0; j < w->numpoints; j++)
  {
    ...
    if (VectorNormalize3(vec) < 0.5f)
      continue;
    CM_SnapVector(vec);
    for (k = 0; k < 3; k++)
    {
      if (vec[k] == -1.0f || vec[k] == 1.0f
        || (vec[k] == 0.0f && vec[(k + 1) % 3] == 0.0f)) //et
      {
        break;// axial
      }
    }
    ...
    for (axis = 0; axis < 3; axis++)
    {
      for (dir = -1; dir <= 1; dir += 2)
      {
        ...
        if (VectorNormalize3(plane) < 0.5f)
          continue;
...
#ifdef BSPC //hypov8. re'added from etxreal
...
====================
*/
void CM_TracePointThroughPatchCollide(traceWork_t * tw, const cSurfaceCollide_t *pc)
{
...
====================
*/
qboolean CM_PositionTestInPatchCollide(traceWork_t * tw, const cSurfaceCollide_t *pc)
{
  int             i, j;
  float           offset, t;
  cPlane_t   *planes;
