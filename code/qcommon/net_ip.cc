...
//EAGAIN //hypov8 mingw nag
#  define EAGAIN2 WSAEWOULDBLOCK
//#endif
...
#ifdef WINVER
	  typedef char* casttype;
#else
    typedef void* casttype;
#endif
    if(addr.ss_family == AF_INET)
		  ret = sendto(ip_socket, (casttype)data, length, 0, (struct sockaddr *)&addr, sizeof(struct sockaddr_in));
    else if(addr.ss_family == AF_INET6)
      ret = sendto(ip6_socket, (casttype)data, length, 0, (struct sockaddr *)&addr, sizeof(struct sockaddr_in6));
	}
	if( ret == SOCKET_ERROR ) {
		int err = socketError;
...
void NET_OpenSocks(int port)
{
    ...
    typedef void* casttype;
#endif
	if ( send( socks_socket, (casttype)buf, len, 0 ) == SOCKET_ERROR ) {
		Com_Printf("NET_OpenSocks: send: %s\n", NET_ErrorString());
		return;
	}

	// get the response
	len = recv(socks_socket, (casttype)buf, 64, 0);
	if ( len == SOCKET_ERROR ) {
	...
	if(buf[1] == 2)
	{
		...
		// build the request
		ulen = qstrlen(net_socksUsername->string);
		plen = qstrlen(net_socksPassword->string);
		...
		// send it
		if ( send( socks_socket, (casttype)buf, 3 + ulen + plen, 0 ) == SOCKET_ERROR ) {
			Com_Printf("NET_OpenSocks: send: %s\n", NET_ErrorString());
			return;
		}
	...
	*(short *)&buf[8] = htons((short)port);      // port
	if ( send( socks_socket, (casttype)buf, 10, 0 ) == SOCKET_ERROR ) {
		Com_Printf("NET_OpenSocks: send: %s\n", NET_ErrorString());
		return;
	}
