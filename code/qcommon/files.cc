 ...
 * name:		files.c
 *
 * desc:		handle based filesystem for KingpinQ3
 *
...
qboolean FS_CreatePath (char *OSPath) {
	...
	// FIXME: is c: allowed???
	if ( Q_strstr( OSPath, ".." ) || Q_strstr( OSPath, "::" ) ) {
...
void FS_SV_Rename( const char *from, const char *to, qboolean safe )
{
	...
	from_ospath[qstrlen(from_ospath) - 1] = '\0';
	to_ospath[qstrlen(to_ospath) - 1]     = '\0';
...
long FS_FOpenFileReadDir(const char *filename, searchpath_t *search, fileHandle_t *file, qboolean uniqueFILE, qboolean unpure)
{
  ...
  if(Q_strstr(filename, ".." ) || Q_strstr(filename, "::"))
  {
    if(file == NULL)
      return qfalse;
    *file = 0;
  ...
  if(com_fullyInitialized && Q_strstr(filename, "q3key"))
  {
    if(file == NULL)
      return qfalse;
    *file = 0;
...
=================
*/
int FS_Write(const void *buffer, size_t len, fileHandle_t h) {
	int block, remaining;
	int written;
	byte *buf;
	int tries;
...
long FS_ReadFileDir(const char *qpath, void *searchPath, qboolean unpure, void **buffer)
{
	...
	search = static_cast<searchpath_t*>(searchPath);
	if(search == NULL)
	{
...
static pack_t *FS_LoadZipFile(const char *zipfile, const char *basename)
{
	...
	for(i = 0; i < (int)gi.number_entry; i++)
	{
		...
		qstrcpy(buildBuffer[i].name, filename_inzip);
		namePtr += qstrlen(filename_inzip) + 1;
...
int	FS_GetModList( char *listbuf, int bufsize ) {
	...
	*listbuf = 0;
	...
	for ( i = 0 ; i < nPotential ; i++ ) {
		...
		// we drop "basekpq3" "." and ".." and "radiant"
		if(Q_stricmp(name, com_basegame->string) && Q_stricmpn(name, ".", 1) && Q_stricmp(name, "radiant"))
		{
...
{
	if(Q_strstr(checkdir, "../") || Q_strstr(checkdir, "..\\"))
		return qtrue;
	...
	for(i = 0; i < fs_numServerReferencedPaks; i++)
	{
		...
		if ( !havepak && fs_serverReferencedPakNames[i] && *fs_serverReferencedPakNames[i] ) {
			...
			if(dlstring)
			{
				...
				if((int)strlen(origpos) + (origpos - neededpaks) >= len - 1)
				{
					*origpos = '\0';
					break;
				}
...
static void FS_CheckPak0(void)
{
...

	{
		...
		Q_strcat(errorText, sizeof(errorText),
			va("Also check that your kpq3 executable is in "
