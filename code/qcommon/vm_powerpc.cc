	...
	)
{
	...
	if ( iBytes > 0 )
		Com_Memcpy( &(di_now->code[0]), is, iBytes );
...
PPC_ComputeCode( vm_t *vm )
{
	...
	while ( (di_now = di_now->next ) ) {
		...
		if ( di_now->jump == NULL ) {
			Com_Memcpy( codeNow, &(di_now->code[0]), di_now->length * sizeof( ppc_instruction_t ) );
...
VM_Compile( vm_t *vm, vmHeader_t *header )
{
	...
	Com_Memset( di_pointers, 0, header->instructionCount * sizeof( void * ) );
