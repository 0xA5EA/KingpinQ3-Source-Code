...
int EmitCallDoSyscall(vm_t *vm)
{
  ...
  EmitPtr(reinterpret_cast<void*>(&DoSyscall));
...
void VM_Compile(vm_t *vm, vmHeader_t *header)
{
  ...
  buf = static_cast<byte*>(Z_Malloc(maxLength));
  jused = static_cast<byte*>(Z_Malloc(jusedSize));
  code = static_cast<byte*>(Z_Malloc(header->codeLength+32));
...
#ifdef VM_X86_MMAP
  vm->codeBase = static_cast<byte*>(mmap(NULL, compiledOfs, PROT_WRITE, MAP_SHARED|MAP_ANONYMOUS, -1, 0));
  if(vm->codeBase == MAP_FAILED)
    Com_Error(ERR_FATAL, "VM_CompileX86: can't mmap memory");
#elif _WIN32
  // allocate memory with EXECUTE permissions under windows.
  vm->codeBase =  static_cast<byte*>(VirtualAlloc(NULL, compiledOfs, MEM_COMMIT, PAGE_EXECUTE_READWRITE));
  if(!vm->codeBase)
...
int VM_CallCompiled(vm_t *vm, int *args)
{
  ...
  *opStack = 0xDEADBEEF;
  ...
  if(opStackOfs != int(1) || *opStack != int(0xDEADBEEF))
  {
