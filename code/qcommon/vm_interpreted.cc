...
void VM_PrepareInterpreter( vm_t *vm, vmHeader_t *header ) {
	...
	vm->codeBase = static_cast<byte*>(Hunk_Alloc( vm->codeLength*4, h_high ));			// we're now int aligned
...
//				vm->callLevel--;
			...
			if ( programCounter == -1 ) {
				goto done;
			} else if ( programCounter >= vm->codeLength ) {
	...
	if (opStackOfs != (uint8_t)1 || *opStack != (int)0xDEADBEEF)
