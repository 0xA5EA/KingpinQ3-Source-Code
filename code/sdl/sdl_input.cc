...
static const char *IN_TranslateSDLToQ3Key( SDL_keysym *keysym, keyNum_t *key, qboolean down )
{
	...
	*key = (keyNum_t)0;
	...
	if( down && qstrlen( Key_KeynumToString( *key ) ) == 1 &&
		keysym->unicode == 0 )
	{
		if( in_keyboardDebug->integer )
...
static void IN_JoyMove( void )
{
	...
	Com_Memset(joy_pressed, '\0', sizeof (joy_pressed));
	...
	if (total > 0)
	{
		...
		if (balldx || balldy)
		{
			...
			if (abs(balldy) > 1)
				balldy *= 2;
			Com_QueueEvent( 0, SET_MOUSE, balldx, balldy, 0, NULL );
		}
	}
	...
	if (total > 0)
	{
		if (total > (int)ARRAYLEN(stick_state.buttons))
		...
		for (i = 0; i < total; i++)
		{
			...
			{
				Com_QueueEvent( 0, SET_KEY, K_JOY1 + i, pressed, 0, NULL );
		...
		for( i = 0; i < 4; i++ ) {
			if( ((Uint8 *)&hats)[i] != ((Uint8 *)&stick_state.oldhats)[i] ) {
				...
				switch( ((Uint8 *)&stick_state.oldhats)[i] ) {
					case SDL_HAT_UP:
						Com_QueueEvent( 0, SET_KEY, hat_keys[4*i + 0], qfalse, 0, NULL );
						break;
					case SDL_HAT_RIGHT:
						Com_QueueEvent( 0, SET_KEY, hat_keys[4*i + 1], qfalse, 0, NULL );
						break;
					case SDL_HAT_DOWN:
						Com_QueueEvent( 0, SET_KEY, hat_keys[4*i + 2], qfalse, 0, NULL );
						break;
					case SDL_HAT_LEFT:
						Com_QueueEvent( 0, SET_KEY, hat_keys[4*i + 3], qfalse, 0, NULL );
						break;
				...
				switch( ((Uint8 *)&hats)[i] ) {
					case SDL_HAT_UP:
						Com_QueueEvent( 0, SET_KEY, hat_keys[4*i + 0], qtrue, 0, NULL );
						break;
					case SDL_HAT_RIGHT:
						Com_QueueEvent( 0, SET_KEY, hat_keys[4*i + 1], qtrue, 0, NULL );
						break;
					case SDL_HAT_DOWN:
						Com_QueueEvent( 0, SET_KEY, hat_keys[4*i + 2], qtrue, 0, NULL );
						break;
					case SDL_HAT_LEFT:
						Com_QueueEvent( 0, SET_KEY, hat_keys[4*i + 3], qtrue, 0, NULL );
						break;
					case SDL_HAT_RIGHTUP:
						Com_QueueEvent( 0, SET_KEY, hat_keys[4*i + 0], qtrue, 0, NULL );
						Com_QueueEvent( 0, SET_KEY, hat_keys[4*i + 1], qtrue, 0, NULL );
	...
	if (total > 0)
	{
		...
		for (i = 0; i < total; i++)
		{
			...
			if (in_joystickUseAnalog->integer)
			{
				...
				{
					Com_QueueEvent( 0, SET_JOYSTICK_AXIS, i, axis, 0, NULL );
	...
	if (axes != stick_state.oldaxes)
	{
		for( i = 0; i < 16; i++ ) {
			if( ( axes & ( 1 << i ) ) && !( stick_state.oldaxes & ( 1 << i ) ) ) {
				Com_QueueEvent( 0, SET_KEY, joy_keys[i], qtrue, 0, NULL );
			}

			if( !( axes & ( 1 << i ) ) && ( stick_state.oldaxes & ( 1 << i ) ) ) {
				Com_QueueEvent( 0, SET_KEY, joy_keys[i], qfalse, 0, NULL );
			}
		}
	}
	...
	while( SDL_PollEvent( &e ) )
	{
		switch( e.type )
		{
			case SDL_KEYDOWN:
				...
				if( key )
					Com_QueueEvent( 0, SET_KEY, key, qtrue, 0, NULL );

				if( character )
					Com_QueueEvent( 0, SET_CHAR, *character, 0, 0, NULL );
				break;
			...
			case SDL_KEYUP:
				...
				if( key )
					Com_QueueEvent( 0, SET_KEY, key, qfalse, 0, NULL );
				break;

			case SDL_MOUSEMOTION:
				if( mouseActive )
					Com_QueueEvent( 0, SET_MOUSE, e.motion.xrel, e.motion.yrel, 0, NULL );
				break;
