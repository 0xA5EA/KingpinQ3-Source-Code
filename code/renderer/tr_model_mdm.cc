...
static void AddSurfaceToVBOSurfacesListMDM(growList_t * vboSurfaces, growList_t * vboTriangles, mdmModel_t * mdm, mdmSurfaceIntern_t * surf, int skinIndex, int numBoneReferences, int boneReferences[MAX_BONES])
{
	...
	// create surface
	vboSurf = static_cast<srfVBOMDMMesh_t*>(ri.Hunk_Alloc(sizeof(*vboSurf), h_low));
	...
	data = static_cast<byte*>(ri.Hunk_AllocateTempMemory(dataSize));
	dataOfs = 0;
	...
	do
	{
		...
		if(renderCount == surf->numVerts)
		{
			...
			indexes = static_cast<byte*>(ri.Hunk_AllocateTempMemory(indexesSize));
			indexesOfs = 0;
			for(j = 0; j < vboTriangles->currentElements; j++)
			{
				tri = static_cast<skelTriangle_t*>(Com_GrowListElement(vboTriangles, j));
				for(k = 0; k < 3; k++)
				{
					index = tri->indexes[k];
...
//    mdmFrame_t            *frame;
	...
	mdmModel = mod->mdm = static_cast<mdmModel_t*>(ri.Hunk_Alloc(sizeof(mdmModel_t), h_low));
...
/*	mdm->skel = RE_RegisterModel(mdm->bonesfile);
	...
	mdmModel->tags = tag = static_cast<mdmTagIntern_t*>(ri.Hunk_Alloc(sizeof(*tag) * mdm->numTags, h_low));
	...
	for(i = 0; i < mdm->numTags; i++, tag++)
	{
		...
		tag->boneReferences = static_cast<int*>(ri.Hunk_Alloc(sizeof(*bonerefOut) * mdmTag->numBoneReferences, h_low));
	...
	mdmModel->surfaces = static_cast<mdmSurfaceIntern_t*>(ri.Hunk_Alloc(sizeof(*surf) * mdmModel->numSurfaces, h_low));
	...
	for(i = 0, surf = mdmModel->surfaces; i < mdm->numSurfaces; i++, surf++)
	{
		...
		surf->triangles = static_cast<srfTriangle_t*>(ri.Hunk_Alloc(sizeof(*tri) * surf->numTriangles, h_low));
		...
		surf->verts = static_cast<md5Vertex_t*>(ri.Hunk_Alloc(sizeof(*v) * surf->numVerts, h_low));
		...
		for(j = 0, v = surf->verts; j < mdmSurf->numVerts; j++, v++)
		{
			...
			v->weights = static_cast<md5Weight_t**>(ri.Hunk_Alloc(sizeof(*v->weights) * v->numWeights, h_low));
			for(k = 0; k < v->numWeights; k++)
			{
				md5Weight_t *weight = static_cast<md5Weight_t*>(ri.Hunk_Alloc(sizeof(*weight), h_low));
		...
		surf->collapseMap = static_cast<int*>(ri.Hunk_Alloc(sizeof(*collapseMapOut) * mdmSurf->numVerts, h_low));
		...
		surf->boneReferences = static_cast<int*>(ri.Hunk_Alloc(sizeof(*bonerefOut) * mdmSurf->numBoneReferences, h_low));
	...
	// split the surfaces into VBO surfaces by the maximum number of GPU vertex skinning bones
	{
		...
		for(i = 0, surf = mdmModel->surfaces; i < mdmModel->numSurfaces; i++, surf++)
		{
			...
			for(j = 0, tri = surf->triangles; j < surf->numTriangles; j++, tri++)
			{
				skelTriangle_t *sortTri = static_cast<skelTriangle_t*>(Com_Allocate(sizeof(*sortTri)));
			...
			while(numRemaining)
			{
				...
				for(j = 0; j < sortedTriangles.currentElements; j++)
				{
					skelTriangle_t *sortTri = static_cast<skelTriangle_t*>(Com_GrowListElement(&sortedTriangles, j));
		...
		mdmModel->vboSurfaces = static_cast<srfVBOMDMMesh_t**>(ri.Hunk_Alloc(mdmModel->numVBOSurfaces * sizeof(*mdmModel->vboSurfaces), h_low));
