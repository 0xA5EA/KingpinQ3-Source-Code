...
static ID_INLINE void *IQMPtr( iqmHeader_t *header, int offset ) {
           ...
           size_t *len_names )
{
  ...
  LL( header->filesize );
  if( header->filesize > (unsigned int)filesize || header->filesize > 1<<24 ) {
...
qboolean R_LoadIQModel( model_t *mod, void *buffer, int filesize,
  ...
  anim = ( iqmAnim_t* )IQMPtr( header, header->ofs_anims );
  for( i = 0; (int)i < IQModel->num_anims; i++, IQAnim++, anim++ ) {
  ...
  if( r_vboModels->integer && glConfig2.vboVertexSkinningAvailable
    ...
    if( IQModel->colors ) {
      ...
      for( i = 0; (int)i < IQModel->num_vertexes; i++ ) {
