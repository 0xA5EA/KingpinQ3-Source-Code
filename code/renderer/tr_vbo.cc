...
static qboolean R_AttributeTightlyPacked( const VBO_t *vbo, uint32_t attribute )
{
  ...
  return layout->numComponents * ((int)R_GetSizeForType( layout->componentType ) == layout->realStride);
}
...
static vboData_t R_CreateVBOData( const VBO_t *vbo, const srfVert_t *verts )
{
  ...
  for ( v = 0; v < vbo->vertexesNum; v++ )
  {
    ...
    if ( ( vbo->attribBits & ATTR_LIGHTDIRECTION ) )
    {
      if ( !data.lightDir )
      {
        data.lightDir = ( vec3_t * ) ri.Hunk_AllocateTempMemory( sizeof( *data.lightDir ) * data.numVerts );
      }
      VectorCopy( vert->lightDirection, data.lightDir[ v ] );
    }
#endif
  }
