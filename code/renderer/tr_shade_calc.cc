...
static float GetOpValue( const expOperation_t *op )
{
  ...
  switch ( op->type )
  {
    ...
    case OP_PARM8: // hypov8 change shaders based on reflectionmapping enabled.
      ...
      if (r_reflectionMapping->integer == 1)
      {
        value = 1.0;
      }
      else
    ...
    case OP_PARM11:
      value = 0.0;
      break; //hypov8 disable TDM Frob Highlight Stage. makes texture brighter and washes out bumpmaps
    case OP_GLOBAL0:
    case OP_GLOBAL1:
    case OP_GLOBAL2:
    case OP_GLOBAL3:
...
void RB_CalcDeformVertexes( deformStage_t *ds )
{
  ...
  if ( ds->deformationWave.frequency < 0 )
  {
    ...
    for ( i = 0; i < (int)tess.numVertexes; i++, xyz += 4, normal += 4 )
    {
...
void RB_CalcBulgeVertexes( deformStage_t *ds )
{
  ...
  for ( i = 0; i < (int)tess.numVertexes; i++, xyz += 4, st += 4, normal += 4 )
  {
    int   off;
    float scale;
...
void RB_CalcMoveVertexes( deformStage_t *ds )
{
  ...
  for ( i = 0; i < (int)tess.numVertexes; i++, xyz += 4 )
  {
...
static void Autosprite2Deform( void )
{
  ...
  for ( i = 0, indexes = 0; i < (int)tess.numVertexes; i += 4, indexes += 6 )
  {
    float  lengths[ 2 ];
    int    nums[ 2 ];
    vec3_t mid[ 2 ];
