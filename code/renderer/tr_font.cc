...
==================
*/
static void WritePNG(const char *name, const byte *pic, int width, int height)
{
	png_structp png;
	png_infop info;
	int i;
...
void RE_RegisterFont(const char *fontName, int pointSize, fontInfo_t *font)
{
	...
	Com_sprintf(fontFileName, sizeof(fontFileName), "fonts/%s_%i.dat", fontName, pointSize);
#else
	...
	len = ri.FS_ReadFile(fontFileName, NULL);
	if (len == sizeof(fontInfo_t))
	{
		ri.FS_ReadFile(fontFileName, &faceData);
		fdOffset = 0;
		...
		Q_strncpyz(font->name, fontFileName, sizeof(font->name));
		for (i = GLYPH_START; i < GLYPH_END; i++)
		{
			font->glyphs[i].glyph = RE_RegisterShader(font->glyphs[i].shaderName, RSF_NOMIP);
		}
...
//	Com_sprintf(fileName, sizeof(fileName), "%s.ttf", strippedName); // so machts xreal,
//	len = ri.FS_ReadFile(fileName, &faceData);
...
#ifdef SAVE_FONTS_AS_TGA
			Com_sprintf(fontFileName, sizeof(fontFileName), "%s_%i_%i.tga", strippedName, imageNumber++, pointSize);
      if (!ri.FS_FileExists(fontFileName))
			{
				WriteTGA(fontFileName, imageBuff, FONT_SIZE, FONT_SIZE);
			}
#else
			Com_sprintf(fontFileName, sizeof(fontFileName), "%s_%i_%i.png", strippedName, imageNumber++, pointSize);
			if (!ri.FS_FileExists(fontFileName))
			{
				WritePNG(fontFileName, imageBuff, FONT_SIZE, FONT_SIZE);
			}
#endif
			image = R_CreateImage(fontFileName, imageBuff, FONT_SIZE, FONT_SIZE, IF_NOPICMIP, FT_LINEAR, WT_CLAMP);
			h  = RE_RegisterShaderFromImage(fontFileName, image);

			for(j = lastStart; j < i; j++)
			{
				font->glyphs[j].glyph = h;
				Q_strncpyz(font->glyphs[j].shaderName, fontFileName, sizeof(font->glyphs[j].shaderName));
			}
			lastStart = i;
			Com_Memset(out, 0, 1024 * 1024);
			xOut = 0;
	...
	if (!ri.FS_FileExists(fontFileName))
		ri.FS_WriteFile(fontFileName, font, sizeof(fontInfo_t));
