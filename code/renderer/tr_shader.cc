...
#if 1 //add hypov8 new default images
  else if (!Q_stricmp(token, "_grey") || !Q_stricmp(token, "$grey")|| // add hypov8
       !Q_stricmp(token, "_gray") || !Q_stricmp(token, "$gray")) // add hypov8
  {
    stage->bundle[0].image[0] = tr.greyImage;
    return qtrue;
  }

  else if (!Q_stricmp(token, "_red") || !Q_stricmp(token, "$red")) // add hypov8
  {
    stage->bundle[0].image[0] = tr.redImage;
    return qtrue;
  }
  else if (!Q_stricmp(token, "_green") || !Q_stricmp(token, "$green")) // add hypov8
  {
...
static qboolean LoadCubeMap( shaderStage_t *stage, char *buffer )
{
  char         *token;
  int          imageBits = 0;
  wrapType_t   wrapType;
//	qboolean   uncompressed;
  ...
  if ( !buffer || !buffer[ 0 ] )
  {
    ri.Printf( PRINT_WARNING, "WARNING: NULL parameter for LoadCubeMap in shader '%s'\n", shader.name );
    return qfalse;
  }
...
//  ri.Printf(PRINT_ALL, "LoadMap: buffer '%s'\n", buffer);
  ...
  if (!Q_stricmp(token, "_quadratic")) // add hypov8
  {
    stage->bundle[0].image[0] = tr.quadraticImage;
    return qtrue;
  }
...
#if 0 // removes compiler warning unused function
static qboolean LoadCubeBlendedMap( shaderStage_t *stage, char *buffer )
{
  char         *token;
  int          imageBits = 0;
  wrapType_t   wrapType;
...
static qboolean ParseStage( shaderStage_t *stage, char **text )
{
  ...
  while ( 1 )
  {
    ...
    else if ( !Q_stricmp( token, "cubeMap" ) || !Q_stricmp( token, "cameraCubeMap" ) )
    {
      ...
      //add hypov8 latest nvidia driver will crash on sky with texture filtering
      stage->bundle[0].image[0] = R_FindCubeImage(token, imageBits, FT_LINEAR, WT_EDGE_CLAMP, shader.name);
  ...
  if ( stage->rgbGen == CGEN_BAD )
  {
    ...
    {
      stage->rgbGen = ReturnRGBGenModes(); //CGEN_IDENTITY;
    }
  }
...
static void ParseSort( char **text )
{
  ...
  else if ( !Q_stricmp( token, "skyNoFog" ) )//hypov8 add
  {
  ...
  else if (!Q_stricmp(token, "fog")) // hypov8 add
  {
    shader.sort = SS_FOG;
  }
  ...
  else if (!Q_stricmp(token, "water")) // add hypov8
  {
...
static const infoParm_t infoParms[] =
{
  ...
  {"missileclip",			1,							0,						CONTENTS_MISSILECLIP					},	// impact only specific weapons (rl, gl)
  ...
  { "trans",              0,                         0,                      CONTENTS_TRANSLUCENT                   }, // don't eat contained surfaces  //hypov8 does not bock light or vis
  { "translucent",        0,                         0,                      CONTENTS_TRANSLUCENT                   }, // don't eat contained surfaces //hypov8 does not bock light or vis
  { "detail",             0,                         0,                      CONTENTS_DETAIL                        }, // don't include in structural bsp  //hypov8 no vis information
  ...
  { "trigger",            0,                         CONTENTS_TRIGGER,       0                                      },
  { "flashlight_trigger", 0,                         0,                      0                                      },
  { "aassolid",           0,                         0,                      0                                      },
  { "aasobstacle",        0,                         0,                      0                                      },
  { "nullNormal",         0,                         0,                      0                                      },
  { "discrete",           0,                         0,                      CONTENTS_DETAIL                        },

  //hypov8 missing surface flags.. 'surfaceparm' not required in shaders
  { "lightgrid",			0, 0, 0 },
  { "antiportal",			0, 0, 0 },
  { "skip",				0, 0, 0 },
  ...
  { "metallightsteps",	0, SURF_METALLIGHT, 0 },
  { "rugsteps",			0, SURF_RUG,	0 },
  { "snowsteps",			0, SURF_SNOW,	0 },
  { "woodsteps",			0, SURF_WOOD,	0 },
  { "icesteps",			0, SURF_ICE,	0 }, /*enabled hypov8*/
  { "grassteps",			0, SURF_GRASS,	0 },
  { "marblesteps",		0, SURF_MARBLE, 0 },
  { "metaltinsteps",		0, SURF_TIN,	0 },
  { "glassteps",			0, SURF_GLASS,	0 },
...
static void ParseColorMap(shaderStage_t *stage, char **text)
{
  char buffer[1024] = "";
...
static void ParseReflectionMap( shaderStage_t *stage, char **text )
{
  ...
  if ( ParseMap( stage, text, buffer, sizeof( buffer ) ) )
  {
    LoadCubeMap(stage, buffer);
  }
}

// ReflectionMapBlended <cubmap>
// ReflectionMapBlended _autocube
...
// ReflectionMapAdd <cubmap>
// ReflectionMapAdd _autocube
static ID_INLINE void ParseReflectionMapAdd(shaderStage_t * stage, char **text)
{
  char buffer[1024] = "";
  stage->active = qtrue;
  stage->type = ST_REFLECTIONMAP; //hypov8 todo: should set another varable. if _cubemap. build cubemaps
...
static qboolean ParseShader( char *_text )
{
  ...
  while ( 1 )
  {
...
#if 0 //hypov8
    //add hypov8: smooth vertex normals across uv borders
    else if (!Q_stricmp(token, "smooth_normal"))
    {
      shader.smoothNormal = qtrue;
      continue;
    }
#endif
        ...
        !Q_stricmp(token, "xmap_sunExt") ||	!Q_stricmp(token, "kmap_sunExt")  ||!Q_stricmp(token, "q3map_sunExt")) //add hypov8
    {
      ...
      if (!token[0])
      {
        ri.Printf(PRINT_WARNING, "WARNING: missing parm for 'kmap_sun' keyword in shader '%s'\n", shader.name);
        continue;
      }
      ...
      SkipRestOfLine( text ); //hypov8 add: bsp uses more inputs.
      continue;
    }
    // noShadows
    ...
    else if (!Q_stricmpn(token, "kmap", 4)  || !Q_stricmpn(token, "xmap", 4) || !Q_stricmpn(token, "q3map", 5))
    {
      SkipRestOfLine( text );
      continue;
    }
    ...
    else if ( !Q_stricmp( token, "waterfogvars" ) )
    {
      ...
      {
        // distance "linear" fog
        RE_SetFog( FOG_WATER, 0, fogvar, watercolor[ 0 ], watercolor[ 1 ], watercolor[ 2 ], 1.1f );
      }
      else
      {
        // density "exp" fog
...
//----(SA)  end
    ...
    // colorMap <image>
    else if ( !Q_stricmp( token, "colorMap" ) )
    {
      ParseColorMap(&stages[s], text);
      s++;
      continue;
    }
        ...
        case RC_DRAW_VIEW:
          {
            ...
            {
              if ( (int)drawSurf->shaderNum >= newShader )
              {
                drawSurf->shaderNum++;
              }
            }
...
static shader_t *FinishShader( void )
{
  ...
  if ( r_shadows->integer >= SHADOWING_ESM16 )
  {
    ...
    {
      shader.noShadows = qfalse;//note1 hypov8 causing backfaces to display shadows with torch??
                  // can use twosided.  but the normals are wrong in brush face
                  // note2 maybe kmap needs to calcualte a new face, with new vetex normal
                  // note3 use kmap_cloneShader for map textures. bumpmap normals are calculated correct
      ...
      !shader.polygonOffset )
  {
    ...
    shader.numStages = 1;
    strncat(shader.name, "$depth", sizeof(shader.name) -1);
...
static void ScanAndLoadGuideFiles( void )
{
  ...
  int  guideTextHashTableSizes[ MAX_GUIDETEXT_HASH ], size;
  unsigned  hash;
  char filename[ MAX_QPATH ];
  long sum = 0;
...
#else
#define SHADER_FOLDER "materials"
static void ScanAndLoadShaderFiles(void)
{
  char **shaderFiles;
  char *buffers[MAX_SHADER_FILES];
  ...
  int i;
  char *oldp, *token, *hashMem;
  int shaderTextHashTableSizes[MAX_SHADERTEXT_HASH], size;
  unsigned  hash;
  char filename[MAX_QPATH];
  long sum = 0;
  ...
  // scan for shader files
  shaderFiles = ri.FS_ListFiles(SHADER_FOLDER, ".mtr", &numShaders);
  ...
  {
    Com_sprintf(filename, sizeof(filename), SHADER_FOLDER"/%s", shaderFiles[i]);
    sum += ri.FS_ReadFile(filename, NULL);
  }
  ...
  for (i = numShaders - 1; i >= 0; i--)
  {
    ...
    qstrcat(s_shaderText, "\n");
    p = &s_shaderText[qstrlen(s_shaderText)];
    qstrcat(s_shaderText, buffers[i]);
    ri.FS_FreeFile(buffers[i]);
    buffers[i] = p;
    COM_Compress(p);
  }
