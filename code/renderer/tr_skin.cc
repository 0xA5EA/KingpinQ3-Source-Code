...
qhandle_t RE_GetShaderFromModel( qhandle_t modelid, int surfnum, int withlightmap )
{
  ...
  if ( model )
  {
    ...
    {
      if ( surfnum >= (int)bmodel->numSurfaces )
      {
...
qhandle_t RE_RegisterSkin( const char *name )
{
  ...
  if ( !name || !name[ 0 ] )
  {
    Com_Printf(COLOR_YELLOW+"Empty name passed to RE_RegisterSkin\n" );
    return 0;
  }

  if ( strlen( name ) >= MAX_QPATH )
  {
    Com_Printf(COLOR_YELLOW+"Skin name exceeds MAX_QPATH\n" );
    return 0;
  }
...
//          in thier "skin" or "head" field
  ...
  if ( !text )
  {
    ri.Printf( PRINT_WARNING, "WARNING: Failed to load skin '%s'\n", name );
    return 0;
  }
...
//		surf->hash = Com_HashKey(surf->name, sizeof(surf->name));
  ...
  if ( skin->numSurfaces == 0 )
  {
    ri.Printf( PRINT_WARNING, "WARNING: Skin has no shaders in '%s'\n", name );
