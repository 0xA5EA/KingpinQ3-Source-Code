...
static shader_t *GetMDMSurfaceShader(const trRefEntity_t * ent, mdmSurfaceIntern_t * mdmSurface)
{
	...
	else if(ent->e.customSkin > 0 && ent->e.customSkin < tr.numSkins)
	{
		...
		for(j = 0; j < skin->numSurfaces; j++)
		{
			...
			if(!qstrcmp(skin->surfaces[j]->name, mdmSurface->name))
			{
		...
		if(ent->e.renderfx & RF_BLINK)
		{
			...
			for(j = 0; j < skin->numSurfaces; j++)
			{
				...
				if(!qstrcmp(skin->surfaces[j]->name, s))
				{
					shader = skin->surfaces[j]->shader;
					break;
				}
		...
		if(shader == tr.defaultShader)
		{
			...
			for(j = 0; j < skin->numSurfaces; j++)
			{
				...
				if(!qstrcmp(skin->surfaces[j]->name, surface->name))
				{
					shader = skin->surfaces[j]->shader;
					break;
				}
