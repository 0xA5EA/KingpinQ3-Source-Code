...
// the client game, as well as some locally derived info
	typedef struct trRefLight_s
	{
		...
		qboolean useSkySunAngle; //hypov8 add: use sky shader value instead of the 'light_center' value
...
// useful helper struct
	...
	enum
	{
	  IF_NONE,
	  IF_NOPICMIP = Q_BIT( 0 ),
	  IF_NOCOMPRESSION = Q_BIT( 1 ),
	  IF_ALPHA = Q_BIT( 2 ),
	  IF_NORMALMAP = Q_BIT( 3 ),
	  IF_RGBA16F = Q_BIT( 4 ),
	  IF_RGBA32F = Q_BIT( 5 ),
	  IF_TWOCOMP16F = Q_BIT( 6 ),
	  IF_TWOCOMP32F = Q_BIT( 7 ),
	  IF_ONECOMP16F = Q_BIT( 8 ),
	  IF_ONECOMP32F = Q_BIT( 9 ),
	  IF_DEPTH16 = Q_BIT( 10 ),
	  IF_DEPTH24 = Q_BIT( 11 ),
	  IF_DEPTH32 = Q_BIT( 12 ),
	  IF_PACKED_DEPTH24_STENCIL8 = Q_BIT( 13 ),
	  IF_LIGHTMAP = Q_BIT( 14 ),
	  IF_RGBA16 = Q_BIT( 15 ),
	  IF_RGBE = Q_BIT( 16 ),
	  IF_ALPHATEST = Q_BIT( 17 ),
	  IF_DISPLACEMAP = Q_BIT( 18 ),
	  IF_NOLIGHTSCALE = Q_BIT( 19 )
	};
	...
	typedef struct wrapType_s
	{
		...
		ID_INLINE struct wrapType_s &operator =( wrapTypeEnum_t w ) { this->s = this->t = w; return *this; }

	} wrapType_t;

	static ID_INLINE bool operator ==( const wrapType_t &a, const wrapType_t &b ) { return a.s == b.s && a.t == b.t; }
	static ID_INLINE bool operator !=( const wrapType_t &a, const wrapType_t &b ) { return a.s != b.s || a.t != b.t; }
	...
	enum
	{
	  ATTR_POSITION       = Q_BIT( ATTR_INDEX_POSITION ),
	  ATTR_TEXCOORD       = Q_BIT( ATTR_INDEX_TEXCOORD ),
	  ATTR_LIGHTCOORD     = Q_BIT( ATTR_INDEX_LIGHTCOORD ),
	  ATTR_TANGENT        = Q_BIT( ATTR_INDEX_TANGENT ),
	  ...
	  ATTR_POSITION2      = Q_BIT( ATTR_INDEX_POSITION2 ),
	  ATTR_TANGENT2       = Q_BIT( ATTR_INDEX_TANGENT2 ),
	  ATTR_BINORMAL2      = Q_BIT( ATTR_INDEX_BINORMAL2 ),
	  ATTR_NORMAL2        = Q_BIT( ATTR_INDEX_NORMAL2 ),
...
//  char            name[MAX_QPATH];    // polyset name
	...
	typedef struct
	{
		...
		vec3_t   origin;
		quat_t   rotation;
	matrix_t   inverseTransform;	// full inverse for tangent space transformation //kpq3
	} md5Bone_t;
	...
	typedef struct md5Model_s
	{
		...
		vec3_t          bounds[ 2 ];
		//float		internalScale; //hypov8 merge: unvan .50
	} md5Model_t;
	...
	typedef struct
	{
		...
		//union { //hypov8 curupting model (duplicates)
			md5Animation_t *md5;
			IQAnim_t       *iqm;
			psaAnimation_t *psa;
		//};
...
// 4 = shadow mapping
	...
	extern cvar_t *r_showSkeletonNames;
