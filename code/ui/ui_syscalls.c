...
{
  syscall(UI_CVAR_SETVALUE, var_name, FloatAsInt(value));
}
...
{
	return syscall(UI_R_BUILDSKELETON, skel, anim, startFrame, endFrame, FloatAsInt(frac), clearOrigin);
}
...
{
  syscall(UI_R_ADDADDITIVELIGHTTOSCENE, org, FloatAsInt( intensity ),FloatAsInt( r ),FloatAsInt( g ), FloatAsInt( b ));
}

void trap_R_AddLightToScene( const vec3_t org, float radius, float intensity, float r, float g, float b, qhandle_t hShader, int flags )
{
  syscall(UI_R_ADDLIGHTTOSCENE , org, FloatAsInt( radius ), FloatAsInt( intensity ), FloatAsInt( r ), FloatAsInt( g ), FloatAsInt( b ), hShader, flags );
}
...
{
  syscall(UI_R_DRAWSTRETCHPIC, FloatAsInt(x), FloatAsInt(y), FloatAsInt(w), FloatAsInt(h), FloatAsInt(s1), FloatAsInt(t1), FloatAsInt(s2), FloatAsInt(t2), hShader);
}

void trap_R_DrawRotatedPic(float x, float y, float w, float h, float s1, float t1, float s2, float t2, qhandle_t hShader, float angle)
{
  syscall(UI_R_DRAWROTATEDPIC, FloatAsInt(x), FloatAsInt(y), FloatAsInt(w), FloatAsInt(h), FloatAsInt(s1),
                               FloatAsInt(t1), FloatAsInt(s2), FloatAsInt(t2), hShader, FloatAsInt(angle));
}
...
{
   return syscall(UI_R_LERPTAG,tag, refent, tagName, startIndex);
}
...
int trap_R_LerpTag(orientation_t *tag, clipHandle_t mod, int startFrame, int endFrame, float frac, const char *tagName)
{
  floatint_t _frac;
  _frac.f = frac;
  return syscall(UI_R_LERPTAG, tag, mod, startFrame, endFrame, _frac.i, tagName);
}
#endif
...
{
  return syscall(UI_CM_LERPTAG, tag, mod, startFrame, endFrame, FloatAsInt(frac), tagName);
}
...
void trap_GetCpuConfig(cpuConfig_t * cpuConfig)
{
  syscall(UI_GETCPUCONFIG, cpuConfig);
}
