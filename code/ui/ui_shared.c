...
#if 0 //HYPODEBUG
static qboolean debugMode = qtrue; //hypov8 debug UI with helper boxes //use f11 in menu. cant use ingame
#else
...
const char *String_Alloc(const char *p)
{
  ...
  if (len + strPoolIndex + 1 < STRING_POOL_SIZE)
  {
    ...
    qstrcpy(&strPool[strPoolIndex], p);
    strPoolIndex += len + 1;
...
void Window_Paint(Window *w, float fadeAmount, float fadeClamp, float fadeCycle)
{
  ...
  if (w->style == WINDOW_STYLE_FILLED)
  {
    ...
    {
//			Com_Printf(S_COLOR_RED "Backcolor: %f, %f, %f, %f\n", w->backColor[0],
//					w->backColor[1],
//					w->backColor[2],
//					w->backColor[3] );
...
void Script_RandomPlay(itemDef_t *item, char **args)
{
  const char *val, *ext;
  ...
  float x, y;
  int i, j, d, idx;
  int random; // maby unitialized is better than rand() ?
  static int randincrement;
  ...
  if (String_Parse(args, &val))
  {
    if (Float_Parse(args, &x) && Float_Parse(args, &y))
    {
      ...
      if (i != j)
      {
        ...
        DC->startLocalSound(DC->registerSound(finalName, qfalse), CHAN_LOCAL_SOUND);
        return;
      }
    }
    DC->startLocalSound(DC->registerSound(val, qfalse), CHAN_LOCAL_SOUND);
...
void Script_RandomPlayLooped(itemDef_t *item, char **args)
{
  const char *val, *ext;
  char strippedName[MAX_QPATH];
  char finalName[MAX_QPATH];
  float x, y;
  int i, j, d, idx;
  int random; // unitialized is better than rand() ?
  static int randincrement;
  ...
  if (String_Parse(args, &val))
  {
    if (Float_Parse(args, &x) && Float_Parse(args, &y))
    {
      ...
      if (i != j)
      {
        ...
        DC->startBackgroundTrack(finalName, finalName);
        return;
      }
    }
    DC->stopBackgroundTrack();
...
commandDef_t commandList[] = {
  ...
  {"randplay", &Script_RandomPlay},
  {"randplaylooped", Script_RandomPlayLooped},
...
float Item_Slider_ThumbPosition(itemDef_t *item)
{
  ...
  if (!editDef || !item->cvar) //this was changed from ioq3 to &&??
  {
    return x;
  }
...
int Item_Slider_OverSlider(itemDef_t *item, float x, float y)
{
  ...
  r.h = item->window.rect.h - 4; // SLIDER_THUMB_HEIGHT; //hypov8 set height to match rectangle
...
qboolean Item_TextField_HandleKey(itemDef_t *item, int key)
{
  ...
  if (item->cvar)
  {
    ...
    DC->getCVarString(item->cvar, buff, sizeof(buff));
    len = qstrlen(buff);
    if (editPtr->maxChars && len > (size_t)editPtr->maxChars)
    ...
    if (key & K_CHAR_FLAG)
    {
      ...
      if (key == 'h' - 'a' + 1)       // ctrl-h is backspace
      {
        if (item->cursorPos > 0)
        {
          Com_Memmove(&buff[item->cursorPos - 1], &buff[item->cursorPos], len + 1 - item->cursorPos);
          item->cursorPos--;
      ...
      {
        if ((len == MAX_EDITFIELD - 1) || (editPtr->maxChars && len >= (size_t)editPtr->maxChars))
          return qtrue;
      ...
      if ((size_t)item->cursorPos < len + 1)
      {
        item->cursorPos++;
    ...
    else
    {
      ...
      {
        if ((size_t)item->cursorPos < len)
        {
          Com_Memmove(buff + item->cursorPos, buff + item->cursorPos + 1, len - item->cursorPos);
          DC->setCVar(item->cvar, buff);
        }
        return qtrue;
      }
      ...
      if (key == K_RIGHTARROW || key == K_KP_RIGHTARROW)
      {
        if (editPtr->maxPaintChars && item->cursorPos >= editPtr->maxPaintChars && (size_t)item->cursorPos < len)
        {
          item->cursorPos++;
          editPtr->paintOffset++;
          return qtrue;
...
void Item_SetTextExtents(itemDef_t *item, int *width, int *height, const char *text)
{
  ...
  *height = item->textRect.h;
  ...
  {
    int originalWidth = DC->textWidth(item->text, item->textscale, 0, NULL_FONT);
    ...
    else if (item->type == ITEM_TYPE_EDITFIELD && item->textalignment == ITEM_ALIGN_CENTER && item->cvar)
    {
      ...
      originalWidth += DC->textWidth(buff, item->textscale, 0, NULL_FONT);
    }

    *width           = DC->textWidth(textPtr, item->textscale, 0, NULL_FONT);
    *height          = DC->textHeight(textPtr, item->textscale, 0, NULL_FONT);
...
void Item_Text_AutoWrapped_Paint(itemDef_t *item)
{
  ...
  while (p)
  {
    ...
    textWidth = DC->textWidth(buff, item->textscale, 0, NULL_FONT);
    if ((newLine && textWidth > item->window.rect.w) || *p == '\n' || *p == '\0')
    {
      if (len)
      {
        ...
        //
        buff[newLine] = '\0';
        DC->drawText(item->textRect.x, item->textRect.y, item->textscale, color, buff, 0, 0, item->textStyle, NULL_FONT);
      }
      if (*p == '\0')
        break;
      //
...
void Item_Text_Wrapped_Paint(itemDef_t *item)
{
  ...
  while (p && *p)
  {
    ...
    buff[p - start] = '\0';
    DC->drawText(x, y, item->textscale, color, buff, 0, 0, item->textStyle, NULL_FONT);
    y     += height + 5;
    start += p - start + 1;
    p      = strchr(p + 1, '\r');
  }
  DC->drawText(x, y, item->textscale, color, start, 0, 0, item->textStyle, NULL_FONT);
}
...
//	}
  ...
  DC->drawText(item->textRect.x, item->textRect.y, item->textscale, color, textPtr, 0, 0, item->textStyle, NULL_FONT);
}
...
void Item_TextField_Paint(itemDef_t *item)
{
  ...
  {
    DC->drawText(item->textRect.x + item->textRect.w + offset, item->textRect.y, item->textscale, newColor, buff + editPtr->paintOffset, 0, editPtr->maxPaintChars, item->textStyle, NULL_FONT);
  }
}
...
void Item_YesNo_Paint(itemDef_t *item)
{
  ...
  {
    Item_Text_Paint(item);
    DC->drawText(item->textRect.x + item->textRect.w + 8, item->textRect.y, item->textscale, newColor, (value != 0) ? "Yes" : "No", 0, 0, item->textStyle, NULL_FONT);
  }
  else
  {
    DC->drawText(item->textRect.x, item->textRect.y, item->textscale, newColor, (value != 0) ? "Yes" : "No", 0, 0, item->textStyle, NULL_FONT);
  }
}
...
void Item_Multi_Paint(itemDef_t *item)
{
  ...
  {
    Item_Text_Paint(item);
    DC->drawText(item->textRect.x + item->textRect.w + 8, item->textRect.y, item->textscale, newColor, text, 0, 0, item->textStyle, NULL_FONT);
  }
  else
  {
    DC->drawText(item->textRect.x, item->textRect.y, item->textscale, newColor, text, 0, 0, item->textStyle, NULL_FONT);
  }
}
...
static const int KEY_BIND_COUNT = sizeof(g_bindings) / sizeof(bind_t);
  ...
  for (i = 0; i < KEY_BIND_COUNT; i++)
  {
    if (Q_stricmp(cvar, g_bindings[i].command) == 0)
    {
      ...
      if (b2 != -1)
      {
        ...
        qstrcat(g_nameBind1, " or ");
        qstrcat(g_nameBind1, g_nameBind2);
      }
      return;
    }
  }
...
void Item_Bind_Paint(itemDef_t *item)
{
  ...
  if (item->text)
  {
    ...
    DC->drawText(item->textRect.x + item->textRect.w + 8, item->textRect.y, item->textscale, newColor, g_nameBind1, 0, maxChars, item->textStyle, NULL_FONT);
  }
  else
  {
    DC->drawText(item->textRect.x, item->textRect.y, item->textscale, newColor, (value != 0) ? "FIXME" : "FIXME", 0, maxChars, item->textStyle, NULL_FONT);
  }
}
        ...
        if (listPtr->numColumns > 0)
        {
          ...
          for (j = 0; j < listPtr->numColumns; j++)
          {
            ...
            if (optionalImage >= 0)
              ...
              DC->drawText(x + 4 + listPtr->columnInfo[j].pos, y + listPtr->elementHeight, item->textscale, item->window.foreColor, text, 0, listPtr->columnInfo[j].maxChars, item->textStyle, NULL_FONT);
          }
        }
        else
        {
          ...
          if (optionalImage >= 0)
            ...
            DC->drawText(x + 4, y + listPtr->elementHeight, item->textscale, item->window.foreColor, text, 0, 0, item->textStyle, NULL_FONT);
        }
...
void Menu_HandleMouseMove(menuDef_t *menu, float x, float y)
{
  ...
  if (itemCapture)
    return;  //Item_MouseMove(itemCapture, x, y);
...
qboolean ItemParse_asset_model(itemDef_t *item, int handle)
{
  ...
  item->asset = DC->registerModel(temp); // F'IXME: 0xA5EA, qtrue ok ?
...
void Menu_PaintAll(void)
{
  ...
  {
    vec4_t v = {1, 1, 1, 1};
    DC->drawText(5, 25, .5, v, va("fps: %f", DC->FPS), 0, 0, 0, NULL_FONT);
  }
}
