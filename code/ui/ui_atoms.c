...
{
  int developer = trap_Cvar_VariableValue("developer");
  if (developer)
  {
    va_list argptr;
    char text[1024];
...
void UI_LoadBestScores(const char *map, int game)
{
  ...
  Com_sprintf(fileName, MAX_QPATH, "demos/%s_%d.%s%d", map, game, DEMOEXT, protocol); //hypov8 "protocol"
...
static void UI_CalcPostGameStats(void)
{
  ...
  trap_Cvar_Set("g_capturelimit", UI_Cvar_VariableString("ui_saveCaptureLimit"));//hypov8 "capturelimit"
  trap_Cvar_Set("g_fraglimit", UI_Cvar_VariableString("ui_saveFragLimit")); //hypov8 "fraglimit"
  // cashlimit rein 0xA5EA
...
#else
  float size UI_AdjustFrom640(&x, &y, &w, &h);
