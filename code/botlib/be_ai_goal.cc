...
#define IFL_NOTCTF       32
#define IFL_NOT1CTF      64
#define IFL_NOTBAGMAN   128
...
itemconfig_t *LoadItemConfig(char *filename)
{
	...
	{
		if (!qstrcmp(token.string, "iteminfo"))
		{
...
void BotInitInfoEntities(void)
{
	...
	for (ent = AAS_NextBSPEntity(0); ent; ent = AAS_NextBSPEntity(ent))
	{
		...
		//camp spots
		else if (!qstrcmp(classname, "info_camp"))
		{
...
void BotInitLevelItems(void)
{
	...
	for (ent = AAS_NextBSPEntity(0); ent; ent = AAS_NextBSPEntity(ent))
	{
		...
		for (i = 0; i < ic->numiteminfo; i++)
		{
			if (!qstrcmp(classname, ic->iteminfo[i].classname)) break;
		} //end for
		if (i >= ic->numiteminfo)
		{
...
void BotPushGoal(int goalstate, bot_goal_t *goal)
{
	...
	//FIXME(0xA5EA): had thisError: goal heap overflow
	// with two bots on g_gametype 0 on fortknox
