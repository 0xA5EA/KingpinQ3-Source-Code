...
void BotDefaultCharacteristics(bot_character_t *ch, bot_character_t *defaultch)
{
	...
	for (i = 0; i < MAX_CHARACTERISTICS; i++)
	{
		...
		else if (defaultch->c[i].type == CT_STRING)
		{
			...
			ch->c[i].value.string = (char *) GetMemory(qstrlen(defaultch->c[i].value.string)+1);
			qstrcpy(ch->c[i].value.string, defaultch->c[i].value.string);
		} //end else if
	} //end for
...
bot_character_t *BotLoadCharacterFromFile(char *charfile, int skill)
{
	...
	qstrcpy(ch->filename, charfile);
	while(PC_ReadToken(source, &token))
	{
		if (!qstrcmp(token.string, "skill"))
		{
			...
			if (skill < 0 || (int)token.intvalue == skill)
			{
				...
				while(PC_ExpectAnyToken(source, &token))
				{
					...
					else if (token.type == TT_STRING)
					{
						...
						ch->c[index].value.string = (char*)GetMemory(qstrlen(token.string)+1);
						qstrcpy(ch->c[index].value.string, token.string);
			...
			else
			{
				...
				while(indent)
				{
					...
					if (!qstrcmp(token.string, "{")) indent++;
					else if (!qstrcmp(token.string, "}")) indent--;
				} //end while
			} //end else
		} //end if
		else
...
int BotFindCachedCharacter(char *charfile, float skill)
{
	...
	for (handle = 1; handle <= MAX_CLIENTS; handle++)
	{
		...
		if ( qstrcmp( botcharacters[handle]->filename, charfile ) == 0 &&
...
int BotInterpolateCharacters(int handle1, int handle2, float desiredskill)
{
	...
	qstrcpy(out->filename, ch1->filename);
	...
	for (i = 0; i < MAX_CHARACTERISTICS; i++)
	{
		...
		else if (ch1->c[i].type == CT_STRING)
		{
			...
			out->c[i].value.string = (char *) GetMemory(qstrlen(ch1->c[i].value.string)+1);
			qstrcpy(out->c[i].value.string, ch1->c[i].value.string);
		} //end else if
	} //end for
	return handle;
