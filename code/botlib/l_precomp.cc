...
int PC_StringizeTokens(token_t *tokens, token_t *token)
{
	...
	{
#ifndef BSPC
	strncat(token->string, t->string, MAX_TOKEN - qstrlen(token->string) - 1);
#else
...
#ifndef BSPC
	strncat(token->string, "\"", MAX_TOKEN - qstrlen(token->string) - 1);
#else
...
int PC_MergeTokens(token_t *t1, token_t *t2)
{
	...
	if (t1->type == TT_STRING && t2->type == TT_STRING)
	{
		...
		t1->string[qstrlen(t1->string)-1] = '\0';
		//concat without leading double quote
		qstrcat(t1->string, &t2->string[1]);
#else
		//remove trailing double quote
...
define_t *PC_FindHashedDefine(define_t **definehash, char *name)
{
	...
	{
		if (!qstrcmp(d->name, name)) return d;
	} //end for
	return NULL;
...
int PC_FindDefineParm(define_t *define, char *name)
{
	...
	{
		if (!qstrcmp(p->string, name)) return i;
		i++;
	} //end for
	return -1;
...
//		{ "__STDC__", BUILTIN_STDC },
	...
	for (i = 0; builtin[i].string; i++)
	{
		...
		define->name = (char *) GetMemory(qstrlen(builtin[i].string) + 1);
#else
										...
										token_t **firsttoken, token_t **lasttoken)
{
	...
	switch(define->builtin)
	{
		...
		case BUILTIN_FILE:
		{
			...
			token->type = TT_NAME;
#ifndef BSPC
			token->subtype = qstrlen(token->string);
#else
...
int PC_Directive_undef(source_t *source)
{
	...
	{
		if (!qstrcmp(define->name, token.string))
		{
...
int PC_Directive_define(source_t *source)
{
...
#ifndef BSPC
	Com_Memset(define, 0, sizeof(define_t));
	define->name = (char *) GetMemory(qstrlen(token.string) + 1);
#else
	memset(define, 0, sizeof(define_t));
	...
	if (!PC_WhiteSpaceBeforeToken(&token) && !qstrcmp(token.string, "("))
	{
		//read the define parameters
		last = NULL;
	...
	{
		t = PC_CopyToken(&token);
		if (t->type == TT_NAME && !qstrcmp(t->string, define->name))
		{
	...
	if (last)
	{
		...
		if (!qstrcmp(define->tokens->string, "##") ||
				!qstrcmp(last->string, "##"))
		{
...
define_t *PC_DefineFromString(char *string)
{
	...
	//create a new source
#ifndef BSPC
	 script = LoadScriptMemory(string, qstrlen(string), "*extern");
...
#ifndef BSPC
	newdefine->name = (char *) GetMemory(qstrlen(define->name) + 1);
#else
	 newdefine->name = (char *) GetMemory(strlen(define->name) + 1);
#endif
	 //FIXME (0xA5EA): standart strcpy
																	...
																	float *floatvalue, int integer)
{
	...
	while(!error && firstoperator)
	{
		...
		for (o = firstoperator; o->next; o = o->next)
		{
			...
			if (o->ooperator != P_LOGIC_NOT
					&& o->ooperator != P_BIN_NOT) v = v->next;
		...
		if (integer)
		{
			Log_Write("operator %s, value1 = %d", PunctuationFromNum(source->scriptstack, o->ooperator), v1->intvalue);
			if (v2) Log_Write("value2 = %d", v2->intvalue);
		} //end if
		else
		{
			Log_Write("operator %s, value1 = %f", PunctuationFromNum(source->scriptstack, o->ooperator), v1->floatvalue);
...
#endif //DEBUG_EVAL
		if (error) break;
		//lastoperatortype = o->ooperator;
		//if not an operator with arity 1
		if (o->ooperator != P_LOGIC_NOT
				&& o->ooperator != P_BIN_NOT)
		{
			//remove the second value if not question mark operator
			if (o->ooperator != P_QUESTIONMARK) v = v->next;
			//
												...
												float *floatvalue, int integer)
{
	...
	do
	{
		...
		if (token.type == TT_NAME)
		{
			...
			else if (!qstrcmp(token.string, "defined"))
			{
				defined = qtrue;
				t = PC_CopyToken(&token);
				t->next = NULL;
...
int PC_Directive_eval(source_t *source)
{
	...
	token.endwhitespace_p = source->scriptstack->script_p;
	token.linescrossed = 0;
	sprintf(token.string, "%i", (int)abs(value));
	token.type = TT_NUMBER;
...
int PC_ReadDirective(source_t *source)
{
	...
	if (token.type == TT_NAME)
	{
		...
		{
			if (!qstrcmp(directives[i].name, token.string))
			{
...
int PC_ReadDollarDirective(source_t *source)
{
	...
	if (token.type == TT_NAME)
	{
		...
		{
			if (!qstrcmp(dollardirectives[i].name, token.string))
			{
	...
	while(1)
	{
		...
		if (token->type == TT_STRING)
		{
			...
			if (PC_ReadToken(source, &newtoken))
			{
				...
				{
#ifndef BSPC
					token->string[qstrlen(token->string)-1] = '\0';
	        if (qstrlen(token->string) + qstrlen(newtoken.string+1) + 1 >= MAX_TOKEN)
#else
...
#ifndef BSPC
					qstrcat(token->string, newtoken.string+1);
#else
...
int PC_ExpectTokenString(source_t *source, char *string)
{
	...
	if (qstrcmp(token.string, string))
	{
...
#ifndef BSPC
			if (subtype & TT_LONG) qstrcat(str, " long");
			if (subtype & TT_UNSIGNED) qstrcat(str, " unsigned");
			if (subtype & TT_FLOAT) qstrcat(str, " float");
			if (subtype & TT_INTEGER) qstrcat(str, " integer");
#else
...
int PC_CheckTokenString(source_t *source, char *string)
{
	...
	if (!qstrcmp(tok.string, string)) return qtrue;
	//
...
int PC_SkipUntilString(source_t *source, char *string)
{
	...
	{
		if (!qstrcmp(token.string, string)) return qtrue;
	} //end while
	return qfalse;
...
#ifndef BSPC
	if (source->includepath[qstrlen(source->includepath)-1] != '\\' &&
		source->includepath[qstrlen(source->includepath)-1] != '/')
#else
