...
#define MODELTYPE_FUNC_DOOR_ROTATING 5 //add hypov8 func_door_rotating
...
void BotSetBrushModelTypes(void)
{
	...
	for (ent = AAS_NextBSPEntity(0); ent; ent = AAS_NextBSPEntity(ent))
	{
		...
		else if (!Q_stricmp(classname, "func_door_rotating"))
			modeltypes[modelnum] = MODELTYPE_FUNC_DOOR_ROTATING;
...
//	vec3_t runstart, dir1, dir2, hordir;
	...
	VectorSubtract(ms->origin, reach->start, dir1);
	dir1[2] = 0;
	dist1 = VectorNormalize3(dir1);
	VectorSubtract(ms->origin, runstart, dir2);
	dir2[2] = 0;
	dist2 = VectorNormalize3(dir2);
	//if just before the reachability start
	if (DotProduct(dir1, dir2) < -0.8 || dist2 < 5)
	{
...
#if 1					/* add hypov8 func_door_rotating */
						|| modeltype == MODELTYPE_FUNC_DOOR_ROTATING
#endif
						)
				{
