...
int BotValidWeaponNumber(int weaponnum)
{
	...
	{
		botimport.Print(PRT_ERROR, "weapon number out of range (%i)\n", weaponnum);
		return qfalse;
	} //end if
	return qtrue;
...
weaponconfig_t *LoadWeaponConfig(char *filename)
{
	...
	{
		if (!qstrcmp(token.string, "weaponinfo"))
		{
		...
		else if (!qstrcmp(token.string, "projectileinfo"))
		{
	...
	for (i = 0; i < wc->numweapons; i++)
	{
		...
		{
			if (!qstrcmp(wc->projectileinfo[j].name, wc->weaponinfo[i].projectile))
			{
