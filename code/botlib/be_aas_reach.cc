...
#define AAS_MAX_REACHABILITYSIZE			2*65536
...
int AAS_GetJumpPadInfo(int ent, vec3_t areastart, vec3_t absmins, vec3_t absmaxs, vec3_t velocity)
{
	...
	dist = VectorNormalize3(velocity);
	forward = dist / time;
...
//#endif REACH_DEBUG
		...
		else if (!qstrcmp(classname, "trigger_teleport"))
		{
			AAS_ValueForBSPEpairKey(ent, "model", model, MAX_EPAIRKEY);
//#ifdef REACH_DEBUG
...
void AAS_SetWeaponJumpAreaFlags(void)
{
	...
	for (ent = AAS_NextBSPEntity(0); ent; ent = AAS_NextBSPEntity(ent))
	{
		if (!AAS_ValueForBSPEpairKey(ent, "classname", classname, MAX_EPAIRKEY)) continue;
		if (
			!qstrcmp(classname, "item_armor_body") ||
			!qstrcmp(classname, "item_armor_combat") ||
			!qstrcmp(classname, "item_health_mega") ||
			!qstrcmp(classname, "weapon_grenadelauncher") ||
			!qstrcmp(classname, "weapon_rocketlauncher") ||
			!qstrcmp(classname, "weapon_lightning") ||
			!qstrcmp(classname, "weapon_plasmagun") ||
			!qstrcmp(classname, "weapon_railgun") ||
			!qstrcmp(classname, "weapon_bfg") ||
			!qstrcmp(classname, "item_quad") ||
			!qstrcmp(classname, "item_regen") ||
			!qstrcmp(classname, "item_invulnerability"))
		{
			if (AAS_VectorForBSPEpairKey(ent, "origin", origin))
			{
				spawnflags = 0;
