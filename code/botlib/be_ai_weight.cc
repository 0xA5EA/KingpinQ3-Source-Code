...
fuzzyseperator_t *ReadFuzzySeperators_r(source_t *source)
{
	...
	do
	{
		def = !qstrcmp(token.string, "default");
		if (def || !qstrcmp(token.string, "case"))
		{
			...
			if (!qstrcmp(token.string, "return"))
			{
			...
			else if (!qstrcmp(token.string, "switch"))
			{
...
weightconfig_t *ReadWeightConfig(char *filename)
{
	...
	if (!LibVarGetValue("bot_reloadcharacters"))
	{
		...
		for( n = 0; n < MAX_WEIGHT_FILES; n++ )
		{
			...
			if( qstrcmp( filename, config->filename ) == 0 )
			{
	...
	{
		if (!qstrcmp(token.string, "weight"))
		{
			...
			config->weights[config->numweights].name = (char *) GetClearedMemory(qstrlen(token.string) + 1);
			qstrcpy(config->weights[config->numweights].name, token.string);
			if (!PC_ExpectAnyToken(source, &token))
			{
				FreeWeightConfig(config);
				FreeSource(source);
			...
			if (!qstrcmp(token.string, "switch"))
			{
			...
			else if (!qstrcmp(token.string, "return"))
			{
				fs = (fuzzyseperator_t *) GetClearedMemory(sizeof(fuzzyseperator_t));
				fs->index = 0;
				fs->value = MAX_INVENTORYVALUE;
...
int FindFuzzyWeight(weightconfig_t *wc, char *name)
{
	...
	{
		if (!qstrcmp(wc->weights[i].name, name))
		{
			return i;
		} //end if
	} //end if
...
void ScaleWeight(weightconfig_t *config, char *name, float scale)
{
	...
	for (i = 0; i < config->numweights; i++)
	{
		if (!qstrcmp(name, config->weights[i].name))
		{
