...
void AAS_ParseBSPEntities(void)
{
	...
	while(PS_ReadToken(script, &token))
	{
		...
		while(PS_ReadToken(script, &token))
		{
			...
			epair->key = (char *) GetHunkMemory(qstrlen(token.string) + 1);
			qstrcpy(epair->key, token.string);
			...
			StripDoubleQuotes(token.string);
			epair->value = (char *) GetHunkMemory(qstrlen(token.string) + 1);
			qstrcpy(epair->value, token.string);
		} //end while
		if (qstrcmp(token.string, "}"))
		{
...
{
	AAS_DumpBSPData();
	bspworld.entdatasize = qstrlen(botimport.BSPEntityData()) + 1;
