...
int PS_ExpectTokenType(script_t *script, int type, int subtype, token_t *token)
{
	...
	if (token->type != type)
	{
		if (type == TT_STRING) qstrcpy(str, "string");
		if (type == TT_LITERAL) qstrcpy(str, "literal");
		if (type == TT_NUMBER) qstrcpy(str, "number");
		if (type == TT_NAME) qstrcpy(str, "name");
		if (type == TT_PUNCTUATION) qstrcpy(str, "punctuation");
	...
	if (token->type == TT_NUMBER)
	{
		...
		{
			if (subtype & TT_DECIMAL) qstrcpy(str, "decimal");
			if (subtype & TT_HEX) qstrcpy(str, "hex");
			if (subtype & TT_OCTAL) qstrcpy(str, "octal");
			if (subtype & TT_BINARY) qstrcpy(str, "binary");
...
int PS_CheckTokenString(script_t *script, char *string)
{
	...
	if (!qstrcmp(tok.string, string)) return 1;
	//token not available
...
int PS_SkipUntilString(script_t *script, char *string)
{
	...
	{
		if (!qstrcmp(token.string, string)) return 1;
	} //end while
	return 0;
...
void StripDoubleQuotes(char *string)
{
  if (*string == '\"')
  {
	...
	if (string[qstrlen(string)-1] == '\"')
		string[qstrlen(string)-1] = '\0';
}
...
script_t *LoadScriptFile(const char *filename)
{
	...
	qstrcpy(script->filename, filename);
