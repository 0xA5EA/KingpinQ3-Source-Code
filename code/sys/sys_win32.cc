...
// mingw doesn't seem to have these defined :(
  ...
  extern "C"  unsigned int _controlfp(unsigned int neww, unsigned int mask);
#endif
...
char *Sys_DefaultHomePath(void)
{
	...
	FARPROC qSHGetFolderPath = 0;
	HRESULT (WINAPI *pfun)(HWND,int,HANDLE,DWORD,LPSTR) = 0;
	...
	if(!*homePath && com_homepath)
	{
		...
		pfun = reinterpret_cast<HRESULT(WINAPI *)(HWND,int,HANDLE,DWORD,LPSTR)>(qSHGetFolderPath);
		...
		if (!SUCCEEDED(pfun(NULL, CSIDL_APPDATA, NULL, 0, szPath)))
		{
...
qboolean Sys_PIDIsRunning( int pid )
{
	...
	{
		if (processes[i] == (DWORD)pid) //hypov8 changed type to dword
			return qtrue;
	}
