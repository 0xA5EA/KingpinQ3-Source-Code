...
static void RadSample(int lightmapNum, bspDrawSurface_t * ds, rawLightmap_t * lm, shaderInfo_t * si, radWinding_t * rw,
					  vec3_t average, vec3_t gradient, int *style)
{
...
void RadLightForTriangles(int num, int lightmapNum, rawLightmap_t * lm, shaderInfo_t * si, float scale, float subdivide,
						  clipWork_t * cw)
{
	int             i, j, k, v;
	bspDrawSurface_t *ds;
					  ...
					  clipWork_t * cw)
{
	...
	for(y = 0; y < (mesh->height - 1); y++)
	{
		for(x = 0; x < (mesh->width - 1); x++)
		{
			...
			/* planar? */
			planar = PlaneFromPoints(plane, dv[0]->xyz, dv[1]->xyz, dv[2]->xyz, qtrue);
			if(planar)
			{
		...
		if(file)
		{
			for(light = lights; light; light = light->next)
			{
				fprintf(file,
						...
						"}\n",
						(int)light->add,
						light->origin[0], light->origin[1], light->origin[2], light->color[0], light->color[1], light->color[2]);
			}
			fclose(file);
		}
	}
