...
//	unsigned char   rgbe[4];
	...
	if(fprintf(file,"# Generated by kmap2 \n") < 0)
			Error("RGBE write error: %s", filename);
...
int ExportLightmapsMain(int argc, char **argv)
{
	...
	if(argc < 1)
	{
		Sys_Printf("Usage: kmap -export [-v] <mapname>\n");
		return 0;
	}
...
int ImportLightmapsMain(int argc, char **argv)
{
	...
	if(argc < 1)
	{
		Sys_Printf("Usage: kmap -import [-v] <mapname>\n");
		return 0;
	}
...
void StoreSurfaceLightmaps(void)
{
...
#endif
			{
				sprintf(filename, "%s/" EXTERN_LIGHTMAP_FILE, dirname, numExtLightmaps);
				Sys_FPrintf(SYS_VRB, "\nwriting %s", filename);
#ifdef USE_PNG_LIGHTMAP
	...
	for(i = 0; i < numBSPDrawSurfaces; i++)
	{
		...
		if(olm != NULL && lm != NULL && lm->styles[1] != LS_NONE && game->load != LoadRBSPFile)	//% info->si->styleMarker > 0 )
		{
			...
			/* setup */
			sprintf(styleStages, "\n\t// kmap2 custom lightstyle stage(s)\n");
			dv = &bspDrawVerts[ds->firstVert];
			...
			for(lightmapNum = 1; lightmapNum < MAX_LIGHTMAPS; lightmapNum++)
			{
				...
				if(lm->outLightmapNums[lightmapNum] == lm->outLightmapNums[0])
					...
					sprintf(lightmapName, "maps/%s/" EXTERN_LIGHTMAP_FILE, mapName, olm->extLightmapNum);
			...
			/*
			   ...
			   csi = CustomShader(info->si, "kmap_styleMarker2", styleStages);
			   else
			   csi = CustomShader(info->si, "kmap_styleMarker", styleStages);
			 */
				...
				(olm->customWidth != game->lightmapSize || olm->customHeight != game->lightmapSize))
		{
			...
			/* create custom shader */
			//% Tr3B: commented csi = CustomShader(info->si, "$lightmap", lightmapName);
