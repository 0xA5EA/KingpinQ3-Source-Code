...
int Winding_PlanesConcave(fixedWinding_t * w1, fixedWinding_t * w2, vec3_t normal1, vec3_t normal2, float dist1, float dist2)
{
	int             i;
...
#ifdef KMAP2_NO_TYPE_PUNNING
	blah.i = portalclusters;
	byteptr = bspVisBytes;
	Vec4_Copy(blah.b, byteptr);
	blah.i = leafbytes;
	byteptr += 4;
	Vec4_Copy(blah.b, byteptr);
#else
