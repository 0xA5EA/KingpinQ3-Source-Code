...
static void ParseShaderFile(const char *filename)
{
	...
	char            shaderText[8192];	/* ydnar: fixme (make this bigger?) hypov8 todo: MAX_SHADER_INFO*/
	...
	while(1)
	{
		...
		while(1)
		{
				...
				while(1)
				{
						...
						&& si->diffuseImagePath[0] == '\0')
					{
						   ...
						   !Q_stricmp(token, "clampMap") || !Q_stricmp(token, "mapComp") || !Q_stricmp(token, "mapNoComp"))
						{
							...
							/* get an image */
							GetTokenAppend(shaderText, qfalse);
							if(token[0] != '*' && token[0] != '$') //hypov8 add: _white etc..?
							{
			...
			/* ydnar: kmap_normalimage <image> (bumpmapping normal map) */
			else if(!Q_stricmp(token+PREFIX_LEN, "normalImage"))
			{
				GetTokenAppend(shaderText, qfalse);
			...
			/* kmap_lightimage <image> */
			else if(!Q_stricmp(token+PREFIX_LEN, "lightImage"))
			{
				GetTokenAppend(shaderText, qfalse);
				strcpy(si->lightImagePath, token);
				DefaultExtension(si->lightImagePath, ".tga");

				Sys_FPrintf(SYS_VRB, "kmap_lightImage: %s\n", si->lightImagePath);
			}
			...
			/* kmap_sun <red> <green> <blue> <intensity> <degrees> <elevation>
			   color will be normalized, so it doesn't matter what range you use
			   intensity falls off with angle but not distance 100 is a fairly bright sun
			   degree of 0 = from the east, 90 = north, etc.  altitude of 0 = sunrise/set, 90 = noon
			   ydnar: sof2map has bareword 'sun' token, so we support that as well */
			else if(!Q_stricmp(token, "sun") /* sof2 */ || 
					!Q_stricmp(token+PREFIX_LEN, "sun")	|| 
					!Q_stricmp(token+PREFIX_LEN, "sunExt"))
			{
				float           a, b;
				sun_t          *sun;
				qboolean        ext;
				...
				if(!Q_stricmp(token+PREFIX_LEN, "sunext"))
					ext = qtrue;
			...
			/* match kmap_ */
			else if(!Q_strncasecmp(token, "kmap_", PREFIX_LEN) || !Q_strncasecmp(token, "xmap_", PREFIX_LEN))
			{
				/* ydnar: kmap_baseShader <shader> (inherit this shader's parameters) */
				if(!Q_stricmp(token + PREFIX_LEN, "baseShader") )
				{
					shaderInfo_t   *si2;
				...
				/* ydnar: kmap_surfacemodel <path to model> <density> <min scale> <max scale> <min angle> <max angle> <oriented (0 or 1)> */
				else if(!Q_stricmp(token + PREFIX_LEN, "surfacemodel"))
				{
				...
				/* ydnar/sd: kmap_foliage <path to model> <scale> <density> <odds> <invert alpha (1 or 0)> */
				else if(!Q_stricmp(token + PREFIX_LEN, "foliage"))
				{
					foliage_t      *foliage;
				...
				/* ydnar: kmap_bounce <value> (fraction of light to re-emit during radiosity passes) */
				else if(!Q_stricmp(token+PREFIX_LEN, "bounce") || !Q_stricmp(token+PREFIX_LEN, "bounceScale"))
				{
					GetTokenAppend(shaderText, qfalse);
					si->bounceScale = atof(token);
				}

				/* ydnar/splashdamage: kmap_skyLight <value> <iterations> */
				else if(!Q_stricmp(token+PREFIX_LEN, "skyLight"))
				{
					GetTokenAppend(shaderText, qfalse);
				...
				/* kmap_surfacelight <value> */
				else if(!Q_stricmp(token+PREFIX_LEN, "surfacelight") )
				{
					GetTokenAppend(shaderText, qfalse);
					si->value = atof(token);
					  Sys_FPrintf(SYS_VRB, "kmap_surfacelight: %f (%s)\n", si->value, si->shader);
				}

				/* kmap_lightStyle (sof2/jk2 lightstyle) */
				else if(!Q_stricmp(token+PREFIX_LEN, "lightStyle") )
				{
					GetTokenAppend(shaderText, qfalse);
					val = atoi(token);
					if(val < 0)
				...
				/* wolf: kmap_lightRGB <red> <green> <blue> */
				else if(!Q_stricmp(token+PREFIX_LEN, "lightRGB"))
				{
				...
				/* kmap_lightSubdivide <value> */
				else if(!Q_stricmp(token+PREFIX_LEN, "lightSubdivide"))
				{
					GetTokenAppend(shaderText, qfalse);
					si->lightSubdivide = atoi(token);
				}

				/* kmap_backsplash <percent> <distance> */
				else if(!Q_stricmp(token+PREFIX_LEN, "backsplash"))
				{
					GetTokenAppend(shaderText, qfalse);
				...
				/* xmap_floodLight <r> <g> <b> <diste> <intensity> <light_direction_power> */
				else if(!Q_stricmp(token+PREFIX_LEN, "floodLight"))
				{
					/* get color */
					GetTokenAppend(shaderText, qfalse);
				...
				/* jal: xmap_nodirty : skip dirty */
				else if(!Q_stricmp(token+PREFIX_LEN, "nodirty"))
				{
					si->noDirty = qtrue;
				}

				/* kmap_lightmapSampleSize <value> */
				else if(!Q_stricmp(token+PREFIX_LEN, "lightmapSampleSize"))
				{
					GetTokenAppend(shaderText, qfalse);
					si->lightmapSampleSize = atoi(token);
				}

				/* kmap_lightmapSampleSffset <value> */
				else if(!Q_stricmp(token+PREFIX_LEN, "lightmapSampleOffset"))
				{
					GetTokenAppend(shaderText, qfalse);
					si->lightmapSampleOffset = atof(token);
				}

				/* ydnar: kmap_lightmapFilterRadius <self> <other> */
				else if(!Q_stricmp(token+PREFIX_LEN, "lightmapFilterRadius"))
				{
					GetTokenAppend(shaderText, qfalse);
				...
				/* ydnar: kmap_lightmapAxis [xyz] */
				else if(!Q_stricmp(token+PREFIX_LEN, "lightmapAxis"))
				{
					GetTokenAppend(shaderText, qfalse);
				...
				/* ydnar: kmap_lightmapSize <width> <height> (for autogenerated shaders + external tga lightmaps) */
				else if(!Q_stricmp(token+PREFIX_LEN, "lightmapSize") )
				{
					GetTokenAppend(shaderText, qfalse);
				...
				/* ydnar: kmap_lightmapBrightness N (for autogenerated shaders + external tga lightmaps) */
				else if(!Q_stricmp(token+PREFIX_LEN, "lightmapBrightness") || !Q_stricmp(token+PREFIX_LEN, "lightmapGamma"))
				{
					GetTokenAppend(shaderText, qfalse);
				...
				/* kmap_vertexScale (scale vertex lighting by this fraction) */
				else if(!Q_stricmp(token+PREFIX_LEN, "vertexScale"))
				{
					GetTokenAppend(shaderText, qfalse);
				...
				/* kmap_noVertexLight */
				else if(!Q_stricmp(token+PREFIX_LEN, "noVertexLight"))
				{
					si->noVertexLight = qtrue;
				}

				/* kmap_flare[Shader] <shader> */
				else if(!Q_stricmp(token+PREFIX_LEN, "flare") || !Q_stricmp(token+PREFIX_LEN, "flareShader"))
				{
					GetTokenAppend(shaderText, qfalse);
					if(token[0] != '\0')
					{
				...
				/* kmap_backShader <shader> */
				else if(!Q_stricmp(token+PREFIX_LEN, "backShader"))
				{
					GetTokenAppend(shaderText, qfalse);
					if(token[0] != '\0')
					{
				...
				/* ydnar: kmap_cloneShader <shader> */
				else if(!Q_stricmp(token+PREFIX_LEN, "cloneShader"))
				{
					GetTokenAppend(shaderText, qfalse);
					if(token[0] != '\0')
					{
				...
				/* kmap_remapShader <shader> */
				else if(!Q_stricmp(token+PREFIX_LEN, "remapShader"))
				{
					GetTokenAppend(shaderText, qfalse);
					if(token[0] != '\0')
					{
				...
				/* xmap_deprecateShader <shader> */
				else if(!Q_stricmp(token+PREFIX_LEN, "deprecateShader"))
				{
					GetTokenAppend(shaderText, qfalse);
					if(token[0] != '\0')
					{
				...
				/* ydnar: kmap_offset <value> */
				else if(!Q_stricmp(token+PREFIX_LEN, "offset"))
				{
					GetTokenAppend(shaderText, qfalse);
					si->offset = atof(token);
				}

				/* ydnar: kmap_textureSize <width> <height> (substitute for kmap_lightimage derivation for terrain) */
				else if(!Q_stricmp(token+PREFIX_LEN, "fur") )
				{
					GetTokenAppend(shaderText, qfalse);
				...
				else if(!Q_stricmp(token+PREFIX_LEN, "terrain"))
				{
				...
				/* ydnar: picomodel: kmap_forceMeta (forces brush faces and/or triangle models to go through the metasurface pipeline) */
				else if(!Q_stricmp(token+PREFIX_LEN, "forceMeta"))
				{
					si->forceMeta = qtrue;
				}

				/* ydnar: gs mods: kmap_shadeAngle <degrees> */
				else if(!Q_stricmp(token+PREFIX_LEN, "shadeAngle"))
				{
					GetTokenAppend(shaderText, qfalse);
				...
				/* ydnar: kmap_textureSize <width> <height> (substitute for kmap_lightimage derivation for terrain) */
				else if(!Q_stricmp(token+PREFIX_LEN, "textureSize"))
				{
					GetTokenAppend(shaderText, qfalse);
				...
				/* ydnar: gs mods: kmap_tcGen <style> <parameters> */
				else if(!Q_stricmp(token+PREFIX_LEN, "tcGen"))
				{
					si->tcGen = qtrue;
					GetTokenAppend(shaderText, qfalse);

					/* kmap_tcGen vector <s vector> <t vector> */
					...
					/* kmap_tcGen ivector <1.0/s vector> <1.0/t vector> (inverse vector, easier for mappers to understand) */
					...
					{
						Sys_Printf("WARNING: Unknown kmap_tcGen method: %s\n", token);
						VectorClear(si->vecs[0]);
						VectorClear(si->vecs[1]);
					}
				}

				/* ydnar: gs mods: kmap_[color|rgb|alpha][Gen|Mod] <style> <parameters> */
				else if(!Q_stricmp(token+PREFIX_LEN, "colorGen") || !Q_stricmp(token+PREFIX_LEN, "colorMod") ||
						!Q_stricmp(token+PREFIX_LEN, "rgbGen")   || !Q_stricmp(token+PREFIX_LEN, "rgbMod") ||
						!Q_stricmp(token+PREFIX_LEN, "alphaGen") || !Q_stricmp(token+PREFIX_LEN, "alphaMod"))
				{
					colorMod_t     *cm, *cm2;
					int             alpha;
					...
					alpha = (!Q_stricmp(token+PREFIX_LEN, "alphaGen") || !Q_stricmp(token+PREFIX_LEN, "alphaMod")) ? 1 : 0;
				...
				/* ydnar: gs mods: kmap_tcMod <style> <parameters> */
				else if(!Q_stricmp(token+PREFIX_LEN, "tcMod"))
				{
					float           a, b;
					...
					/* kmap_tcMod [translate | shift | offset] <s> <t> */
					...
					/* kmap_tcMod rotate <s> <t> (fixme: make this communitive) */
					else if(!Q_stricmp(token, "rotate"))
					{
						...
						Sys_Printf("WARNING: Unknown kmap_tcMod method: %s\n", token);
				}

				/* kmap_fogDir (direction a fog shader fades from transparent to opaque) */
				else if(!Q_stricmp(token+PREFIX_LEN, "fogDir"))
				{
				...
				/* kmap_globaltexture */
				else if(!Q_stricmp(token+PREFIX_LEN, "globaltexture"))
					si->globalTexture = qtrue;

				/* ydnar: gs mods: kmap_nonplanar (make it a nonplanar merge candidate for meta surfaces) */
				else if(!Q_stricmp(token+PREFIX_LEN, "nonplanar"))
					si->nonplanar = qtrue;

				/* ydnar: gs mods: kmap_noclip (preserve original face winding, don't clip by bsp tree) */
				else if(!Q_stricmp(token+PREFIX_LEN, "noclip"))
					si->noClip = qtrue;

				/* kmap_notjunc */
				else if(!Q_stricmp(token+PREFIX_LEN, "notjunc"))
					si->notjunc = qtrue;

				/* kmap_nofog */
				else if(!Q_stricmp(token+PREFIX_LEN, "nofog"))
					si->noFog = qtrue;

				/* ydnar: gs mods: kmap_indexed (for explicit terrain-style indexed mapping) */
				else if(!Q_stricmp(token+PREFIX_LEN, "indexed"))
					si->indexed = qtrue;

				/* ydnar: kmap_invert (inverts a drawsurface's facing) */
				else if(!Q_stricmp(token+PREFIX_LEN, "invert"))
					si->invert = qtrue;

				/* ydnar: gs mods: kmap_lightmapMergable (ok to merge non-planar */
				else if(!Q_stricmp(token+PREFIX_LEN, "lightmapMergable"))
					si->lmMergable = qtrue;

				/* ydnar: kmap_nofast */
				else if(!Q_stricmp(token+PREFIX_LEN, "noFast"))
					si->noFast = qtrue;

				/* kmap_patchshadows */
				else if(!Q_stricmp(token+PREFIX_LEN, "patchShadows"))
					si->patchShadows = qtrue;

				/* kmap_vertexshadows */
				else if(!Q_stricmp(token+PREFIX_LEN, "vertexShadows"))
					si->vertexShadows = qtrue;	/* ydnar */

				/* kmap_novertexshadows */
				else if(!Q_stricmp(token+PREFIX_LEN, "noVertexShadows"))
					si->vertexShadows = qfalse;	/* ydnar */

				/* kmap_splotchfix (filter dark lightmap luxels on lightmapped models) */
				else if(!Q_stricmp(token+PREFIX_LEN, "splotchfix"))
					si->splotchFix = qtrue;	/* ydnar */

				/* kmap_forcesunlight */
				else if(!Q_stricmp(token+PREFIX_LEN, "forceSunlight"))
					si->forceSunlight = qtrue;

				/* kmap_onlyvertexlighting (sof2) */
				else if(!Q_stricmp(token+PREFIX_LEN, "onlyVertexLighting"))
					ApplySurfaceParm("pointlight", &si->contentFlags, &si->surfaceFlags, &si->compileFlags);

				/* kmap_material (sof2) */
				else if(!Q_stricmp(token+PREFIX_LEN, "material"))
				{
					GetTokenAppend(shaderText, qfalse);
				...
				/* ydnar: kmap_clipmodel (autogenerate clip brushes for model triangles using this shader) */
				else if(!Q_stricmp(token+PREFIX_LEN, "clipmodel"))
					si->clipModel = qtrue;

				/* ydnar: kmap_styleMarker[2] */
				else if(!Q_stricmp(token+PREFIX_LEN, "styleMarker"))
					si->styleMarker = 1;
				else if(!Q_stricmp(token+PREFIX_LEN, "styleMarker2"))	/* uses depthFunc equal */
					si->styleMarker = 2;

				/* ydnar: default to searching for kmap_<surfaceparm> */
				else
				{
...
static void ParseCustomInfoParms(void)
{
	...
	/* file exists? */
	if(vfsGetFileCount("materials/custinfoparms.txt") == 0)
	{
		Sys_Printf("WARNING: Couldn't find custinfoparms.txt\n");
		return;
	}

	/* load it */
	LoadScriptFile("materials/custinfoparms.txt", 0);
